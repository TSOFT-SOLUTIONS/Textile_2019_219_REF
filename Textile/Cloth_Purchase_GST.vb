Public Class Cloth_Purchase_GST
    ...
    Private Pk_Condition As String = "GCLPR-"
    Private Pk_Condition2 As String = "GCPAC-"
    Private Pk_Condition3 As String = "GCPRC-"
    Private PkCondition4_TDSCP As String = "TDSCP-"
    Private Pk_Condition5 As String = "CPFRT-"
    ...
    Private Enum dgvCol_Details As Integer
        ...
        Net_Meters
        ORDER_NO
        ClothPurchase_Order_Code
        ClothPurchase_Order_Slno
        ClothPurchase_Deatils_Slno
        Invoice_Meters
        ...
        Try
            If Asc(e.KeyChar) = 27 Then
                ...
                ElseIf pnl_OpenRecord.Visible = True Then
                    btn_close_OpenRecord_Click(sender, e)
                    Exit Sub
    ...
    Private Sub Cloth_Purchae_Entry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1158" Then
            btn_SaveAll.Visible = True
        End If

        If Trim(Common_Procedures.settings.CustomerCode) = "1381" Then 'KRS TEx
            lbl_FreightFor.Visible = True
            txt_Freight_For.Visible = True
            txt_Transport_Freight_Caption.Visible = True
            txt_Transport_Freight.Visible = True
        Else
            lbl_FreightFor.Visible = False
            txt_Freight_For.Visible = False
            txt_Transport_Freight_Caption.Visible = False
            txt_Transport_Freight.Visible = False
        End If

        If Trim(Common_Procedures.settings.CustomerCode) = "1186" Then
            txt_TradeDic_Name.Visible = False

            txt_Trade_Disc.Visible = False
            lbl_Trade_Disc_Perc.Visible = False
            Label9.Visible = False
        End If

        If Val(Common_Procedures.settings.Multi_Godown_Status) = 1 Then
            cbo_Delivery_At.Visible = True
            lbl_Delivery_At.Visible = True
        Else
            cbo_Delivery_At.Visible = False
            lbl_Delivery_At.Visible = False
        End If

        lbl_Lot_No.Visible = False
        ...
        If Common_Procedures.settings.CustomerCode = "1516" Then
            ...
            If Trim(UCase(cbo_Type.Text)) = "DIRECT" Then
                ...
                'On Error Resume Next
                btn_ReceiptSelection.Enabled = False
                dgv_Details.Columns(dgvCol_Details.CLOTH_NAME).ReadOnly = False
                dgv_Details.Columns(dgvCol_Details.TYPE).ReadOnly = False
                dgv_Details.Columns(dgvCol_Details.FOLDING).ReadOnly = False
                dgv_Details.AllowUserToAddRows = True

            Else

                ' On Error Resume Next
                btn_ReceiptSelection.Enabled = True
                dgv_Details.Columns(dgvCol_Details.CLOTH_NAME).ReadOnly = True
                dgv_Details.Columns(dgvCol_Details.TYPE).ReadOnly = True
                dgv_Details.Columns(dgvCol_Details.FOLDING).ReadOnly = True
        ...
        Else
            ...
            If Val(Common_Procedures.settings.Multi_Godown_Status) = 0 Then
                dgv_Details.Location = New Point(9, 105)
                dgv_Details.Size = New Size(927, 155)
            End If

        End If


        '--  CODE BY GOPI 2024-01-22

        If Trim(Common_Procedures.settings.CustomerCode) = "1520" Then    '-- RAINBOW COTTON FABRIC 

            txt_Transport_Freight_Caption.Visible = True
            txt_Transport_Freight_Caption.Left = txt_AddLess_AfterTax.Left
            txt_Transport_Freight_Caption.Size = New Size(100, 16)
            txt_Transport_Freight_Caption.Text = "Transport  Freight"


            txt_Transport_Freight.Visible = True
            txt_Transport_Freight.Left = lbl_BillAmount_CAption.Left
        ...
        AddHandler cbo_Delivery_At.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler cbo_Delivery_At.LostFocus, AddressOf ControlLostFocus
        ...
        dgv_Selection.Columns(12).DisplayIndex = 3
            ...
            If IsNothing(dgv1) = False Then
                ...
                With dgv1
                    ...
                    If keyData = Keys.Enter Or keyData = Keys.Down Then
                        If .CurrentCell.ColumnIndex >= dgvCol_Details.RATE Then
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("Select a.* from Cloth_Purchase_Head a Where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " And a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and Entry_VAT_GST_Type = 'GST'", con)
            dt1 = New DataTable
            da1.Fill(dt1)
            ...
            If dt1.Rows.Count > 0 Then
                ...
                lbl_RoundOff.Text = dt1.Rows(0).Item("RoundOff_Amount").ToString
                ...
                txt_AddLess_AfterTax.Text = Format(Val(dt1.Rows(0).Item("Add_Less_AfterTax").ToString), "########0.00")
                ...
                txt_Freight_For.Text = Format(Val(dt1.Rows(0).Item("Freight_RateFor_Bundle").ToString), "#########0.00")
                ...
                If Not IsDBNull(dt1.Rows(0).Item("Deliver_At_IdNo")) Then
                    If dt1.Rows(0).Item("Deliver_At_IdNo") > 0 Then
                        cbo_Delivery_At.Text = Common_Procedures.Ledger_IdNoToName(con, dt1.Rows(0).Item("Deliver_At_IdNo"))
                    End If
                End If

                If Not IsDBNull(dt1.Rows(0).Item("Delivery_Purpose_IdNo")) Then
                    If dt1.Rows(0).Item("Delivery_Purpose_IdNo") > 0 Then
                        cbo_Delivery_Purpose.Text = Common_Procedures.Process_IdNoToName(con, dt1.Rows(0).Item("Delivery_Purpose_IdNo"))
                    End If
                End If

                If Not IsDBNull(dt1.Rows(0).Item("Lot_IdNo")) Then
                    If dt1.Rows(0).Item("Lot_IdNo") > 0 Then
                        cbo_LotNo.Text = Common_Procedures.Lot_IdNoToNo(con, dt1.Rows(0).Item("Lot_IdNo"))
                    End If
                End If
                ...
                With dgv_Details
                    ...
                    If dt2.Rows.Count > 0 Then
                        ...
                        For i = 0 To dt2.Rows.Count - 1
                            ...
                            .Rows(n).Cells(dgvCol_Details.CLOTH_NAME).Value = dt2.Rows(i).Item("Cloth_Name").ToString
                            .Rows(n).Cells(dgvCol_Details.TYPE).Value = dt2.Rows(i).Item("ClothType_Name").ToString
                            .Rows(n).Cells(dgvCol_Details.FOLDING).Value = Format(Val(dt2.Rows(i).Item("Fold_Perc").ToString), "########0.00")
                            .Rows(n).Cells(dgvCol_Details.PCS).Value = Val(dt2.Rows(i).Item("Pcs").ToString)
                            .Rows(n).Cells(dgvCol_Details.METERS).Value = Format(Val(dt2.Rows(i).Item("Meters").ToString), "########0.00")
                            ...
                            .Rows(n).Cells(dgvCol_Details.Trade_Disc_PERC).Value = Val(dt2.Rows(i).Item("Trade_Discount_Percentage").ToString)
                            .Rows(n).Cells(dgvCol_Details.Trade_Disc_Value).Value = Val(dt2.Rows(i).Item("Trade_Discount_Amount").ToString)
                            .Rows(n).Cells(dgvCol_Details.Cash_Disc_PERC).Value = Val(dt2.Rows(i).Item("Cash_Discount_Percentage").ToString)
                            .Rows(n).Cells(dgvCol_Details.Cash_Disc_Value).Value = Val(dt2.Rows(i).Item("Cash_Discount_Amount").ToString)
                            ...
                            .Rows(n).Cells(dgvCol_Details.HSN_Code).Value = (dt2.Rows(i).Item("HSN_Code").ToString)
                            .Rows(n).Cells(dgvCol_Details.Folding_Meters).Value = Val(dt2.Rows(i).Item("Folding_Meters").ToString)
                            .Rows(n).Cells(dgvCol_Details.Net_Meters).Value = Val(dt2.Rows(i).Item("Net_Meters").ToString)
                            .Rows(n).Cells(dgvCol_Details.ORDER_NO).Value = (dt2.Rows(i).Item("ClothPurchase_Order_No").ToString)
                            .Rows(n).Cells(dgvCol_Details.ClothPurchase_Order_Code).Value = (dt2.Rows(i).Item("ClothPurchase_Order_Code").ToString)
                            .Rows(n).Cells(dgvCol_Details.ClothPurchase_Order_Slno).Value = (dt2.Rows(i).Item("ClothPurchase_Order_Slno").ToString)
                            .Rows(n).Cells(dgvCol_Details.ClothPurchase_Deatils_Slno).Value = (dt2.Rows(i).Item("Cloth_Purchase_Detail_Slno").ToString)
                            .Rows(n).Cells(dgvCol_Details.Invoice_Meters).Value = Format((Val(dt2.Rows(i).Item("Invoice_Meters").ToString) + Val(dt2.Rows(i).Item("Return_Meters").ToString)), "########0.00")
                ...
                da4 = New SqlClient.SqlDataAdapter("Select a.* from Cloth_Purchase_GST_Tax_Details a Where a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' ", con)
                dt4 = New DataTable
                da4.Fill(dt4)
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Cloth_Purchase_Entry, "~L~") = 0 And InStr(Common_Procedures.UR.Cloth_Purchase_Entry, "~D~") = 0 Then MessageBox.Show("You have No Rights to Insert", "DOES NOT DELETE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error) : Exit Sub
        ...
        Da = New SqlClient.SqlDataAdapter("select sum(Invoice_Meters) from Cloth_Purchase_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
        ...
        Da = New SqlClient.SqlDataAdapter("select sum(Return_Meters) from Cloth_Purchase_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
            If IsDBNull(Dt1.Rows(0)(0).ToString) = False Then
        ...
        Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "DELETE", "Cloth_Purchase_Head", "Cloth_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, "", "", "Cloth_Purchase_Code, Company_IdNo, for_OrderBy", trans)

        Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "DELETE", "Cloth_Purchase_Details", "Cloth_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, "Cloth_IdNo,ClothType_IdNo,Fold_Perc,Pcs,Meters,Rate,Amount,Trade_Discount_Percentage,Trade_Discount_Amount,  Cash_Discount_Percentage,Cash_Discount_Amount,Taxable_Value,GST_Percentage,HSN_Code,Folding_Meters,Net_Meters, ClothPurchase_Order_No    ,   ClothPurchase_Order_Code ,  ClothPurchase_Order_Slno", "Sl_No", "Cloth_Purchase_Code, For_OrderBy, Company_IdNo, Cloth_Purchase_No, Cloth_Purchase_Date, Ledger_Idno", trans)
        ...
        Try
            ...
            Common_Procedures.Voucher_Deletion(con, Val(lbl_Company.Tag), Trim(PkCondition4_TDSCP) & Trim(NewCode), trans)
            Common_Procedures.Voucher_Deletion(con, Val(lbl_Company.Tag), Trim(Pk_Condition5) & Trim(NewCode), trans)

            cmd.CommandText = "Update ClothPurchase_order_Details set Purchase_Meters = a.Purchase_Meters - b.Meters from ClothPurchase_order_Details a, Cloth_Purchase_Details b Where b.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and b.Purchase_Selection_Type = 'ORDER' and a.ClothPurchase_Order_code = b.ClothPurchase_Order_code and a.ClothPurchase_Order_SlNo = b.ClothPurchase_Order_SlNo"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "delete from Cloth_Purchase_GST_Tax_Details  where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
            cmd.CommandText = "Update Stock_Cloth_Processing_Details set Reference_Date = b.Cloth_Purchase_Receipt_Date, UnChecked_Meters = b.Receipt_Meters, Meters_Type1 = 0, Meters_Type2 = 0, Meters_Type3 = 0, Meters_Type4 = 0, Meters_Type5 = 0 from Stock_Cloth_Processing_Details a, Cloth_Purchase_Receipt_Head b Where b.Cloth_Purchase_code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and b.Weaver_Piece_Checking_Code = '' and a.Reference_Code = '" & Trim(Pk_Condition3) & "' + b.Cloth_Purchase_Receipt_Code"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "Update Cloth_Purchase_Receipt_Head set Cloth_Purchase_code = '' where Cloth_Purchase_code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
            ...
            cmd.CommandText = "delete from Cloth_Purchase_Details where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "delete from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Cloth_Purchase_No from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' and Entry_VAT_GST_Type = 'GST' Order by for_Orderby, Cloth_Purchase_No", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Cloth_Purchase_No from Cloth_Purchase_Head where for_orderby > " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' and Entry_VAT_GST_Type = 'GST' Order by for_Orderby, Cloth_Purchase_No", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Cloth_Purchase_No from Cloth_Purchase_Head where for_orderby < " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' and Entry_VAT_GST_Type = 'GST' Order by for_Orderby desc, Cloth_Purchase_No desc", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select top 1 Cloth_Purchase_No from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' and Entry_VAT_GST_Type = 'GST' Order by for_Orderby desc, Cloth_Purchase_No desc", con)
            da.Fill(dt)
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        '    Da = New SqlClient.SqlDataAdapter("select Cloth_Purchase_No from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(InvCode) & "'", con)
        '    Da.Fill(Dt)
        ...
        ''    inpno = InputBox("Enter Purchase Bill No.", "FOR FINDING...")

        ''    'InvCode = Trim(Val(lbl_Company.Tag)) & "-" & Trim(inpno) & "/" & Trim(Common_Procedures.FnYearCode)

        ''    Da = New SqlClient.SqlDataAdapter("select Cloth_Purchase_No from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and upper(Bill_No) = '" & UCase(inpno) & "'", con)
        ''    Da.Fill(Dt)
        ...
        ''    Else
        ''        MessageBox.Show("Purchase Bill No. does not exists", "DOES NOT FIND...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Cloth_Purchase_Entry, "~L~") = 0 And InStr(Common_Procedures.UR.Cloth_Purchase_Entry, "~I~") = 0 Then MessageBox.Show("You have No Rights to Insert", "DOES NOT INSERT...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error) : Exit Sub
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Dim vTotPcs As String, vTotMtrs As String, vTotBals As String, vTotAmt As String
        ...
        Dim Del_At_IdNo As Int16 = 0
        Dim Del_Purpose_IdNo As Int16 = 0
        ...
        Lot_IdNo = 0
        If cbo_LotNo.Visible = True Then
            Lot_IdNo = Common_Procedures.Lot_NoToIdNo(con, cbo_LotNo.Text)
            If Lot_IdNo = 0 Then
        ...
        Del_At_IdNo = 0
        If Len(Trim(cbo_Delivery_At.Text)) > 0 And cbo_Delivery_At.Visible Then
            Del_At_IdNo = Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_Delivery_At.Text)
        End If
        If Del_At_IdNo = 0 Then Del_At_IdNo = Common_Procedures.CommonLedger.Godown_Ac
        ...
        Led_Type = Common_Procedures.get_FieldValue(con, "Ledger_Head", "Ledger_Type", "Ledger_IdNo = " & Del_At_IdNo.ToString)

        If Del_At_IdNo <> 0 And Led_Type <> "GODOWN" And Del_Purpose_IdNo = 0 Then
        ...
        If PurcAc_ID = 0 And Val(CDbl(lbl_Net_Amt.Text)) <> 0 Then
            MessageBox.Show("Invalid Purchase A/c", "DOES NOT SAVE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
        ...
        For i = 0 To dgv_Details.RowCount - 1
            ...
            If Val(dgv_Details.Rows(i).Cells(dgvCol_Details.PCS).Value) <> 0 Or Val(dgv_Details.Rows(i).Cells(dgvCol_Details.METERS).Value) <> 0 Or Val(dgv_Details.Rows(i).Cells(dgvCol_Details.AMOUNT).Value) <> 0 Then

                clth_ID = Common_Procedures.Cloth_NameToIdNo(con, dgv_Details.Rows(i).Cells(dgvCol_Details.CLOTH_NAME).Value)
                If clth_ID = 0 Then
                ...
                clthtyp_ID = Common_Procedures.ClothType_NameToIdNo(con, dgv_Details.Rows(i).Cells(dgvCol_Details.TYPE).Value)
                If clthtyp_ID = 0 Then
                    ...
                    If dgv_Details.Enabled And dgv_Details.Visible Then
                        dgv_Details.Focus()
                        dgv_Details.CurrentCell = dgv_Details.Rows(i).Cells(dgvCol_Details.TYPE)
                    End If
                    Exit Sub
                End If

                If Val(dgv_Details.Rows(i).Cells(dgvCol_Details.FOLDING).Value) = 0 Then
                    MessageBox.Show("Invalid Folding", "DOES NOT SAVE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
                    If dgv_Details.Enabled And dgv_Details.Visible Then
                        dgv_Details.Focus()
                        dgv_Details.CurrentCell = dgv_Details.Rows(i).Cells(dgvCol_Details.FOLDING)
                    End If
                    Exit Sub
                End If


                If Val(dgv_Details.Rows(i).Cells(dgvCol_Details.PCS).Value) = 0 And Val(dgv_Details.Rows(i).Cells(dgvCol_Details.METERS).Value) = 0 Then
                    MessageBox.Show("Invalid Purchase Pcs/Metres", "DOES NOT SAVE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
                    If dgv_Details.Enabled And dgv_Details.Visible Then
                        dgv_Details.Focus()
                        dgv_Details.CurrentCell = dgv_Details.Rows(i).Cells(dgvCol_Details.METERS)
                    End If
                    Exit Sub
                End If
        ...
        If dgv_Details_Total.RowCount > 0 Then
            ...
            vTotMtrs = Format(Val(dgv_Details_Total.Rows(0).Cells(5).Value()), "##########0.00")
            vTotAmt = Format(Val(dgv_Details_Total.Rows(0).Cells(7).Value()), "##########0.00")
        End If
        ...
        If Trim(txt_BillNo.Text) <> "" Then
            NewCode = Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RefNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
            Da = New SqlClient.SqlDataAdapter("select * from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Ledger_IdNo = " & Str(Val(Led_ID)) & " and Bill_no = '" & Trim(txt_BillNo.Text) & "' and Cloth_Purchase_code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' and Cloth_Purchase_code <> '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            Dt1 = New DataTable
            Da.Fill(Dt1)
            If Dt1.Rows.Count > 0 Then
        ...
        Try
            ...
            Else
                ...
                lbl_RefNo.Text = Common_Procedures.get_MaxCode(con, "Cloth_Purchase_Head", "Cloth_Purchase_Code", "For_OrderBy", "Entry_VAT_GST_Type = 'GST'", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
            ...
            Da = New SqlClient.SqlDataAdapter("select Weaver_Piece_Checking_Code, Cloth_Purchase_Code from Cloth_Purchase_Receipt_Head Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Receipt_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            ...
            Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "NEW", "Cloth_Purchase_Head", "Cloth_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "", "", "Cloth_Purchase_Code, Company_IdNo, for_OrderBy", tr)


            If Trim(UCase(cbo_Type.Text)) = "RECEIPT" And Trim(lbl_ReceiptCode.Text) <> "" Then

                cmd.CommandText = "Update Cloth_Purchase_Receipt_Head set Cloth_Purchase_code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' where Cloth_Purchase_Receipt_Code = '" & Trim(lbl_ReceiptCode.Text) & "' and Ledger_IdNo = " & Str(Val(Led_ID))
            ...
            cmd.CommandText = "Delete from Cloth_Purchase_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and Invoice_Meters = 0 "
            cmd.ExecuteNonQuery()
            ...
            cmd.CommandText = " Insert into Textile_Processing_Delivery_Head(ClothProcess_Delivery_Code                , Company_IdNo                     , ClothProcess_Delivery_No      , for_OrderBy                                                            , ClothProcess_Delivery_Date, Ledger_IdNo                   , Purchase_OrderNo  , Transport_IdNo          , Freight_Charges , Note                          ,Total_Pcs                         ,Total_Qty                 , Total_Meters                     , Total_Weight                  , Processing_Idno          , JobOrder_No                             ,  User_idNo  , Vehicle_No ,FabricPurchase_Weaver_Lot_IdNo   ,Lot_IdNo) " &
                                                          " Values           ('" & Pk_Condition & Trim(NewCode) & "'    , " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ", @PurchaseDate             , " & Str(Val(Del_At_IdNo)) & ", ''                , 0                       ,       0         ,  '" & Trim(txt_Note.Text) & "'," & Val(dgv_Details_Total.Rows(0).Cells(dgvCol_Details.PCS).Value).ToString & ",0                         , " & Val(dgv_Details_Total.Rows(0).Cells(dgvCol_Details.METERS).Value).ToString & ", 0  ,  " & Str(Val(Del_Purpose_IdNo)) & ",'" & lbl_RefNo.Text & "' ," & Val(Common_Procedures.User.IdNo) & " ,''                        ," & Lot_IdNo.ToString & "        ," & Lot_IdNo.ToString & ")"
            cmd.ExecuteNonQuery()
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(.Rows(i).Cells(dgvCol_Details.PCS).Value) <> 0 Or Val(.Rows(i).Cells(dgvCol_Details.METERS).Value) <> 0 Then

                        Sno = Sno + 1

                        clth_ID = Common_Procedures.Cloth_NameToIdNo(con, .Rows(i).Cells(dgvCol_Details.CLOTH_NAME).Value, tr)

                        clthtyp_ID = Common_Procedures.ClothType_NameToIdNo(con, .Rows(i).Cells(dgvCol_Details.TYPE).Value, tr)
                        PurCd = ""
                        OrdCd = ""
                        OrdSlNo = 0
                        OrdNo = ""
                        If Trim(UCase(cbo_Type.Text)) = "ORDER" Then
                            OrdNo = Trim(.Rows(i).Cells(dgvCol_Details.ORDER_NO).Value)
                            OrdCd = Trim(.Rows(i).Cells(dgvCol_Details.ClothPurchase_Order_Code).Value)

                            OrdSlNo = Val(.Rows(i).Cells(dgvCol_Details.ClothPurchase_Order_Slno).Value)
                        End If
                        If Trim(YrnClthNm) = "" Then YrnClthNm = Trim(.Rows(i).Cells(dgvCol_Details.CLOTH_NAME).Value)

                        Nr = 0
                        cmd.CommandText = "Update  Cloth_Purchase_Details set Cloth_Purchase_Date = @PurchaseDate , Purchase_Selection_Type = '" & Trim(UCase(cbo_Type.Text)) & "',Ledger_Idno = " & Val(Led_ID) & ",  Sl_No  = " & Str(Val(Sno)) & " , Cloth_IdNo = " & Str(Val(clth_ID)) & " , ClothType_IdNo = " & Str(Val(clthtyp_ID)) & " , Fold_Perc =  " & Str(Val(.Rows(i).Cells(3).Value)) & ", Pcs = " & Str(Val(.Rows(i).Cells(4).Value)) & " ,   Meters = " & Val(.Rows(i).Cells(5).Value) & ",      Rate   = " & Str(Val(.Rows(i).Cells(6).Value)) & ",  Amount = " & Str(Val(.Rows(i).Cells(7).Value)) & ",Trade_Discount_Percentage = " & Str(Val(.Rows(i).Cells(8).Value)) & ",Trade_Discount_Amount = " & Val((.Rows(i).Cells(9).Value)) & "    ,Cash_Discount_Percentage = " & Val((.Rows(i).Cells(10).Value)) & "    , Cash_Discount_Amount =" & Val((.Rows(i).Cells(11).Value)) & "   ,Taxable_Value = " & Val((.Rows(i).Cells(12).Value)) & "  ,GST_Percentage= " & Val((.Rows(i).Cells(13).Value)) & "   ,HSN_Code = '" & Trim((.Rows(i).Cells(14).Value)) & "'  ,Folding_Meters =" & Val((.Rows(i).Cells(15).Value)) & "  ,Net_Meters =" & Val((.Rows(i).Cells(16).Value)) & "      where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'  and Cloth_Purchase_Detail_SlNo = " & Str(Val(.Rows(i).Cells(20).Value))
                        Nr = cmd.ExecuteNonQuery()

                        If Nr = 0 Then

                            cmd.CommandText = "Insert into Cloth_Purchase_Details (    Cloth_Purchase_Code ,               Company_IdNo       ,      Cloth_Purchase_No        ,                               for_OrderBy                              , Cloth_Purchase_Date,         Purchase_Selection_Type                                   ,   Ledger_idno ,       Sl_No        ,        Cloth_IdNo        ,       ClothType_IdNo        ,                      Fold_Perc                                       ,                             Pcs                             ,                      Meters                                   ,                     Rate                                ,                       Amount                                   ,   Trade_Discount_Percentage                                          ,Trade_Discount_Amount                                                  ,  Cash_Discount_Percentage                                             ,Cash_Discount_Amount                                                  ,Taxable_Value                                                        ,GST_Percentage                                                  ,HSN_Code                                                      ,Folding_Meters                                                   ,Net_Meters                                                        , ClothPurchase_Order_No    ,   ClothPurchase_Order_Code ,  ClothPurchase_Order_Slno  ) " &
                                                    "     Values                  ( '" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ",    @PurchaseDate   ,          '" & Trim(UCase(cbo_Type.Text)) & "', " & Val(Led_ID) & " , " & Str(Val(Sno)) & ", " & Str(Val(clth_ID)) & ", " & Str(Val(clthtyp_ID)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.FOLDING).Value)) & ",  " & Str(Val(.Rows(i).Cells(dgvCol_Details.PCS).Value)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.METERS).Value)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.RATE).Value)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.AMOUNT).Value)) & " ," & Str(Val(.Rows(i).Cells(dgvCol_Details.Trade_Disc_PERC).Value)) & "," & Str(Val(.Rows(i).Cells(dgvCol_Details.Trade_Disc_Value).Value)) & "," & Str(Val(.Rows(i).Cells(dgvCol_Details.Cash_Disc_PERC).Value)) & "  ," & Str(Val(.Rows(i).Cells(dgvCol_Details.Cash_Disc_Value).Value)) & "," & Str(Val(.Rows(i).Cells(dgvCol_Details.Taxable_Value).Value)) & "," & Str(Val(.Rows(i).Cells(dgvCol_Details.Gst_PERC).Value)) & ",'" & Trim(.Rows(i).Cells(dgvCol_Details.HSN_Code).Value) & "',  " & Val((.Rows(i).Cells(dgvCol_Details.Folding_Meters).Value)) & "," & Val((.Rows(i).Cells(dgvCol_Details.Net_Meters).Value)) & "   ,'" & Trim(OrdNo) & "'     ,   '" & Trim(OrdCd) & "'    ,   " & Val(OrdSlNo) & "      ) "
                            cmd.ExecuteNonQuery()

                        End If

                        If Trim(UCase(cbo_Type.Text)) = "ORDER" And Trim(OrdCd) <> "" Then
                            Nr = 0
                            cmd.CommandText = "Update ClothPurchase_Order_Details set Purchase_Meters = Purchase_Meters + " & Str(Val(dgv_Details.Rows(i).Cells(dgvCol_Details.METERS).Value)) & " Where ClothPurchase_Order_Code = '" & Trim(OrdCd) & "' and ClothPurchase_Order_Slno = " & Str(Val(OrdSlNo)) & " and Ledger_IdNo = " & Str(Val(Led_ID))
                            Nr = cmd.ExecuteNonQuery()
                            If Nr = 0 Then
                        ...
                        Da = New SqlClient.SqlDataAdapter("Select Stock_In , Meters_Pcs from  Cloth_Head Where Cloth_idno = " & Str(Val(clth_ID)), con)
                        Da.SelectCommand.Transaction = tr
                        Dt2 = New DataTable
                        Da.Fill(Dt2)
                        If Dt2.Rows.Count > 0 Then
                        ...
                        If Trim(UCase(clthStock_In)) = "PCS" Then
                            vCloStk_QTY = Val(.Rows(i).Cells(4).Value)
                        Else
                            vCloStk_QTY = Val(.Rows(i).Cells(5).Value)
                        End If
                        ...
                        If Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1186" Then
                            ...
                            If Trim(UCase(cbo_Type.Text)) = "RECEIPT" Then
                                ...
                                If Trim(PcsChkCode) = "" Then
                                    cmd.CommandText = "Update Stock_Cloth_Processing_Details set Reference_Date = @PurchaseDate , Meters_Type" & Trim(Val(clthtyp_ID)) & " = Meters_Type" & Trim(Val(clthtyp_ID)) & " + " & Str(Val(vCloStk_QTY)) & " where Reference_Code = '" & Trim(Pk_Condition3) & Trim(lbl_ReceiptCode.Text) & "'"
                                    cmd.ExecuteNonQuery()
                                End If

                            Else

                                cmd.CommandText = "Insert into Stock_Cloth_Processing_Details ( Reference_Code,             Company_IdNo         ,           Reference_No        ,              for_OrderBy  , Reference_Date,                                            StockOff_IdNo   ,          DeliveryTo_Idno     ,     ReceivedFrom_Idno   ,         Entry_ID     ,       Party_Bill_No  ,       Particulars      ,           Sl_No      ,           Cloth_Idno     ,                      Folding                                   ,   Meters_Type" & Trim(Val(clthtyp_ID)) & ", Lot_IdNo                 ) " &
                                                        " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ",  @PurchaseDate, " & Str(Val(Common_Procedures.CommonLedger.OwnSort_Ac)) & ", " & Str(Val(Del_At_IdNo)) & ", " & Str(Val(Led_ID)) & ", '" & Trim(EntID) & "', '" & Trim(PBlNo) & "', '" & Trim(Partcls) & "', " & Str(Val(Sno)) & ", " & Str(Val(clth_ID)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.FOLDING).Value)) & ",  " & Str(Val(vCloStk_QTY)) & "             , " & Lot_IdNo.ToString & ") "
                                    cmd.ExecuteNonQuery()

                                ''----------------------
                                'If Val(.Rows(i).Cells(dgvCol_Details.METERS).Value) <> 0 And (Del_Purpose_IdNo = 0 Or Del_At_IdNo = 0) Then
                                ...
                                '                                " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ", @PurchaseDate , " & Str(Val(Common_Procedures.CommonLedger.OwnSort_Ac)) & ", " & Str(Val(Del_At_IdNo.ToString)) & ", " & Str(Val(Led_ID)) & ", '" & Trim(EntID) & "', '" & Trim(PBlNo) & "', '" & Trim(Partcls) & "',   1  , " & Str(Val(clth_ID)) & ", " & Str(Val(.Rows(i).Cells(dgvCol_Details.FOLDING).Value)) & ", " & Str(Val(vCloStk_QTY)) & ",       0      ,       0     ,       0     ,       0     ,       0      ," & Lot_IdNo.ToString & ") "
                                '    cmd.ExecuteNonQuery()

                                'End If
                                ''--------------

                                If Val(.Rows(i).Cells(dgvCol_Details.METERS).Value) <> 0 And Del_At_IdNo <> 0 And Del_Purpose_IdNo <> 0 Then
                                    ...
                                    cmd.CommandText = "Insert into Textile_Processing_Delivery_Details(Cloth_Processing_Delivery_Code               , Company_IdNo                     , Cloth_Processing_Delivery_No  , for_OrderBy                                                                , Cloth_Processing_Delivery_Date,Sl_No               , Ledger_IdNo                   ,  Item_Idno             ,Item_To_Idno             , Colour_Idno ,Processing_Idno                 ,   Bales     ,  Bales_Nos    ,  Delivery_Pcs                                           ,Delivery_Qty ,Meter_Qty                         ,Delivery_Meters                                                ,Delivery_Weight                , PackingSlip_Codes , ClothProcessing_Delivery_PackingSlno ,FabricPurchase_Weaver_Lot_IdNo ,Folding                                                       ,Lot_IdNo                 ) " &
                                                                                             "Values  ('" & Pk_Condition & Trim(NewCode) & "'       , " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ", @PurchaseDate                 ," & i.ToString & "  , " & Str(Val(Del_At_IdNo)) & " ," & Str(Val(clth_ID)) & ", " & Str(Val(clth_ID)) & ", 0           , " & Val(Del_Purpose_IdNo) & " ,0           ,''           , " & Str(Val(.Rows(i).Cells(dgvCol_Details.PCS).Value)) & ", 0           ,      0                            , " & Str(Val(.Rows(i).Cells(dgvCol_Details.METERS).Value)) & ", 0                             ,''                 ,''                                    ," & Lot_IdNo.ToString & "      ," & Str(Val(.Rows(i).Cells(dgvCol_Details.FOLDING).Value)) & "," & Lot_IdNo.ToString & ")"
                                    cmd.ExecuteNonQuery()
                ...
                Next
                Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "NEW", "Cloth_Purchase_Details", "Cloth_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "Cloth_IdNo,ClothType_IdNo,Fold_Perc,Pcs,Meters,Rate,Amount,Trade_Discount_Percentage,Trade_Discount_Amount,  Cash_Discount_Percentage,Cash_Discount_Amount,Taxable_Value,GST_Percentage,HSN_Code,Folding_Meters,Net_Meters, ClothPurchase_Order_No    ,   ClothPurchase_Order_Code ,  ClothPurchase_Order_Slno", "Sl_No", "Cloth_Purchase_Code, For_OrderBy, Company_IdNo, Cloth_Purchase_No, Cloth_Purchase_Date, Ledger_Idno", tr)
            ...
            With dgv_Tax_Details
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    If Val(.Rows(i).Cells(3).Value) <> 0 Or Val(.Rows(i).Cells(5).Value) <> 0 Or Val(.Rows(i).Cells(7).Value) <> 0 Then
                        ...
                        cmd.CommandText = "Insert into Cloth_Purchase_GST_Tax_Details   ( Cloth_Purchase_Code                       ,               Company_IdNo       ,      Cloth_Purchase_No        ,                               for_OrderBy                              , Cloth_Purchase_Date    ,         Ledger_IdNo     ,            Sl_No     , HSN_Code                               ,Taxable_Amount                            ,CGST_Percentage                           ,CGST_Amount                               ,SGST_Percentage                            ,SGST_Amount                              ,IGST_Percentage                          ,IGST_Amount                              ,                                 Gst_Percent    ) " &
            ...
            If Val(Agt_Idno) <> 0 Then
                ...
                cmd.CommandText = "Insert into AgentCommission_Processing_Details (  Reference_Code   ,             Company_IdNo         ,            Reference_No       ,                               For_OrderBy                              , Reference_Date, Commission_For,     Ledger_IdNo    ,           Agent_IdNo      ,         Entry_ID     ,      Party_BillNo    ,       Particulars      ,      Yarn_Cloth_Name     ,         Bags_Meters       ,               Amount              ,             Commission_Type      ,       Commission_Rate              ,            Commission_Amount     ) " &
                                                " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(vOrdByNo)) & ",   @PurchaseDate,     'CLOTH'   , " & Str(Led_ID) & ", " & Str(Val(Agt_Idno)) & ", '" & Trim(EntID) & "', '" & Trim(PBlNo) & "', '" & Trim(Partcls) & "', '" & Trim(YrnClthNm) & "', " & Str(Val(vTotMtrs)) & ", " & Val(CDbl(lbl_Net_Amt.Text)) & ", '" & Trim(cbo_Com_Type.Text) & "', " & Str(Val(txt_com_per.Text)) & ", " & Str(Val(txt_CommAmt.Text)) & ")"
                cmd.ExecuteNonQuery()
            ...
            cmd.CommandText = "insert into Entry_GST_Tax_Details_Temp(GST_Percentage, CGST_Amount, SGST_Amount, IGST_Amount) select (CGST_Percentage + SGST_Percentage + IGST_Percentage), sum(CGST_Amount), sum(SGST_Amount), sum(IGST_Amount) from Cloth_Purchase_GST_Tax_Details where Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and (CGST_Percentage <> 0 or SGST_Percentage <> 0 or IGST_Percentage <> 0) and (CGST_Amount <> 0 or SGST_Amount <> 0 or IGST_Amount <> 0) Group by CGST_Percentage,  SGST_Percentage,  IGST_Percentage Having sum(CGST_Amount) <> 0 or sum(SGST_Amount) <> 0 or sum(IGST_Amount) <> 0"
            cmd.ExecuteNonQuery()

            Dim vVOUPOS_GSTAC_IDNOS As String = "", vVOUPOS_GST_AMTS As String = "", vNarr As String = ""
            ...
            Dim vASSVal As String = 0
            Dim vVouPos_Narr As String = ""

            vASSVal = Format(Val(vTotAmt) - Val(lbl_Trade_Disc_Perc.Text) - Val(lbl_Cash_Disc_Perc.Text) + Val(txt_Insurance.Text) + Val(txt_Freight.Text) + Val(txt_Packing.Text), "###########0.00")
            vLed_IdNos = Led_ID & "|" & PurcAc_ID & "|" & Trim(vVOUPOS_GSTAC_IDNOS) & "|" & Common_Procedures.CommonLedger.ADDLESS_AMOUNT_AC & "|" & Common_Procedures.CommonLedger.TCS_RECEIVABLE_AC & "|" & Common_Procedures.CommonLedger.ROUNDOFF_AC

            vVou_Amts = Val(CDbl(lbl_BillAmount.Text)) & "|" & -1 * Val(vASSVal) & "|" & Trim(vVOUPOS_GST_AMTS) & "|" & -1 * Val(txt_AddLess_AfterTax.Text) & "|" & -1 * Val(lbl_TcsAmount.Text) & "|" & -1 * (Val(lbl_RoundOff.Text) + Val(lbl_RoundOff_Invoice_Value_Before_TCS.Text))

            'vLed_IdNos = Led_ID & "|" & PurcAc_ID & "|" & Common_Procedures.CommonLedger.CGST_AC & "|" & Common_Procedures.CommonLedger.SGST_AC & "|" & Common_Procedures.CommonLedger.IGST_AC & "|" & Common_Procedures.CommonLedger.ADDLESS_AMOUNT_AC & "|" & Common_Procedures.CommonLedger.TCS_RECEIVABLE_AC & "|" & Common_Procedures.CommonLedger.ROUNDOFF_AC
            'vVou_Amts = Val(CDbl(lbl_BillAmount.Text)) & "|" & -1 * Val(vASSVal) & "|" & -1 * Val(lbl_CGST_Amount.Text) & "|" & -1 * Val(lbl_SGST_Amount.Text) & "|" & -1 * Val(lbl_IGST_Amount.Text) & "|" & -1 * Val(txt_AddLess_AfterTax.Text) & "|" & -1 * Val(lbl_TcsAmount.Text) & "|" & -1 * (Val(lbl_RoundOff.Text) + Val(lbl_RoundOff_Invoice_Value_Before_TCS.Text))

            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1186" Then
                If Trim(vRmrks) <> "" Then
                    vNarr = "Bill No : " & Trim(txt_BillNo.Text) & " , Remarks :  " & Trim(vRmrks)
                Else
            ...
            If Common_Procedures.Voucher_Updation(con, "GST-CLOT.Purc", Val(lbl_Company.Tag), Trim(Pk_Condition) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(msk_date.Text), Trim(vNarr), vLed_IdNos, vVou_Amts, ErrMsg, tr, Common_Procedures.SoftwareTypes.Textile_Software) = False Then
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1035" Then '---- Kalaimagal Textiles (Avinashi)
                If Val(txt_CommAmt.Text) <> 0 Then
                    ...
                    If Common_Procedures.Voucher_Updation(con, "gst-Ag.Comm", Val(lbl_Company.Tag), Trim(Pk_Condition2) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(msk_date.Text), "Bill No : " & Trim(txt_BillNo.Text), vLed_IdNos, vVou_Amts, ErrMsg, tr, Common_Procedures.SoftwareTypes.Textile_Software) = False Then
                        Throw New ApplicationException(ErrMsg)
                    End If
            ...
            Common_Procedures.Voucher_Deletion(con, Val(lbl_Company.Tag), Trim(PkCondition4_TDSCP) & Trim(NewCode), tr)
            ...
            If Common_Procedures.Voucher_Updation(con, "ClothPurc.Tds", Val(lbl_Company.Tag), Trim(PkCondition4_TDSCP) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(msk_date.Text), "Bill No : " & Trim(txt_BillNo.Text) & " , ClothPurc.No : " & Trim(lbl_RefNo.Text), vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                Throw New ApplicationException(ErrMsg)
            End If
            ...
            ''Transport A/C Post
            If Trim(Common_Procedures.settings.CustomerCode) = "1520" Then
                ...
                '  vVouPos_Narr = "Sal : Inv No. " & Trim(lbl_RefNo.Text) & " "

                If Common_Procedures.Voucher_Updation(con, "ClothPurc.Frgt", Val(lbl_Company.Tag), Trim(Pk_Condition5) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(dtp_Date.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                End If
            ...
            '-----Bill Posting
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), Convert.ToDateTime(msk_date.Text), Led_ID, Trim(txt_BillNo.Text), Agt_Idno, Val(CDbl(lbl_Net_Amt.Text)), "CR", Trim(Pk_Condition) & Trim(NewCode), tr, Common_Procedures.SoftwareTypes.Textile_Software, SaveAll_STS)
        ...
        With dgv_Details_Total
            ...
            With dgv_Details_Total
                If .Rows.Count > 0 Then
                    tlamt = (Val(.Rows(0).Cells(dgvCol_Details.AMOUNT).Value))
                    tlmtr = (Val(.Rows(0).Cells(dgvCol_Details.METERS).Value))
    ...
    Private Sub NetAmount_Calculation()
        ...
        Dim vTotAfter_Disc As String = 0
        ...
        With dgv_Details_Total
            If .Rows.Count > 0 Then
                GrsAmt = Format(Val(.Rows(0).Cells(dgvCol_Details.AMOUNT).Value), "##########0.00")
            End If
        End With
        ...
        vTotAfter_Disc = Format(GrsAmt - Val(lbl_Cash_Disc_Perc.Text) - Val(lbl_Trade_Disc_Perc.Text), "########0.00")
        ...
        vInvAmt_Bfr_TCS = Format(Val(GrsAmt) - Val(lbl_Trade_Disc_Perc.Text) - Val(lbl_Cash_Disc_Perc.Text) + Val(txt_Insurance.Text) + Val(txt_Freight.Text) + Val(txt_Packing.Text) + Val(GST_Amt), "###########0.00")
        ...
        Dim vTDS_DED_STS As Boolean = False
        Dim vTDS_Led_PERC As String = "0"
        ...
        If chk_TDS_Tax.Checked = True Then
            ...
            If DateDiff("d", vTDS_StartDate.Date, dtp_Date.Value.Date) > 0 Then
                ...
                Led_ID = Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_PartyName.Text)
                vTDS_Led_STS = Common_Procedures.get_FieldValue(con, "ledger_head", "Purchase_TDS_Deduction_Status", "(ledger_idno = " & Str(Val(Led_ID)) & ")")
            ...
            If vTDS_DED_STS = True Then
                ...
                If txt_TDS_TaxableValue.Enabled = False Then
                    ...
                    ElseIf (Val(CDbl(lbl_TotalSales_Amount_Current_Year.Text)) + Val(vTOT_SalAmt)) > Val(vMIN_TDS_assval) Then
                        vTDS_AssVal = Format(Val(CDbl(lbl_TotalSales_Amount_Current_Year.Text)) + Val(vTOT_SalAmt) - Val(vMIN_TDS_assval), "############0")
                    ...
                    If Common_Procedures.settings.CustomerCode = "1087" Then ' Kalaimagal Palladam
                        txt_TDS_TaxableValue.Text = Format(Val(vTotAfter_Disc), "############0.00")
                    ElseIf Common_Procedures.settings.CustomerCode = "1098" Then ' Kalaimagal Palladam
                        txt_TDS_TaxableValue.Text = Format(Val(lbl_BillAmount.Text), "############0.00")
                    Else
                        txt_TDS_TaxableValue.Text = Format(Val(vTOT_SalAmt), "############0.00")
                    End If
                    ...
                    If Val(txt_TDS_TaxableValue.Text) > 0 Then
                        If Val(txt_TdsPerc.Text) = 0 Then
                            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1530" Then
                                vTDS_Led_PERC = Common_Procedures.get_FieldValue(con, "ledger_head", "Tds_Perc", "(ledger_idno = " & Str(Val(Led_ID)) & ")")
                                txt_TdsPerc.Text = Val(vTDS_Led_PERC)
                            Else
                ...
                'If Common_Procedures.settings.CustomerCode = "1087" Then ' Kalaimagal Palladam
                '    vTDS_Amt = Format(Val(GrsAmt) * Val(txt_TdsPerc.Text) / 100, "##########0")
                'Else
        ...
        'NtAmt = Format(Val(lbl_Invoice_Value_Before_TCS.Text) + Val(lbl_TcsAmount.Text) + Val(txt_AddLess_AfterTax.Text), "##########0.00")

        'lbl_Net_Amt.Text = Format(Val(NtAmt), "###########0")
        'lbl_RoundOff.Text = Format(Val(lbl_Net_Amt.Text) - Val(NtAmt), "###########0.00")
        'lbl_Net_Amt.Text = Common_Procedures.Currency_Format(Val(lbl_Net_Amt.Text))


        Bill_Amt = Format(Val(lbl_Invoice_Value_Before_TCS.Text) + Val(lbl_TcsAmount.Text) + Val(txt_AddLess_AfterTax.Text), "##########0.00")

        lbl_BillAmount.Text = Format(Val(Bill_Amt), "##########0")
        lbl_BillAmount.Text = Format(Val(lbl_BillAmount.Text), "##########0.00")

        lbl_RoundOff.Text = Format(Val(lbl_BillAmount.Text) - Val(Bill_Amt), "#########0.00")
    ...
    Private Sub Amount_Calculation(ByVal CurRow As Integer, ByVal CurCol As Integer)
        ...
        With dgv_Details
            If .Visible Then
                ...
                If CurCol = dgvCol_Details.FOLDING Or CurCol = dgvCol_Details.PCS Or CurCol = dgvCol_Details.METERS Or CurCol = dgvCol_Details.RATE Or CurCol = dgvCol_Details.AMOUNT Then

                    If CurCol = dgvCol_Details.FOLDING Or CurCol = dgvCol_Details.PCS Or CurCol = dgvCol_Details.METERS Or CurCol = dgvCol_Details.RATE Then
                        If Val(.Rows(CurRow).Cells(dgvCol_Details.PCS).Value) <> 0 And Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value) = 0 Then
                            .Rows(CurRow).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(.Rows(CurRow).Cells(dgvCol_Details.PCS).Value) * Val(.Rows(CurRow).Cells(dgvCol_Details.RATE).Value), "#########0.00")

                        Else

                            If (Val(.Rows(CurRow).Cells(dgvCol_Details.FOLDING).Value) = 0 Or Val(.Rows(CurRow).Cells(dgvCol_Details.FOLDING).Value) = 100 Or chk_No_Folding.Checked = True) And Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value) <> 0 Then

                                .Rows(CurRow).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value) * Val(.Rows(CurRow).Cells(dgvCol_Details.RATE).Value), "#########0.00")

                            Else

                                fmt = Format(((100 - Val(.Rows(CurRow).Cells(dgvCol_Details.FOLDING).Value)) / 100) * Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value), "##########0.00")
                                ...
                                If (100 - Val(.Rows(CurRow).Cells(dgvCol_Details.FOLDING).Value)) > 0 Then
                                    fldmtr = Format(Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value) - Val(fmt), "#########0.00")
                                Else
                                    fldmtr = Format(Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value) + Val(fmt), "#########0.00")
                                End If

                                .Rows(CurRow).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(fldmtr) * Val(.Rows(CurRow).Cells(dgvCol_Details.RATE).Value), "#########0.00")

                            End If

                            .Rows(CurRow).Cells(dgvCol_Details.Folding_Meters).Value = Format(Val(fmt), "#########0.00")
                            If Val(fldmtr) <> 0 Then
                                .Rows(CurRow).Cells(dgvCol_Details.Net_Meters).Value = Format(Val(fldmtr), "#########0.00")
                            Else
                                .Rows(CurRow).Cells(dgvCol_Details.Net_Meters).Value = Format(Val(.Rows(CurRow).Cells(dgvCol_Details.METERS).Value), "#########0.00")
                            End If
    ...
    Private Sub Total_Calculation()
        ...
        With dgv_Details
            For i = 0 To .RowCount - 1
                ...
                If Val(.Rows(i).Cells(dgvCol_Details.PCS).Value) <> 0 Or Val(.Rows(i).Cells(dgvCol_Details.METERS).Value) <> 0 Then
                    ...
                    TotPcs = Format(Val(TotPcs) + Val(.Rows(i).Cells(dgvCol_Details.PCS).Value()), "###########0.00")
                    TotMtrs = Format(Val(TotMtrs) + Val(.Rows(i).Cells(dgvCol_Details.METERS).Value()), "###########0.00")
                    TotAmt = Format(Val(TotAmt) + Val(.Rows(i).Cells(dgvCol_Details.AMOUNT).Value()), "###########0.00")
                    Ttl_TradeDisc = Format(Val(Ttl_TradeDisc) + Val(.Rows(i).Cells(dgvCol_Details.Trade_Disc_Value).Value()), "###########0.00")
                    Ttl_CashDisc = Format(Val(Ttl_CashDisc) + Val(.Rows(i).Cells(dgvCol_Details.Cash_Disc_Value).Value()), "###########0.00")
                    Ttl_Taxable_Amount = Format(Val(Ttl_Taxable_Amount) + Val(.Rows(i).Cells(dgvCol_Details.Taxable_Value).Value()), "###########0.00")
        ...
        With dgv_Details_Total
            ...
            .Rows(0).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(TotAmt), "########0.00")
            .Rows(0).Cells(dgvCol_Details.Trade_Disc_Value).Value = Format(Val(Ttl_TradeDisc), "########0.00")
            .Rows(0).Cells(dgvCol_Details.Cash_Disc_Value).Value = Format(Val(Ttl_CashDisc), "########0.00")
            .Rows(0).Cells(dgvCol_Details.Taxable_Value).Value = Format(Val(Ttl_Taxable_Amount), "########0.00")
    ...
    Private Sub cbo_PartyName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PartyName.KeyDown
        ...
        If e.KeyValue = 40 And cbo_PartyName.DroppedDown = False Then
            If cbo_Type.Visible And cbo_Type.Enabled Then
                cbo_Type.Focus()
            ElseIf btn_ReceiptSelection.Visible And btn_ReceiptSelection.Enabled Then
                btn_ReceiptSelection.Focus()
            Else
                txt_BillNo.Focus()
            End If
        End If
    ...
    Private Sub cbo_PartyName_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PartyName.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            If cbo_Type.Visible And cbo_Type.Enabled Then
                cbo_Type.Focus()
            Else
                If cbo_Type.Enabled = False Then
                    If Trim(UCase(cbo_Type.Text)) = "RECEIPT" Then
    ...
    Public Sub Get_vehicle_from_Transport()
        ...
        Da.Fill(Dt)
        If Dt.Rows.Count <> 0 Then
            txt_Vechile.Text = Dt.Rows(0).Item("vehicle_no").ToString
        ...
        With dgv_Details
            ...
            If (e.KeyValue = 38 And cbo_Grid_ClothName.DroppedDown = False) Or (e.Control = True And e.KeyValue = 38) Then
                If .CurrentCell.RowIndex <= 0 Then
                    ...
                    If cbo_LotNo.Visible And cbo_LotNo.Enabled Then
                        cbo_LotNo.Focus()
                    ElseIf cbo_Delivery_At.Visible And cbo_Delivery_At.Enabled Then
                        cbo_Delivery_At.Focus()
    ...
    Private Sub cbo_Grid_ClothName_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Grid_ClothName.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            With dgv_Details
                ...
                Else
                    .Focus()
                    .CurrentCell = .Rows(.CurrentRow.Index).Cells(dgvCol_Details.TYPE)
    ...
    Private Sub dgv_Details_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellEnter
        ...
        With dgv_Details
            ...
            If Trim(.CurrentRow.Cells(dgvCol_Details.TYPE).Value) = "" Then
                .CurrentRow.Cells(dgvCol_Details.TYPE).Value = Common_Procedures.ClothType_IdNoToName(con, 1)
            End If

            If Val(.CurrentRow.Cells(dgvCol_Details.FOLDING).Value) = 0 Then
                .CurrentRow.Cells(dgvCol_Details.FOLDING).Value = "100"
            End If
    ...
    Private Sub dgv_Details_CellLeave(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellLeave
        With dgv_Details
            If .CurrentCell.ColumnIndex = dgvCol_Details.METERS Or .CurrentCell.ColumnIndex = dgvCol_Details.RATE Or .CurrentCell.ColumnIndex = dgvCol_Details.AMOUNT Then
    ...
    Private Sub dgv_Details_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellValueChanged
        ...
        With dgv_Details
            If .Visible Then
                ...
                If e.ColumnIndex = dgvCol_Details.FOLDING Or e.ColumnIndex = dgvCol_Details.PCS Or e.ColumnIndex = dgvCol_Details.METERS Or e.ColumnIndex = dgvCol_Details.RATE Then
                    Amount_Calculation(e.RowIndex, e.ColumnIndex)
                ElseIf e.ColumnIndex = dgvCol_Details.Folding_Meters Then
                    .Rows(e.RowIndex).Cells(dgvCol_Details.Net_Meters).Value = Format(Val(.Rows(e.RowIndex).Cells(dgvCol_Details.METERS).Value) - Val(.Rows(e.RowIndex).Cells(dgvCol_Details.Folding_Meters).Value), "###########0.00")
                End If
        ...
        With dgv_Details
            ...
            If .Visible Then
                If e.KeyValue = Keys.Delete Then
                    ...
                    '    End If
                    'End If
                    If Val(.Rows(.CurrentCell.RowIndex).Cells(dgvCol_Details.Invoice_Meters).Value) <> 0 Then
                        e.Handled = True
                    End If
                End If
            End If
    ...
    Private Sub dgtxt_Details_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles dgtxt_Details.KeyPress
        ...
        With dgv_Details
            If .Visible Then
                ...
                If .CurrentCell.ColumnIndex = dgvCol_Details.FOLDING Or .CurrentCell.ColumnIndex = dgvCol_Details.PCS Or .CurrentCell.ColumnIndex = dgvCol_Details.METERS Or .CurrentCell.ColumnIndex = dgvCol_Details.RATE Or .CurrentCell.ColumnIndex = dgvCol_Details.AMOUNT Then
    ...
    Private Sub dgv_Details_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles dgv_Details.RowsAdded
        ...
        With dgv_Details
            n = .RowCount
            .Rows(n - 1).Cells(dgvCol_Details.SNO).Value = Val(n)
        End With
    End Sub
    ...
    Private Sub cbo_Grid_ClothName_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_Grid_ClothName.TextChanged
        Try
            If cbo_Grid_ClothName.Visible Then
                ...
                With dgv_Details
                    If Val(cbo_Grid_ClothName.Tag) = Val(.CurrentCell.RowIndex) And .CurrentCell.ColumnIndex = dgvCol_Details.CLOTH_NAME Then
    ...
    Private Sub cbo_Grid_Clothtype_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_Grid_Clothtype.TextChanged
        Try
            If cbo_Grid_Clothtype.Visible Then
                If IsNothing(dgv_Details.CurrentCell) Then Exit Sub
                With dgv_Details
                    If Val(cbo_Grid_Clothtype.Tag) = Val(.CurrentCell.RowIndex) And .CurrentCell.ColumnIndex = dgvCol_Details.TYPE Then
    ...
    Private Sub btn_ReceiptSelection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_ReceiptSelection.Click
        ...
        If Trim(UCase(cbo_Type.Text)) = "RECEIPT" Then
            With dgv_Selection
                ...
                Da = New SqlClient.SqlDataAdapter("select a.*, c.Cloth_Name,D.Ledger_Name as Del_Loc,P.Process_Name,L.Lot_No from Cloth_Purchase_Receipt_Head a INNER JOIN Cloth_Head c ON a.Cloth_IdNo = c.Cloth_IdNo left outer Join Ledger_Head d on a.Deliver_At_IdNo = D.Ledger_IdNo left outer join Process_Head p on a.Delivery_Purpose_IdNo = p.Process_IdNo left outer join Lot_Head L on a.Lot_IdNo = l.Lot_IdNo where a.Ledger_IdNo = " & Str(Val(LedIdNo)) & " and a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' order by a.Cloth_Purchase_Receipt_Date, a.for_orderby, a.Cloth_Purchase_Receipt_No", con)
                Dt1 = New DataTable
                Da.Fill(Dt1)
                ...
                If Dt1.Rows.Count > 0 Then
                    ...
                    For i = 0 To Dt1.Rows.Count - 1
                        ...
                        If Not IsDBNull(Dt1.Rows(i).Item("Del_Loc")) Then
                            .Rows(n).Cells(10).Value = Dt1.Rows(i).Item("Del_Loc")
                        End If

                        If Not IsDBNull(Dt1.Rows(i).Item("Process_Name")) Then
                            .Rows(n).Cells(11).Value = Dt1.Rows(i).Item("Process_Name")
                        End If

                        If Not IsDBNull(Dt1.Rows(i).Item("Lot_No")) Then
                            .Rows(n).Cells(12).Value = Dt1.Rows(i).Item("Lot_No")
                        End If
                ...
                Da = New SqlClient.SqlDataAdapter("select a.*, c.Cloth_Name,D.Ledger_Name as Del_Loc,P.Process_Name,L.Lot_No from Cloth_Purchase_Receipt_Head a INNER JOIN Cloth_Head c ON a.Cloth_IdNo = c.Cloth_IdNo left outer Join Ledger_Head d on a.Deliver_At_IdNo = D.Ledger_IdNo left outer join Process_Head p on a.Delivery_Purpose_IdNo = p.Process_IdNo left outer join Lot_Head L on a.Lot_IdNo = l.Lot_IdNo where a.Ledger_IdNo = " & Str(Val(LedIdNo)) & " and a.Cloth_Purchase_Code = ''  order by a.Cloth_Purchase_Receipt_Date, a.for_orderby, a.Cloth_Purchase_Receipt_No", con)
                Dt1 = New DataTable
                Da.Fill(Dt1)
            ...
            '    Da = New SqlClient.SqlDataAdapter("select a.*, c.Cloth_Name from Cloth_Purchase_Receipt_Head a INNER JOIN Cloth_Head c ON a.Cloth_IdNo = c.Cloth_IdNo  where a.Ledger_IdNo = " & Str(Val(LedIdNo)) & " and a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and a.Weaver_Piece_Checking_Code <> '' order by a.Cloth_Purchase_Receipt_Date, a.for_orderby, a.Cloth_Purchase_Receipt_No", con)
            'Dt1 = New DataTable
            'Da.Fill(Dt1)
            ...
            '    Da = New SqlClient.SqlDataAdapter("select a.*, c.Cloth_Name from Cloth_Purchase_Receipt_Head a INNER JOIN Cloth_Head c ON a.Cloth_IdNo = c.Cloth_IdNo where a.Ledger_IdNo = " & Str(Val(LedIdNo)) & " and a.Cloth_Purchase_Code = '' and a.Weaver_Piece_Checking_Code <>'' order by a.Cloth_Purchase_Receipt_Date, a.for_orderby, a.Cloth_Purchase_Receipt_No", con)
            'Dt1 = New DataTable
            'Da.Fill(Dt1)
            ...
            If Val(dgv_Selection.Rows(i).Cells(5).Value) = 1 Then
                ...
                If Dt2.Rows.Count > 0 Then
                    ...
                    For j = 0 To Dt2.Rows.Count - 1
                        ...
                        dgv_Details.Rows(n).Cells(dgvCol_Details.SNO).Value = Val(SNo)
                        dgv_Details.Rows(n).Cells(dgvCol_Details.CLOTH_NAME).Value = Dt2.Rows(j).Item("Cloth_Name").ToString
                        dgv_Details.Rows(n).Cells(dgvCol_Details.TYPE).Value = Dt2.Rows(j).Item("ClothType_Name").ToString
                        dgv_Details.Rows(n).Cells(dgvCol_Details.FOLDING).Value = Format(Val(Dt2.Rows(j).Item("Fold_Perc").ToString), "########0.00")
                        dgv_Details.Rows(n).Cells(dgvCol_Details.PCS).Value = Val(Dt2.Rows(j).Item("Pcs").ToString)
                        dgv_Details.Rows(n).Cells(dgvCol_Details.METERS).Value = Format(Val(Dt2.Rows(j).Item("Meters").ToString), "########0.00")
                        dgv_Details.Rows(n).Cells(dgvCol_Details.RATE).Value = Format(Val(Dt2.Rows(j).Item("Rate").ToString), "########0.00")
                        dgv_Details.Rows(n).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(Dt2.Rows(j).Item("Amount").ToString), "########0.00")
                ...
                Else
                    ...
                    SNo = SNo + 1
                    dgv_Details.Rows(n).Cells(dgvCol_Details.SNO).Value = Val(SNo)
                    dgv_Details.Rows(n).Cells(dgvCol_Details.CLOTH_NAME).Value = dgv_Selection.Rows(i).Cells(2).Value
                    dgv_Details.Rows(n).Cells(dgvCol_Details.TYPE).Value = Common_Procedures.ClothType.Type1
                    dgv_Details.Rows(n).Cells(dgvCol_Details.FOLDING).Value = dgv_Selection.Rows(i).Cells(9).Value
                    dgv_Details.Rows(n).Cells(dgvCol_Details.PCS).Value = dgv_Selection.Rows(i).Cells(3).Value
                    dgv_Details.Rows(n).Cells(dgvCol_Details.METERS).Value = Format(Val(dgv_Selection.Rows(i).Cells(4).Value), "#########0.00")
                    dgv_Details.Rows(n).Cells(dgvCol_Details.RATE).Value = ""
                    dgv_Details.Rows(n).Cells(dgvCol_Details.AMOUNT).Value = ""
    ...
    Private Sub Cloth_Invoice_Selection()
        ...
        For i = 0 To dgv_Order_Selection.RowCount - 1
            ...
            If Val(dgv_Order_Selection.Rows(i).Cells(9).Value) = 1 Then
                ...
                SNo = SNo + 1
                dgv_Details.Rows(n).Cells(dgvCol_Details.SNO).Value = Val(SNo)
                dgv_Details.Rows(n).Cells(dgvCol_Details.CLOTH_NAME).Value = dgv_Order_Selection.Rows(i).Cells(3).Value
                dgv_Details.Rows(n).Cells(dgvCol_Details.TYPE).Value = dgv_Order_Selection.Rows(i).Cells(4).Value
                dgv_Details.Rows(n).Cells(dgvCol_Details.FOLDING).Value = dgv_Order_Selection.Rows(i).Cells(5).Value

                dgv_Details.Rows(n).Cells(dgvCol_Details.ORDER_NO).Value = dgv_Order_Selection.Rows(i).Cells(1).Value
                dgv_Details.Rows(n).Cells(dgvCol_Details.ClothPurchase_Order_Code).Value = dgv_Order_Selection.Rows(i).Cells(14).Value
                dgv_Details.Rows(n).Cells(dgvCol_Details.ClothPurchase_Order_Slno).Value = dgv_Order_Selection.Rows(i).Cells(15).Value
                ...
                If Val(dgv_Order_Selection.Rows(i).Cells(16).Value) <> 0 Then
                    dgv_Details.Rows(n).Cells(dgvCol_Details.PCS).Value = dgv_Order_Selection.Rows(i).Cells(16).Value
                Else
                    dgv_Details.Rows(n).Cells(dgvCol_Details.PCS).Value = dgv_Order_Selection.Rows(i).Cells(6).Value
                End If

                If Val(dgv_Order_Selection.Rows(i).Cells(17).Value) <> 0 Then
                    dgv_Details.Rows(n).Cells(dgvCol_Details.METERS).Value = dgv_Order_Selection.Rows(i).Cells(17).Value
                Else
                    dgv_Details.Rows(n).Cells(dgvCol_Details.METERS).Value = dgv_Order_Selection.Rows(i).Cells(7).Value
                End If

                If Val(dgv_Order_Selection.Rows(i).Cells(18).Value) <> 0 Then
                    dgv_Details.Rows(n).Cells(dgvCol_Details.RATE).Value = dgv_Order_Selection.Rows(i).Cells(18).Value
                Else
                    dgv_Details.Rows(n).Cells(dgvCol_Details.RATE).Value = dgv_Order_Selection.Rows(i).Cells(8).Value
                End If
                dgv_Details.Rows(n).Cells(dgvCol_Details.AMOUNT).Value = Format(Val(dgv_Details.Rows(n).Cells(dgvCol_Details.METERS).Value) * Val(dgv_Details.Rows(n).Cells(dgvCol_Details.RATE).Value), "############0.00")
    ...
    Private Sub Print_Selection()
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select * from Cloth_Purchase_Head Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            dt1 = New DataTable
            da1.Fill(dt1)
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.*, d.Ledger_Name as Transport ,e.Ledger_Name as Agent_Name,CSH.State_Name as Company_State_Name  ,CSH.State_Code as Company_State_Code ,LSH.State_Name as Ledger_State_Name ,LSH.State_Code as Ledger_State_Code from Cloth_Purchase_Head a INNER JOIN Company_Head b ON a.Company_IdNo = b.Company_IdNo LEFT OUTER JOIN State_HEad CSH on b.Company_State_IdNo = CSH.State_IdNo INNER JOIN Ledger_Head c ON a.Ledger_IdNo = c.Ledger_IdNo LEFT OUTER JOIN State_HEad LSH on c.Ledger_State_IdNo = LSH.State_IdNo Left outer JOIN Ledger_Head d ON a.Transport_IdNo = d.Ledger_IdNo Left outer JOIN Ledger_Head e ON a.Agent_IdNo = e.Ledger_IdNo where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'", con)
            prn_HdDt = New DataTable
            da1.Fill(prn_HdDt)

            If prn_HdDt.Rows.Count > 0 Then

                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Cloth_name, d.ClothType_name,L.LoomType_name from Cloth_Purchase_Details a INNER JOIN Cloth_Head b ON a.Cloth_idno = b.Cloth_idno LEFT OUTER JOIN ClothType_Head d ON a.ClothType_idno = d.ClothType_idno LEFT OUTER JOIN ClothPurchase_Order_Details cd ON Cd.ClothPurchase_Order_Code = a.ClothPurchase_Order_Code LEFT OUTER JOIN loomType_Head L ON L.LoomType_idno = cd.LoomType_idno where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Cloth_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' Order by a.Sl_No", con)
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        If Common_Procedures.settings.CustomerCode = "1370" Or Common_Procedures.settings.CustomerCode = "1061" Or Common_Procedures.settings.CustomerCode = "1558" Then
    ...
    Private Sub Printing_Format1_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1136" Then '---- ps TEXTILES
            e.Graphics.DrawImage(DirectCast(Global.Textile.My.Resources.Resources.PS_Textile_Logo_perumaal, Drawing.Image), LMargin + 10, CurY, 115, 115)
        End If
        CurY = CurY + TxtHgt - 10
        ...
        If Common_Procedures.settings.CustomerCode = "1370" Then
            Common_Procedures.Print_To_PrintDocument(e, "CLOTH RECEIPT", LMargin, CurY, 2, PrintWidth, p1Font)
        Else
    ...
    Private Sub txt_Vechile_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_Vechile.KeyDown
        ...
        If (e.KeyValue = 40) Then
            If Trim(UCase(cbo_Type.Text)) = "ORDER" Then
                ...
                '  End If
            Else
                If cbo_Delivery_At.Visible = True And cbo_Delivery_At.Enabled = True Then
                    cbo_Delivery_At.Focus()
    ...
    Private Sub txt_Note_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_Note.KeyDown
        If e.KeyValue = 38 Then
            ' txt_Insurance.Focus()
            If txt_Freight_For.Visible = True Then
                txt_Freight_For.Focus()
            ElseIf txt_Transport_Freight.Enabled And txt_Transport_Freight.Visible Then
            ...
            With dgv_Details
                ...
                If dgv_Details.Rows.Count > 0 Then
                    ...
                    For RowIndx = 0 To dgv_Details.Rows.Count - 1
                        ...
                        .Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_PERC).Value = ""  'Trade Dis%
                        .Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_Value).Value = ""   'Trade Dis Amt
                        .Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_PERC).Value = ""  'Cash Dis%
                        .Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_Value).Value = ""   'Cash Dis Amt
                        ...
                        If Trim(.Rows(RowIndx).Cells(dgvCol_Details.CLOTH_NAME).Value) <> "" Or Val(.Rows(RowIndx).Cells(dgvCol_Details.PCS).Value) = 0 Or Val(.Rows(RowIndx).Cells(dgvCol_Details.METERS).Value) = 0 Or Val(.Rows(RowIndx).Cells(dgvCol_Details.AMOUNT).Value) = 0 Then

                            HSN_Code = ""
                            GST_Per = 0
                            Get_GST_Percentage_From_ClothGroup(Trim(.Rows(RowIndx).Cells(dgvCol_Details.CLOTH_NAME).Value), HSN_Code, GST_Per)

                            '--Trade discount
                            .Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_PERC).Value = Format(Val(txt_Trade_Disc.Text), "########0.00")
                            .Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_Value).Value = Format(Val(.Rows(RowIndx).Cells(dgvCol_Details.AMOUNT).Value) * (Val(.Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_PERC).Value) / 100), "########0.00")

                            '--Cash discount
                            .Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_PERC).Value = Format(Val(txt_Cash_Disc.Text), "########0.00")
                            .Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_Value).Value = Format(Val(.Rows(RowIndx).Cells(dgvCol_Details.AMOUNT).Value) * (Val(.Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_PERC).Value) / 100), "########0.00")

                            '-- Taxable value = amount - (trade disc + cash disc)
                            Taxable_Amount = Val(.Rows(RowIndx).Cells(dgvCol_Details.AMOUNT).Value) - (Val(.Rows(RowIndx).Cells(dgvCol_Details.Trade_Disc_Value).Value) + Val(.Rows(RowIndx).Cells(dgvCol_Details.Cash_Disc_Value).Value))
        ...
        Try
            ...
            If cbo_TaxType.Text = "GST" Then
                ...
                With dgv_Details
                    ...
                    If .Rows.Count > 0 Then
                        For i = 0 To .Rows.Count - 1
                            If Trim(.Rows(i).Cells(dgvCol_Details.CLOTH_NAME).Value) <> "" And Val(.Rows(i).Cells(dgvCol_Details.Gst_PERC).Value) <> 0 And Trim(.Rows(i).Cells(dgvCol_Details.HSN_Code).Value) <> "" Then
            ...
            If Val(prn_HdDt.Rows(0).Item("Tcs_Amount").ToString) <> 0 Then
                ...
                If Val(prn_HdDt.Rows(0).Item("RoundOff_Invoice_Value_Before_TCS").ToString) <> 0 Then
                    ...
                    If Val(rndoff) <> 0 Then
                        ...
                        If Val(rndoff) >= 0 Then
                            Common_Procedures.Print_To_PrintDocument(e, "(+)", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 30, CurY, 0, 0, pFont)
                        Else
                            Common_Procedures.Print_To_PrintDocument(e, "(-)", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 30, CurY, 0, 0, pFont)
                        End If
                        Common_Procedures.Print_To_PrintDocument(e, " " & Format(Math.Abs(Val(rndoff)), "########0.00"), PageWidth - 10, CurY, 1, 0, pFont)
                ...
                CurY = CurY + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, "TOTAL INVOICE VALUE", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + 20, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "" & Trim(Common_Procedures.Currency_Format(Val(prn_HdDt.Rows(0).Item("Invoice_Value_Before_TCS").ToString))), PageWidth - 10, CurY, 1, 0, p1Font)
                ...
                If Val(prn_HdDt.Rows(0).Item("Invoice_Value_Before_TCS").ToString) <> Val(prn_HdDt.Rows(0).Item("TCS_Taxable_Value").ToString) Then
                    CurY = CurY + TxtHgt
                    Common_Procedures.Print_To_PrintDocument(e, "TCS TAXABLE VALUE", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + 20, CurY, 0, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, "" & Trim(prn_HdDt.Rows(0).Item("TCS_Taxable_Value").ToString), PageWidth - 10, CurY, 1, 0, pFont)
                End If

                CurY = CurY + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("TCs_name_caption").ToString & "  @ " & Val(prn_HdDt.Rows(0).Item("Tcs_Percentage").ToString) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + 20, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "(+)", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) - 30, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "" & Trim(prn_HdDt.Rows(0).Item("Tcs_Amount").ToString), PageWidth - 10, CurY, 1, 0, pFont)
                CurY = CurY + TxtHgt
            End If
    ...
    Private Function get_GST_Tax_Percentage_For_Printing(ByVal EntryCode As String) As Single
        ...
        Da = New SqlClient.SqlDataAdapter("Select * from Cloth_Purchase_GST_Tax_Details Where Cloth_Purchase_Code = '" & Trim(EntryCode) & "'", con)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
    ...
    Private Sub txt_AddLess_AfterTax_TextChanged(sender As Object, e As System.EventArgs) Handles txt_AddLess_AfterTax.TextChanged
        NetAmount_Calculation()
    End Sub
    Private Sub Printing_Format_Half(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClAr(1) = Val(35) : ClAr(2) = 270 : ClAr(3) = 110 : ClAr(4) = 110 : ClAr(5) = 110
    ...
    Private Sub Printing_Format_Half_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        ElseIf Common_Procedures.settings.CustomerCode = "1061" Or Common_Procedures.settings.CustomerCode = "1558" Then
            Common_Procedures.Print_To_PrintDocument(e, "JOBWORK RECEIPT", LMargin, CurY, 2, PrintWidth, p1Font)
        Else
            Common_Procedures.Print_To_PrintDocument(e, "CLOTH PURCHASE", LMargin, CurY, 2, PrintWidth, p1Font)
        End If
    ...
    End Sub
    Private Sub txt_Freight_For_TextChanged(sender As Object, e As System.EventArgs) Handles txt_Freight_For.TextChanged
        FreightFor_Calculation()
    End Sub
    Private Sub FreightFor_Calculation()

        If Val(txt_Freight_For.Text) <> 0 Then
            txt_Transport_Freight.Text = Format(Val(txt_Bundles.Text) * Val(txt_Freight_For.Text), "########0.00")
        End If
    ...
    Private Sub Printing_Format_1186(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClAr(1) = Val(35) : ClAr(2) = 250 : ClAr(3) = 110 : ClAr(4) = 0 : ClAr(5) = 80 : ClAr(6) = 100 : ClAr(7) = 80
        ClAr(8) = PageWidth - (LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7))
    ...
    Private Sub btn_EDIT_TDS_TaxableValue_Click(sender As System.Object, e As System.EventArgs) Handles btn_EDIT_TDS_TaxableValue.Click
        ...
        Dim vTaxble_After_Disc As String
        ...
        vTaxble_After_Disc = Format(GrsAmt - Val(lbl_Cash_Disc_Perc.Text) - Val(lbl_Trade_Disc_Perc.Text), "########0.00")

        If txt_TDS_TaxableValue.Enabled Then

            If Common_Procedures.settings.CustomerCode = "1087" Then
                txt_TDS_TaxableValue.Text = Val(vTaxble_After_Disc)
            ElseIf Common_Procedures.settings.CustomerCode = "1098" Then
                txt_TDS_TaxableValue.Text = Val(lbl_BillAmount.Text)
            Else
    ...
    Private Sub cbo_Delivery_At_Enter(sender As Object, e As EventArgs) Handles cbo_Delivery_At.Enter
        cbo_Delivery_At.Tag = cbo_Delivery_At.Text
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Ledger_AlaisHead", "Ledger_DisplayName", "( ( ( Ledger_Type = '' and (AccountsGroup_IdNo = 10 or AccountsGroup_IdNo = 14) ) or Show_In_All_Entry = 1 or Ledger_Type = 'GODOWN' ) and Close_status = 0 ) ", "(Ledger_idno = 0)")
    End Sub

    Private Sub cbo_Delivery_At_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_Delivery_At.SelectedIndexChanged
        'If cbo_Delivery_At.Tag <> cbo_Delivery_At.Text Then
        '    Enable_Disable_Delivery_Purpose()
        '    cbo_Delivery_At.Tag = cbo_Delivery_At.Text
        'End If
    End Sub

    Private Sub cbo_Delivery_At_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_Delivery_At.KeyDown

        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Delivery_At, txt_Vechile, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "( ( ( Ledger_Type = '' and (AccountsGroup_IdNo = 10 or AccountsGroup_IdNo = 14) ) or Show_In_All_Entry = 1 or Ledger_Type = 'GODOWN') and Close_status = 0 )", "(Ledger_idno = 0)")

        If e.KeyValue = 40 Then

            If cbo_Delivery_Purpose.Visible And cbo_Delivery_Purpose.Enabled Then
            ...
            Else
                ...
                'If Trim(UCase(cbo_Type.Text)) = "ORDER" Or Trim(UCase(cbo_Type.Text)) = "RECEIPT" Then
                '    If dgv_Details.Rows.Count > 0 Then
                '        dgv_Details.Focus()
                '        dgv_Details.CurrentCell = dgv_Details.Rows(0).Cells(dgvCol_Details.METERS)
    ...
    Private Sub cbo_Delivery_At_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cbo_Delivery_At.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Delivery_At, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "( ( ( Ledger_Type = '' and (AccountsGroup_IdNo = 10 or AccountsGroup_IdNo = 14) ) or Show_In_All_Entry = 1 or Ledger_Type = 'GODOWN') and Close_status = 0 )", "(Ledger_idno = 0)")
    ...
    Private Sub cbo_Delivery_Purpose_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_Delivery_Purpose.KeyDown
        ...
        If Common_Procedures.settings.CustomerCode = "1516" Then
            Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Delivery_Purpose, Nothing, cbo_LotNo, "Process_Head", "Process_Name", "Cloth_Delivered=1", "(Process_Idno=0)")
        Else
            Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Delivery_Purpose, Nothing, cbo_LotNo, "Process_Head", "Process_Name", "", "(Process_Idno=0)")
        End If
    ...
    Private Sub cbo_Delivery_Purpose_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cbo_Delivery_Purpose.KeyPress
        ...
        If Common_Procedures.settings.CustomerCode = "1516" Then
            Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Delivery_Purpose, cbo_LotNo, "Process_Head", "Process_Name", "Cloth_Delivered=1", "(Process_Idno=0)")
        Else
            Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Delivery_Purpose, cbo_LotNo, "Process_Head", "Process_Name", "", "(Process_Idno=0)")
        End If
    ...
    Private Sub Enable_Disable_Delivery_Purpose()
        ...
        If Len(Trim(cbo_Delivery_At.Text)) = 0 Then
        ...
        Else
            ...
            Dim Led_Type As String = Common_Procedures.get_FieldValue(con, "Ledger_Head", "Ledger_Type", "Ledger_IdNo = " & Common_Procedures.Ledger_AlaisNameToIdNo(con, cbo_Delivery_At.Text))
    ...
    Private Sub cbo_Delivery_At_Leave(sender As Object, e As EventArgs) Handles cbo_Delivery_At.Leave
        If cbo_Delivery_At.Tag <> cbo_Delivery_At.Text Then
            Enable_Disable_Delivery_Purpose()
            cbo_Delivery_At.Tag = cbo_Delivery_At.Text
        End If
    ...
    Private Sub cbo_Lotno_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_LotNo.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_LotNo, cbo_Delivery_Purpose, Nothing, "Lot_Head", "Lot_No", "", "(Lot_Idno=0)")

        If e.KeyValue = 38 And cbo_LotNo.DroppedDown = False Then

            SendKeys.Send("+{Tab}")

        End If

        If (e.KeyValue = 40) And cbo_LotNo.DroppedDown = False Then
            If Trim(UCase(cbo_Type.Text)) = "ORDER" Or Trim(UCase(cbo_Type.Text)) = "RECEIPT" Then
                If dgv_Details.Rows.Count > 0 Then
                    dgv_Details.Focus()
                    dgv_Details.CurrentCell = dgv_Details.Rows(0).Cells(dgvCol_Details.METERS)
    ...
    Private Sub cbo_Lot_No_KeyUp(sender As Object, e As KeyEventArgs) Handles cbo_LotNo.KeyUp

        If Not cbo_Type.Text = "RECEIPT" Then
            If e.Control = False And e.KeyValue = 17 And vcbo_KeyDwnVal = e.KeyValue Then
    ...
    Private Sub pnl_Back_DoubleClick(sender As Object, e As EventArgs) Handles pnl_Back.DoubleClick
        MsgBox(Format(Val(dgv_Details.Rows(0).Cells(7).Value), "#########0.00"))
    End Sub
    ...
    Private Sub cbo_LotNo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_LotNo.SelectedIndexChanged

    End Sub

    Private Sub cbo_Type_Leave(sender As Object, e As EventArgs) Handles cbo_Type.Leave

        If Not Displaying Then
            If cbo_Type.Tag <> cbo_Type.Text And Len(Trim(cbo_Type.Tag)) <> 0 Then
        ...
        If cbo_Type.Text = "ORDER" Then
            dgv_Details.Columns(dgvCol_Details.ORDER_NO).ReadOnly = True
        Else
            dgv_Details.Columns(dgvCol_Details.ORDER_NO).ReadOnly = False
        End If

        cbo_Type.Tag = cbo_Type.Text

    End Sub

    Private Sub cbo_Type_Enter(sender As Object, e As EventArgs) Handles cbo_Type.Enter
        cbo_Type.Tag = cbo_Type.Text
    End Sub
    ...
    Private Sub btn_close_OpenRecord_Click(sender As Object, e As EventArgs) Handles btn_close_OpenRecord.Click
        pnl_Back.Enabled = True
        pnl_OpenRecord.Visible = False
    End Sub
    ...
    Private Sub FindRecord()
        ...
        Try
            ...
            If Trim(txt_BillNoOpen.Text) = "" And Trim(txt_RefNo_Open.Text) = "" Then
                MessageBox.Show("Invalid Bill/Ref No... ", "DOES NOT FIND...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
                Exit Sub

            ElseIf Trim(txt_RefNo_Open.Text) <> "" Then
                InvCode = ""

                inpno = Trim(txt_RefNo_Open.Text)
            ...
            ElseIf Trim(txt_BillNoOpen.Text) <> "" Then
                ...
                Da = New SqlClient.SqlDataAdapter("select Cloth_Purchase_No from Cloth_Purchase_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and upper(Bill_No) = '" & UCase(inpno) & "' and Cloth_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' ", con)
                Dt = New DataTable
                Da.Fill(Dt)
                ...
                Else
                    MessageBox.Show("Bill No. : " & Trim(txt_BillNoOpen.Text) & " does not exists", "DOES NOT FIND...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)

                End If

            ElseIf Trim(txt_BillNoOpen.Text) = "" Or Trim(txt_RefNo_Open.Text) = "" Then
                MessageBox.Show("Invalid Bill/Ref No... ", "DOES NOT FIND...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
                Exit Sub
    ...
    Private Sub btn_CloseOpenRecord_Click(sender As Object, e As EventArgs) Handles btn_CloseOpenRecord.Click
        pnl_Back.Enabled = True
        pnl_OpenRecord.Visible = False
    End Sub

    Private Sub txt_RefNo_Open_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_RefNo_Open.KeyDown
    ...
    Private Sub txt_RefNo_Open_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_RefNo_Open.KeyPress
        If Asc(e.KeyChar) = 13 Then
            e.Handled = True
            txt_BillNoOpen.Focus()
        End If
    ...
    Private Sub txt_BillNoOpen_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_BillNoOpen.KeyDown
