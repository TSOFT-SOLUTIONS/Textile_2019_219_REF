Public Class Cotton_Waste_Sales_GST
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                lbl_Grid_CGST_Perc.Text = Format(Val(lbl_grid_GstPerc.Text) / 2, "##########0.0")
                lbl_Grid_SGST_Perc.Text = Format(Val(lbl_grid_GstPerc.Text) / 2, "##########0.0")
                ' lbl_Grid_CGST_Perc.Text = dt1.Rows(0).Item("GST_Percentage").ToString
                lbl_Grid_HsnCode.Text = dt1.Rows(0).Item("HSN_Code").ToString

                txt_EWayBillNo.Text = dt1.Rows(0).Item("EWay_Bill_No").ToString
    ...
    Private Sub Cotton_Waste_Sales_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        AddHandler txt_EWayBillNo.KeyDown, AddressOf TextBoxControlKeyDown
        ...
        AddHandler txt_EWayBillNo.KeyPress, AddressOf TextBoxControlKeyPress
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        Try
            ...
            'txt_DateAndTimeOFSupply.Text = Format(dtp_Date.Value, "dd/MM/yyyy hh:mm tt")
        Catch ex As Exception
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If Val(vTotWegth) <> 0 Then
                Bill_Details = Bill_Details & IIf(Bill_Details <> "", ", ", "") & " netWeight : " & Val(lbl_NetWgt.Text)
            End If
            ...
            If Common_Procedures.Voucher_Updation(con, "GST.Waste.Sales", Val(lbl_Company.Tag), Trim(Pk_Condition) & Trim(NewCode), Trim(UCase(txt_InvoicePrefixNo.Text)) & "- " & Trim(lbl_BillNo.Text), Convert.ToDateTime(dtp_Date.Text), Trim(Bill_Details), vLed_IdNos, vVou_Amts, ErrMsg, tr, Common_Procedures.SoftwareTypes.OE_Software) = False Then
            ...
            '        If Common_Procedures.Voucher_Updation(con, "YrnSal.AgComm", Val(lbl_Company.Tag), Trim(Pk_Condition2) & Trim(NewCode), Trim(UCase(txt_InvoicePrefixNo.Text)) & "- " & Trim(lbl_BillNo.Text), Convert.ToDateTime(msk_date.Text), Trim(Bill_Details), vLed_IdNos, vVou_Amts, ErrMsg, tr,Common_Procedures.SoftwareTypes.OE_Software) = False Then
            '            Throw New ApplicationException(ErrMsg)
            '        End If
            '    End If
            'End If
            ...
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), dtp_Date.Text, Led_ID, Trim(UCase(txt_InvoicePrefixNo.Text)) & "- " & Trim(lbl_BillNo.Text), Agt_Idno, Val(vVou_BlAmt), "DR", Trim(Pk_Condition) & Trim(NewCode), tr, Common_Procedures.SoftwareTypes.OE_Software)
    ...
    Private Sub btn_Filter_Show_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Filter_Show.Click
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select a.*,b.*, c.Ledger_Name as PartyName, d.Ledger_Name as Agent_Name from Cotton_Waste_Sales_Head a  INNER JOIN Ledger_Head c ON a.Ledger_IdNo = c.Ledger_IdNo  INNER JOIN Cotton_Waste_Sales_Details b ON a.Cotton_Waste_Sales_Code = b.Cotton_Waste_Sales_Code LEFT OUTER JOIN Ledger_Head d ON a.Agent_Idno = d.Ledger_IdNo  Where a.company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Cotton_Waste_Sales_Code like '" & Trim(Pk_Condition) & "%'  and a.Cotton_Waste_Sales_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' " & IIf(Trim(Condt) <> "", " and ", "") & Condt & " Order by a.for_orderby, a.Cotton_Waste_Sales_No", con)
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.* , e.Ledger_Name as Agent_Name ,SH.* ,Lsh.State_Name as Ledger_State_Name ,Lsh.State_Code as Ledger_State_Code, f.Ledger_MainName as DelName , f.Ledger_Address1 as DelAdd1 ,f.Ledger_Address2 as DelAdd2, f.Ledger_Address3 as DelAdd3 ,f.Ledger_Address4 as DelAdd4, f.Pan_No DelPanNo, f.Ledger_GSTinNo as DelGSTinNo, DSH.State_Name as DelState_Name, DSH.State_Code as Delivery_State_Code from Cotton_Waste_Sales_Head a " & _
                                               ...
                                               "  LEFT OUTER JOIN State_HEad DSH   on f.Ledger_State_IdNo = DSH.State_IdNo " & _
    ...
    Private Sub cbo_Agent_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Agent.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Agent, txt_EWayBillNo, cbo_SalesAc, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'AGENT')", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub Cbo_TaxType_KeyDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_TaxType.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_TaxType, txt_VehicleNo, txt_DateAndTimeOFSupply, "", "", "", "")
    End Sub

    Private Sub Cbo_TaxType_KeyPress(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_TaxType.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_TaxType, txt_DateAndTimeOFSupply, "", "", "", "")
    End Sub
...
8:
            ...
            If prn_HdDt.Rows.Count > 0 Then
                ...
                If prn_DetDt.Rows.Count > 0 Then
                    ...
                    Do While prn_DetIndx <= prn_DetDt.Rows.Count - 1
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Bale_No").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) - 5, CurY, 1, 0, pFont)
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Rate").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 5, CurY, 1, 0, pFont)
    ...
    Private Sub Printing_GST_Format9_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        Try
            ...
            If prn_HdDt.Rows(0).Item("Invoice_PrefixNo").ToString <> "" Then
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Invoice_PrefixNo").ToString & "-" & prn_HdDt.Rows(0).Item("Cotton_Waste_Sales_No").ToString, LMargin + W1 + 30, CurY1 - 3, 0, 0, p1Font)
            Else
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Cotton_Waste_Sales_No").ToString, LMargin + W1 + 30, CurY1 - 3, 0, 0, p1Font)
            End If
            ...
            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("EWay_Bill_No").ToString, LMargin + C2 + W1 + 30, CurY1, 0, 0, pFont)
            ...
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + W1 + 10, CurY1, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Cotton_Waste_Sales_Date").ToString), "dd-MM-yyyy").ToString, LMargin + W1 + 30, CurY1, 0, 0, pFont)

            CurX = CurY + TxtHgt
            Common_Procedures.Print_To_PrintDocument(e, "Vehicle Number", LMargin + C2 + 10, CurY1, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + C2 + W1 + 10, CurY1, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Trim(prn_HdDt.Rows(0).Item("Vechile_No").ToString), LMargin + C2 + W1 + 30, CurY1, 0, 0, pFont)
    ...
    Private Sub Printing_GST_Format9_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal Cmp_Name As String, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean)
        ...
        Try
            ...
            If is_LastPage = True Then
                ...
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Taxable_Amount").ToString, LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) + ClAr(8) + ClAr(9) - 5, CurY, 1, 0, pFont)
            ...
            If Val(prn_HdDt.Rows(0).Item("CGst_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "Add : CGST  @ " & Format(Val(lbl_Grid_CGST_Perc.Text), "##########0.0") & " %", LMargin + C1 + 10, CurY, 0, 0, pFont)
            ...
            If Val(prn_HdDt.Rows(0).Item("SGST_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "Add : SGST  @ " & Format(Val(lbl_Grid_SGST_Perc.Text), "##########0.0") & " %", LMargin + C1 + 10, CurY, 0, 0, pFont)
            ...
            If Val(prn_HdDt.Rows(0).Item("IGST_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "Add : IGST  @ " & Format(Val(lbl_Grid_IGST_Perc.Text), "##########0") & " %", LMargin + C1 + 10, CurY, 0, 0, pFont)
            ...
            If Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString) <> 0 Then
                ...
                Common_Procedures.Print_To_PrintDocument(e, "Add : " & "Cess" & "  @ " & Format(Val(prn_HdDt.Rows(0).Item("Cmc_Percentage").ToString), "##########0.0") & " %", LMargin + C1 + 10, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "" & Format(Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString), "##########0.00"), PageWidth - 10, CurY, 1, 0, pFont)
            ...
            ElseIf Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1176" And Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1256" Then ' KALPANA COTTON
                Common_Procedures.Print_To_PrintDocument(e, "Prepared by", LMargin + 35, CurY, 0, 0, pFont)
            End If

            If Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1176" And Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1256" Then ' KALPANA COTTON
            ...
            CurY = CurY + TxtHgt + 10
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1176" And Trim(UCase(Common_Procedures.settings.CustomerCode)) <> "1256" Then
    ...
    '    Cmd.ExecuteNonQuery()
    '    Cmd.CommandText = "Insert into " & Trim(Common_Procedures.EntryTempSubTable) & " (Meters1, Currency1) select (CGST_Percentage+SGST_Percentage), (CGST_Amount+SGST_Amount) from Yarn_Sales_GST_Tax_Details  Where Cotton_Waste_Sales_Code = '" & Trim(EntryCode) & "' and (CGST_Amount+SGST_Amount) <> 0"
    '    Cmd.ExecuteNonQuery()
    ...
    Private Sub cbo_DeliveryTo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_DeliveryTo.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_DeliveryTo, txt_DateAndTimeOFSupply, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "", "(Ledger_IdNo = 0)")
        If e.KeyCode = 40 And cbo_DeliveryTo.DroppedDown = False Or (e.Control = True And e.KeyValue = 40) Then
            If dgv_Details.Rows.Count > 0 Then
                dgv_Details.Focus()
                dgv_Details.CurrentCell = dgv_Details.Rows(0).Cells(1)
            Else
