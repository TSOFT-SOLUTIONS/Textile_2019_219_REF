Imports System.IO
Public Class OE_Cotton_Waste_Sales_GST
    Implements Interface_MDIActions


    Public vEntry_Sales_No As Integer = 0
    Private con As New SqlClient.SqlConnection(Common_Procedures.Connection_String)
    Private FrmLdSTS As Boolean = False
    Private New_Entry As Boolean = False
    Private Insert_Entry As Boolean = False
    Private Filter_Status As Boolean = False
    Private Pk_Condition As String = "GSCWS-"
    Private Pk_Condition2 As String = "GSWSA-"
    ...
    Public vEntry_WasteSales_No As Integer = 0
    ...
    Private Sub clear()
        ...
        lbl_SGstAmount.Text = ""
        lbl_IGstAmount.Text = ""
        lbl_Grid_IGST_Perc.Text = ""
        lbl_Grid_CGST_Perc.Text = ""
        lbl_Grid_SGST_Perc.Text = ""

        If Val(vEntry_WasteSales_No) = 1 Then
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                cbo_DeliveryTo.Text = Common_Procedures.Ledger_IdNoToName(con, Val(dt1.Rows(0).Item("DeliveryTo_IdNo").ToString)) '  Common_Procedures.Despatch_IdNoToName(con, Val(dt1.Rows(0).Item("DeliveryTo_IdNo").ToString))
                ...
                txt_CMC_Text.Text = dt1.Rows(0).Item("CMC_Text").ToString
    ...
    Private Sub Cotton_Waste_Sales_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Val(vEntry_Sales_No) = 1 Then
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1391" Then '--saktidaran
                Label1.Text = "VISCOSE WASTE SALES"
                Label1.Tag = Val(vEntry_Sales_No)
        ...
        Else
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1391" Then
                Label1.Text = " COTTON WASTE SALES"
                Label1.Tag = Val(vEntry_Sales_No)
        ...
        cbo_Bale_Bora.Items.Clear()
        cbo_Bale_Bora.Items.Add("")
        cbo_Bale_Bora.Items.Add("BALE")
        cbo_Bale_Bora.Items.Add("BORA")
        ...
        AddHandler cbo_Bale_Bora.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler cbo_Bale_Bora.LostFocus, AddressOf ControlLostFocus
        ...
        'AddHandler cbo_VehicleNo.KeyDown, AddressOf TextBoxControlKeyDown
        ...
        AddHandler txt_Bagnos.KeyDown, AddressOf TextBoxControlKeyDown
        ...
        'AddHandler cbo_VehicleNo.KeyPress, AddressOf TextBoxControlKeyPress
        ...
        AddHandler txt_Bagnos.KeyPress, AddressOf TextBoxControlKeyPress
        ...
        FrmLdSTS = True
        new_record()
        lbl_grid_GstPerc.Enabled = True
        lbl_Grid_HsnCode.Enabled = True
    End Sub
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.OEENTRY_WASTE_SALES_ENTRY, New_Entry, Me, con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", NewCode, "Cotton_Waste_Sales_Date", "(Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "')") = False Then Exit Sub
        ...
        Try
            ...
            If Common_Procedures.VoucherBill_Deletion(con, Trim(Pk_Condition2) & Trim(NewCode), trans) = False Then
                Throw New ApplicationException("Error on Voucher Bill Deletion")
            End If
            ...
            cmd.CommandText = "delete from Cotton_Waste_Sales_Details where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cotton_Waste_Sales_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
            cmd.CommandText = "delete from Cotton_Waste_Sales_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Cotton_Waste_Sales_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        Try
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1262" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1353" Then      '----- sathuragiri
                'lbl_BillNo.Text = Common_Procedures.get_CotConv_MaxCode(con, Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                lbl_BillNo.Text = Common_Procedures.get_CotConv_MaxCode(con, Val(lbl_Company.Tag), Common_Procedures.FnYearCode)
            ElseIf Trim(Common_Procedures.settings.CustomerCode) = "1087" Then 'Kalaimagal Palladam
                lbl_BillNo.Text = Common_Procedures.get_CottonWaste_Sales_MaxCode(con, Val(lbl_Company.Tag), Common_Procedures.FnYearCode)
            Else
                'lbl_BillNo.Text = Common_Procedures.get_MaxCode(con, "Cotton_Invoice_Head", "Cotton_Invoice_Code", "For_OrderBy", "Entry_VAT_GST_Type ='GST'", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                lbl_BillNo.Text = Common_Procedures.get_MaxCode(con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", "For_OrderBy", "( Cotton_Waste_Sales_Code LIKE '" & Trim(Pk_Condition) & "%')", Val(lbl_Company.Tag), Common_Procedures.FnYearCode)
            End If
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Try
            ...
            nCotmovno = ""
            If Trim(Common_Procedures.settings.CustomerCode) = "1353" Then '--------balaji spinning mill
                ...
                If dt1.Rows.Count > 0 Then
                    If IsDBNull(Dt.Rows(0)(0).ToString) = False Then
                        nCotmovno = Trim(dt1.Rows(0)(0).ToString)
                    End If
                End If
                dt1.Clear()
            End If
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.InsertingEntry, Common_Procedures.UR.OEENTRY_WASTE_SALES_ENTRY, New_Entry, Me) = False Then Exit Sub
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        NewCode = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.OEENTRY_WASTE_SALES_ENTRY, New_Entry, Me, con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", NewCode, "Cotton_Waste_Sales_Date", "(Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cotton_Waste_Sales_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Cotton_Waste_Sales_No desc", dtp_Date.Value.Date) = False Then Exit Sub
        ...
        Try
            ...
            Else
                ...
                If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1262" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1353" Then      '----- sathuragiri
                    ...
                    lbl_BillNo.Text = Common_Procedures.get_CotConv_MaxCode(con, Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                ElseIf Trim(Common_Procedures.settings.CustomerCode) = "1087" Then 'Kalaimagal Palladam
                    lbl_BillNo.Text = Common_Procedures.get_CottonWaste_Sales_MaxCode(con, Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                Else
                    'lbl_BillNo.Text = Common_Procedures.get_MaxCode(con, "Cotton_Invoice_Head", "Cotton_Invoice_Code", "For_OrderBy", "Entry_VAT_GST_Type ='GST'", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                    lbl_BillNo.Text = Common_Procedures.get_MaxCode(con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", "For_OrderBy", "(Cotton_Waste_Sales_Code LIKE '" & Trim(Pk_Condition) & "%')", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
                End If
            ...
            cmd.ExecuteNonQuery()
            cmd.CommandText = "Delete from Stock_Waste_Processing_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Reference_Code = '" & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        cmd.CommandText = "Insert into Cotton_Waste_Sales_Details ( Cotton_Waste_Sales_Code ,               Company_IdNo       ,   Cotton_Waste_Sales_No    ,             for_OrderBy       ,   Cotton_Waste_Sales_Date    ,             Sl_No     ,   Ledger_idNo           ,         Colour_IdNo     ,     Variety_IdNo         ,                Lot_No                    ,    bale                                  ,         Bale_No                       ,                 Weight                      ,         Cotton_Waste_Code             , Cotton_Waste_Details_Slno  ) " &
                        ...
                        If InStr(1, Trim(UCase(.Rows(i).Cells(1).Value)), "WASTE") > 0 Then
                            cmd.CommandText = "Insert into Stock_Waste_Processing_Details (                                       SoftwareType_IdNo          ,   Reference_Code       ,             Company_IdNo         ,           Reference_No        ,           For_OrderBy      ,   Reference_Date,     Party_Bill_No    ,       Entry_ID       ,         Sl_No        ,     Ledger_idNo         ,     Colour_IdNo         , Variety_IdNo ,                     Lot_no              ,                    Bale_No             ,                           Bale                ,                           Weight               ) " &
                                               "            Values                        (" & Str(Val(Common_Procedures.SoftwareTypes.OE_Software)) & " , '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_BillNo.Text) & "', " & Str(Val(vORDBYNO)) & ",    @SalesDate   , '" & Trim(PBlNo) & "', '" & Trim(EntID) & "', " & Str(Val(Sno)) & ", " & Str(Val(Led_ID)) & ", " & Str(Val(clr_ID)) & ",      1       ,  '" & Trim(.Rows(i).Cells(2).Value) & "', '" & Trim(.Rows(i).Cells(4).Value) & "', " & Str(-1 * Val(.Rows(i).Cells(3).Value)) & ", " & Str(-1 * Val(.Rows(i).Cells(5).Value)) & " ) "
                            cmd.ExecuteNonQuery()

                        Else

                            cmd.CommandText = "Insert into Stock_Cotton_Processing_Details (                                        SoftwareType_IdNo          , Reference_Code         ,             Company_IdNo         ,           Reference_No        ,           For_OrderBy      , Reference_Date ,  Entry_ID               ,   Party_Bill_No    ,    Sl_No       ,            Ledger_idNo  ,        Variety_IdNo      ,                           Bale                 ,                           Weight               ) " &
                                                "           Values                          (" & Str(Val(Common_Procedures.SoftwareTypes.OE_Software)) & " , '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_BillNo.Text) & "', " & Str(Val(vORDBYNO)) & ",    @SalesDate   , '" & Trim(EntID) & "' ,'" & Trim(PBlNo) & "'," & Val(Sno) & ", " & Str(Val(Led_ID)) & ", " & Str(Val(Vrty_ID)) & ", " & Str(-1 * Val(.Rows(i).Cells(3).Value)) & " , " & Str(-1 * Val(.Rows(i).Cells(5).Value)) & " ) "
                            cmd.ExecuteNonQuery()
            ...
            If Val(Agt_Idno) <> 0 Then
                ...
                cmd.CommandText = "Insert into AgentCommission_Processing_Details (  Reference_Code , Company_IdNo  ,            Reference_No       ,               For_OrderBy  , Reference_Date,      Ledger_IdNo    ,           Agent_IdNo      ,         Entry_ID     ,      Party_BillNo    ,       Particulars      ,             Amount              ,                       Commission_Amount       ) " &
                                                " Values ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_BillNo.Text) & "', " & Str(Val(vORDBYNO)) & ",   @Salesdate  , " & Str(Led_ID) & ", " & Str(Val(Agt_Idno)) & ", '" & Trim(EntID) & "', '" & Trim(PBlNo) & "', '" & Trim(Partcls) & "',  " & Str(Val(lbl_NetAmount.Text)) & ",   " & Str(Val(ComAmt)) & " ) "
                cmd.ExecuteNonQuery()
            ...
            If Common_Procedures.Voucher_Updation(con, "GST.Was.Sale", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_BillNo.Text), dtp_Date.Value.Date, "Bill No : " & Trim(lbl_BillNo.Text), vLed_IdNos, vVou_Amts, ErrMsg, tr, Common_Procedures.SoftwareTypes.OE_Software) = False Then
            ...
            NewCode2 = Trim(Pk_Condition2) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
            If Common_Procedures.VoucherBill_Deletion(con, Trim(NewCode2), tr) = False Then
                Throw New ApplicationException("Error on Voucher Bill Deletion")
            End If

            'Bill Posting
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), dtp_Date.Text, Led_ID, Trim(lbl_BillNo.Text), Agt_Idno, Val(CSng(lbl_NetAmount.Text)), "DR", Trim(NewCode), tr, Common_Procedures.SoftwareTypes.OE_Software, SaveAll_STS)
            ...
            If Val(Common_Procedures.settings.OnSave_MoveTo_NewEntry_Status) = 1 Then
                If New_Entry = True Then
                    new_record()
                Else
                    move_record(Trim(lbl_BillNo.Text))
                End If
            Else
                move_record(Trim(lbl_BillNo.Text))
            End If
    ...
    Private Sub cbo_PurchaseAc_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_SalesAc.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_SalesAc, cbo_Agent, txt_CommKg, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 28)", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_PurchaseAc_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_SalesAc.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_SalesAc, txt_CommKg, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 28)", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub NetAmount_Calculation()
        ...
        lbl_Assessable.Text = Format(Val(lbl_Amount.Text) - Val(lbl_DiscAmount.Text) + Val(txt_AddLess_AfterTax.Text) + +Val(txt_Freight.Text), "#########0.00")

        'If Trim(Common_Procedures.settings.CustomerCode) = "1088" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Then  '-----KALAIMAGAL TEXTILES (PALLADAM)
        '    lbl_CmcAmount.Text = Format(Val(lbl_Amount.Text) * Val(txt_CmcPerc.Text) / 100, "########0.00")
        'Else
        lbl_CmcAmount.Text = Format(Val(lbl_Assessable.Text) * Val(txt_CmcPerc.Text) / 100, "########0.00")
        'End If

        lbl_CGstAmount.Text = 0
        lbl_SGstAmount.Text = 0
        lbl_IGstAmount.Text = 0
        lbl_Grid_IGST_Perc.Text = 0
        lbl_Grid_CGST_Perc.Text = 0
        lbl_Grid_SGST_Perc.Text = 0
        ...
        If Trim(cbo_TaxType.Text) = "GST" Then
            ...
            If Trim(Company_State_Code) = Trim(Ledger_State_Code) Then
                '-CGST 
                lbl_Grid_CGST_Perc.Text = Format(Val(Val(lbl_grid_GstPerc.Text) / 2), "##0.00")
                lbl_CGstAmount.Text = Format(Val(lbl_Assessable.Text) * (Val(lbl_grid_GstPerc.Text) / 2) / 100, "#########0.00")
                '-SGST 
                lbl_Grid_SGST_Perc.Text = Format(Val(Val(lbl_grid_GstPerc.Text) / 2), "##0.00")
            ...
            ElseIf Trim(Company_State_Code) <> Trim(Ledger_State_Code) Then
                '-IGST 
                lbl_Grid_IGST_Perc.Text = Format(Val(lbl_grid_GstPerc.Text), "##0.00")
        ...
        Else
            ...
            lbl_grid_GstPerc.Enabled = False
            lbl_Grid_HsnCode.Enabled = False
            lbl_CGstAmount.Text = 0
            lbl_SGstAmount.Text = 0
            lbl_IGstAmount.Text = 0
            lbl_Grid_IGST_Perc.Text = 0
        ...
        GST_Amt = Val(lbl_CGstAmount.Text) + Val(lbl_SGstAmount.Text) + Val(lbl_IGstAmount.Text) + Val(lbl_CmcAmount.Text)
        ...
        NtAmt = Val(lbl_Assessable.Text) + Val(lbl_CGstAmount.Text) + Val(lbl_SGstAmount.Text) + Val(lbl_IGstAmount.Text) + Val(lbl_CmcAmount.Text) + Val(lbl_TcsAmount.Text)
    ...
    Public Sub print_record() Implements Interface_MDIActions.print_record
        ...
        NewCode = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.PrintEntry, Common_Procedures.UR.OEENTRY_WASTE_SALES_ENTRY, New_Entry) = False Then Exit Sub
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.* , e.Ledger_Name as Agent_Name ,SH.*, Lsh.State_Name as Ledger_State_Name , SDAH.Ledger_Name as DeliveryTo_Name, SDAH.Ledger_Address1 as DeliveryTo_Address1, SDAH.Ledger_Address2 as DeliveryTo_Address2, SDAH.Ledger_Address3 as DeliveryTo_Address3, SDAH.Ledger_Address4 as DeliveryTo_Address4, SDAH.Ledger_GSTinNo as DeliveryTo_GSTIN, Lsh.State_Code as Ledger_State_Code, Dsh.State_Name as DeliveryTo_State_Name ,Dsh.State_Code as DeliveryTo_State_Code  from Cotton_Waste_Sales_Head a " &
                    ...
                    "  LEFT JOIN Ledger_Head SDAH ON SDAH.Ledger_IdNo = (case when a.DeliveryTo_IdNo <> 0 then a.DeliveryTo_IdNo else a.Ledger_IdNo end) " &
                    "  LEFT OUTER JOIN State_Head Dsh   ON SDAH.Ledger_State_Idno     = Dsh.State_IdNo " &
                    "  LEFT OUTER JOIN State_Head SH    ON b.Company_State_IdNo       = SH.State_Idno           where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'", con)

            'da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.* , e.Ledger_Name as Agent_Name ,SH.* ,Lsh.State_Name as Ledger_State_Name ,SDAH.*,Lsh.State_Code as Ledger_State_Code, Dsh.State_Name as DeliveryTo_State_Name ,Dsh.State_Code as DeliveryTo_State_Code  from Cotton_Waste_Sales_Head a " &
            '        "  INNER JOIN Company_Head b        ON a.Company_IdNo             = b.Company_IdNo " &
            '        "  INNER JOIN Ledger_Head c         ON a.Ledger_IdNo              = c.Ledger_IdNo " &
            '        "  LEFT OUTER JOIN Ledger_Head e    ON e.Ledger_IdNo              = a.Agent_IdNo " &
            '        "  LEFT OUTER JOIN State_Head Lsh   ON c.Ledger_State_Idno        = Lsh.State_IDno " &
            '        "  LEFT JOIN Sales_DeliveryAddress_Head SDAH ON a.DeliveryTo_IdNo = SDAH.Party_IdNo " &
            '        "  LEFT OUTER JOIN State_Head Dsh   ON SDAH.State_IdNo            = Dsh.State_IdNo " &
            '        "  LEFT OUTER JOIN State_Head SH    ON b.Company_State_IdNo       = SH.State_Idno           where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'", con)
    ...
    Private Sub Printing_Format1(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClArr(6) = 90       'WEIGHT
        ClArr(7) = 90      'RATE
        ClArr(8) = PageWidth - (LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5)) 'AMOUNT
            ...
            If prn_HdDt.Rows.Count > 0 Then
                ...
                If prn_DetDt.Rows.Count > 0 Then
                    ...
                    Do While prn_DetIndx <= prn_DetDt.Rows.Count - 1
                        ...
                        Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("HSN_Code").ToString, LMargin + ClArr(1) + ClArr(2) + 10, CurY, 0, 0, pFont)
                        Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("GST_Percentage").ToString), "########0.00"), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) - 10, CurY, 1, 0, pFont)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("bale").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) - 10, CurY, 1, 0, pFont)
                        Common_Procedures.Print_To_PrintDocument(e, prn_DetDt.Rows(prn_DetIndx).Item("Weight").ToString, LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) - 10, CurY, 1, 0, pFont)
                        Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Rate").ToString), "#######0.00"), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 10, CurY, 1, 0, pFont)
    ...
    Private Sub Printing_Format1_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, "RATE/KG ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6), CurY, 2, ClAr(7), pFont)
            Common_Procedures.Print_To_PrintDocument(e, "AMOUNT", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6), CurY, 2, ClAr(8), pFont)
    ...
    Private Sub Printing_Format1_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean)
        ...
        Try
            ...
            If Val(prn_HdDt.Rows(0).Item("Discount_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "Discount", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) - 10, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "( - )", LMargin + C1 + W1 + 10, CurY, 0, 0, pFont)
            ...
            '        Common_Procedures.Print_To_PrintDocument(e, "CMC " & " @ " & Trim(Val(prn_HdDt.Rows(0).Item("Cmc_Percentage").ToString)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) - 10, CurY, 0, 0, pFont)
            '        Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString), "########0.00"), PageWidth - 10, CurY, 1, 0, pFont)
            '    End If
            'End If

            CurY = CurY + TxtHgt
            If Val(prn_HdDt.Rows(0).Item("Freight_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "Freight", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) - 10, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "( + )", LMargin + C1 + W1 + 10, CurY, 0, 0, pFont)
            ...
            Common_Procedures.Print_To_PrintDocument(e, "Bag No's : " & Trim(prn_HdDt.Rows(0).Item("Bag_Nos").ToString), LMargin + 10, CurY, 0, 0, pFont)

            CurY = CurY + TxtHgt
            If Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "AddLess", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) - 10, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "( + )", LMargin + C1 + W1 + 10, CurY, 0, 0, pFont)
            ...
            prn_GST_Perc = Val(lbl_grid_GstPerc.Text)
            ...
            If Val(prn_CGST_Amount) <> 0 Or Val(prn_SGST_Amount) <> 0 Or Val(prn_IGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    ...
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Taxable_Amount").ToString), "########0.00"), PageWidth - 10, CurY, 1, 0, p1Font)
                End If
            End If
            ...
            If Val(prn_CGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    If prn_CGST_Amount <> 0 Then
                        Common_Procedures.Print_To_PrintDocument(e, "CGST @ " & Trim(Val(prn_GST_Perc / 2)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) - 10, CurY, 1, 0, pFont)
                    Else
            ...
            If Val(prn_SGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    If prn_SGST_Amount <> 0 Then
                        Common_Procedures.Print_To_PrintDocument(e, "SGST @ " & Trim(Val(prn_GST_Perc / 2)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) - 10, CurY, 1, 0, pFont)
                    Else
            ...
            If Val(prn_IGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    If prn_IGST_Amount <> 0 Then
                        Common_Procedures.Print_To_PrintDocument(e, "IGST @ " & Trim(Val(prn_GST_Perc)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) - 10, CurY, 1, 0, pFont)
                    Else
            ...
            If Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString) <> 0 Then
                CurY = CurY + TxtHgt
                If is_LastPage = True Then
                    Common_Procedures.Print_To_PrintDocument(e, Trim(prn_HdDt.Rows(0).Item("CMC_Text").ToString) & " @ " & Trim(Val(prn_HdDt.Rows(0).Item("Cmc_Percentage").ToString)) & " %", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) + ClAr(7) - 10, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString), "########0.00"), PageWidth - 10, CurY, 1, 0, pFont)
                End If
            End If
            '***** GST END *****
            ...
            If Val(prn_HdDt.Rows(0).Item("RoundOff_Amount").ToString) <> 0 Then
                Common_Procedures.Print_To_PrintDocument(e, "RoundOff ", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4) + ClAr(5) + ClAr(6) - 10, CurY, 0, 0, pFont)
                Common_Procedures.Print_To_PrintDocument(e, "( + )", LMargin + C1 + W1 + 10, CurY, 0, 0, pFont)
    ...
    Private Sub Printing_Format2(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        Dim ItmNm1 As String, ItmNm2 As String
        Dim ps As Printing.PaperSize
        Dim vLine_Pen, vOUTERBORDERLine_Pen As Pen
        Dim vFontName As String = ""
        Dim vMorePages_STS As Boolean
        Dim vbr As SolidBrush, br2 As SolidBrush
        ...
        ElseIf Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1172" Then         '------SRI RAM WEAVING MILL
            ...
            vLine_Pen = New Pen(Color.Green, 2)
        Else
            vOUTERBORDERLine_Pen = New Pen(Color.Black, 2)
        ...
        ClArr(0) = 0
        ClArr(1) = Val(35) ' S.NO
        ClArr(2) = 180     'DESCRIPTION OF GOODS
        ClArr(3) = 80       'HSN CODE
        ClArr(4) = 80       'GST %
        ClArr(5) = 65       'NO.OF.BAG
        ClArr(6) = 80       'TOTAL WGT
        ClArr(7) = 70       'RATE/KG
        ClArr(8) = PageWidth - (LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) + ClArr(8))  'AMOUNT
                ...
                Try
                    ...
                    If prn_DetDt.Rows.Count > 0 Then
                        ...
                        Do While prn_DetIndx <= prn_DetDt.Rows.Count - 1
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, Trim(Val(prn_DetSNo)), LMargin + 5, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, Trim(ItmNm1), LMargin + ClArr(1) + 5, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(prn_DetIndx).Item("HSN_Code").ToString, LMargin + ClArr(1) + ClArr(2) + 5, CurY, 0, 0, pFont)
                            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(prn_DetIndx).Item("GST_Percentage").ToString), "########0.00"), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) - 5, CurY, 1, 0, pFont)
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_DetDt.Rows(prn_DetIndx).Item("Weight").ToString), "########0.00"), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) - 5, CurY, 1, 0, pFont)
                            ...
                            Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_DetDt.Rows(prn_DetIndx).Item("Weight").ToString) * Val(prn_HdDt.Rows(0).Item("Rate").ToString), "#,##,##,##,##0.00"), PageWidth - 5, CurY, 1, 0, pFont)
    ...
    Private Sub Printing_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByRef NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal vLine_Pen As Pen, ByVal vFontName As String, ByVal vOUTERBORDERLine_Pen As Pen)
        ...
        e.Graphics.DrawLine(vOUTERBORDERLine_Pen, LMargin, CurY, PageWidth, CurY)
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Then
            p1Font = New Font("Americana std", 22, FontStyle.Bold)
        ...
        Try
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1088" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Then '---- Kalaimagal Textiles (OE) (palladam)
                ...
                If prn_HdDt.Rows(0).Item("Invoice_PrefixNo").ToString <> "" Then
                    Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Invoice_PrefixNo").ToString & "/" & Trim(Format(Val(Inv_No), "######000")) & Trim(InvSubNo), LMargin + W3 + 30, CurY, 0, 0, p1Font)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, Trim(Format(Val(Inv_No), "######000")) & Trim(InvSubNo), LMargin + W3 + 30, CurY, 0, 0, p1Font)
                End If
            End If
            ...
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + W3 + 10, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("Cotton_Waste_Sales_Date").ToString), "dd-MM-yyyy").ToString, LMargin + W3 + 30, CurY, 0, 0, pFont)
            ...
            p1Font = New Font(vFontName, 9, FontStyle.Bold)
            If prn_HdDt.Rows(0).Item("DeliveryTo_Name").ToString <> "" Then
                Common_Procedures.Print_To_PrintDocument(e, "M/s. " & prn_HdDt.Rows(0).Item("DeliveryTo_Name").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, p1Font)
            Else
            ...
            If prn_HdDt.Rows(0).Item("DeliveryTo_Address1").ToString <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("DeliveryTo_Address1").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("DeliveryTo_Address2").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("DeliveryTo_Address2").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("DeliveryTo_Address3").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("DeliveryTo_Address3").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("DeliveryTo_Address4").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("DeliveryTo_Address4").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            ...
            'CurY2 = CurY2 + TxtHgt
            If Trim(prn_HdDt.Rows(0).Item("DeliveryTo_GSTIN").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, "GSTIN : " & prn_HdDt.Rows(0).Item("DeliveryTo_GSTIN").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            ...
            Common_Procedures.Print_To_PrintDocument(e, "GST %", LMargin + ClAr(1) + ClAr(2) + ClAr(3), CurY, 2, ClAr(4), pFont)
            If Trim(Common_Procedures.settings.CustomerCode) = "1088" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Then '------KALAIMAGAL TEXTILES (PALLADAM)
                If Trim(cbo_Bale_Bora.Text) = "BORA" Then
                    Common_Procedures.Print_To_PrintDocument(e, "BORA", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4), CurY, 2, ClAr(5), pFont)
                Else
                    Common_Procedures.Print_To_PrintDocument(e, "BALE", LMargin + ClAr(1) + ClAr(2) + ClAr(3) + ClAr(4), CurY, 2, ClAr(5), pFont)
                End If
            Else
    ...
    Private Sub Printing_Format2_PageFooter(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClArr() As Single, ByVal Cmp_Name As String, ByVal NoofDets As Integer, ByVal is_LastPage As Boolean, ByVal vFontName As String, ByRef vIS_HasMorePages As Boolean, ByVal vOUTERBORDERLine_Pen As Pen)
        ...
        Try
            ...
            CurY = CurY + TxtHgt - 10
            Common_Procedures.Print_To_PrintDocument(e, "Bale NO's :" & prn_DetDt.Rows(0).Item("Bale_No").ToString, LMargin + 5, CurY, 0, 0, pFont)
            ...
            If is_LastPage = True Then
                If Val(prn_HdDt.Rows(0).Item("Freight_Amount").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, Trim(txt_Freight_Caption.Text), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 10, CurY, 1, 0, pFont)
            ...
            If is_LastPage = True Then
                If Val(prn_HdDt.Rows(0).Item("AddLess_Amount").ToString) <> 0 Then
                    Common_Procedures.Print_To_PrintDocument(e, Trim(txt_AddLess_Caption.Text), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 10, CurY, 1, 0, pFont)
            ...
            prn_GST_Perc = Val(Common_Procedures.get_FieldValue(con, "Variety_Head", "GST_Percentege", "Variety_Name = '" & Trim(prn_DetDt.Rows(0).Item("Variety_Name").ToString) & "'"))
            'prn_GST_Perc = Val(Common_Procedures.get_FieldValue(con, "Variety_Head", "GST_Percentege", "Variety_Name = '" & Trim(cbo_Grid_Variety.Text) & "'"))
            ...
            If Val(prn_CGST_Amount) <> 0 Or Val(prn_SGST_Amount) <> 0 Or Val(prn_IGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    ...
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Taxable_Amount").ToString), "########0.00"), PageWidth - 5, CurY, 1, 0, pFont)
                End If
            End If
            ...
            If Val(prn_CGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    ...
                    End If
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_CGST_Amount), "########0.00"), PageWidth - 5, CurY, 1, 0, pFont)
                End If
            End If
            ...
            If Val(prn_SGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    ...
                    End If
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_SGST_Amount), "########0.00"), PageWidth - 5, CurY, 1, 0, pFont)
                End If
            End If
            ...
            If Val(prn_IGST_Amount) <> 0 Then
                ...
                If is_LastPage = True Then
                    ...
                    End If
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_IGST_Amount), "########0.00"), PageWidth - 5, CurY, 1, 0, pFont)
                End If
            End If
            If Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString) <> 0 Then
                CurY = CurY + TxtHgt
                If is_LastPage = True Then
                    Common_Procedures.Print_To_PrintDocument(e, Trim(prn_HdDt.Rows(0).Item("CMC_Text").ToString) & " @ " & Trim(Val(prn_HdDt.Rows(0).Item("Cmc_Percentage").ToString)) & " %", LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 10, CurY, 1, 0, pFont)
                    Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_HdDt.Rows(0).Item("Cmc_Amount").ToString), "########0.00"), PageWidth - 5, CurY, 1, 0, pFont)
                End If
            End If
            ...
            If Val(prn_HdDt.Rows(0).Item("TCS_Amount").ToString) <> 0 Then
                CurY = CurY + TxtHgt
                If is_LastPage = True Then
                    Common_Procedures.Print_To_PrintDocument(e, "TCS @ " & Trim(Val(prn_HdDt.Rows(0).Item("TCS_Percentage").ToString)) & " %", LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 10, CurY, 1, 0, pFont)
            ...
            CurY = CurY + TxtHgt - 5
            p1Font = New Font(vFontName, 10, FontStyle.Underline)
            ...
            e.Graphics.DrawLine(vOUTERBORDERLine_Pen, LMargin, CurY, PageWidth, CurY)
            e.Graphics.DrawLine(vOUTERBORDERLine_Pen, LMargin, LnAr(1), LMargin, CurY)
            e.Graphics.DrawLine(vOUTERBORDERLine_Pen, PageWidth, LnAr(1), PageWidth, CurY)
    ...
    Private Sub cbo_Agent_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Agent.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Agent, txt_Description, cbo_SalesAc, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'AGENT')", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub dgv_Waste_Selection_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dgv_Waste_selection.KeyDown
        ...
        If IsNothing(dgv_Waste_selection.CurrentCell) Then Exit Sub
    ...
    Private Sub Cbo_TaxType_KeyDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_TaxType.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_TaxType, cbo_VehicleNo, cbo_DeliveryTo, "", "", "", "")
    End Sub
    ...
    Private Sub Cbo_DelTo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_DeliveryTo.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        'Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_DeliveryTo, cbo_Transport, Nothing, "Sales_DeliveryAddress_Head", "Party_Name", "", "(Party_IdNo = 0)")
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_DeliveryTo, cbo_Transport, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "", "(Ledger_IdNo = 0)")

        If (e.KeyValue = 40 And cbo_DeliveryTo.DroppedDown = False) Or (e.Control = True And e.KeyValue = 40) Then
            If cbo_Bale_Bora.Visible Then
                cbo_Bale_Bora.Focus()
            ElseIf dgv_Details.RowCount > 0 Then
                dgv_Details.Focus()
                dgv_Details.CurrentCell = dgv_Details.Rows(0).Cells(1)
                dgv_Details.CurrentCell.Selected = True
            Else
    ...
    Private Sub Cbo_DelTo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_DeliveryTo.KeyPress
        ' Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_DeliveryTo, Nothing, "Sales_DeliveryAddress_Head", "Party_Name", "", "(Party_IdNo = 0)")
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_DeliveryTo, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "", "(Ledger_IdNo = 0)")
    ...
    Private Sub txt_Remarks_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_Remarks.KeyDown
        If e.KeyCode = 38 Then
            e.Handled = True
            txt_Bagnos.Focus()
            'If txt_RemTime.Visible And txt_Remarks.Enabled Then
            '    txt_RemTime.Focus()
            'Else
            '    dtp_RemDate.Focus()
            'End If
    ...
    Private Sub cbo_Bale_Bora_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Bale_Bora.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Bale_Bora, Nothing, Nothing, "", "", "", "")
        With dgv_Details
            If e.KeyCode = 38 And cbo_Bale_Bora.DroppedDown = False Or (e.Control = True And e.KeyCode = 38) Then
                If cbo_DeliveryTo.Visible Then
                    cbo_DeliveryTo.Focus()
                Else
                    cbo_TaxType.Focus()
                End If
            End If
            If e.KeyCode = 40 And cbo_Bale_Bora.DroppedDown = False Or (e.Control = True And e.KeyCode = 40) Then
                'If .Visible Then
                '    .Focus()
    ...
    Private Sub cbo_Bale_Bora_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Bale_Bora.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Bale_Bora, Nothing, "", "", "", "")
        With dgv_Details
            If Asc(e.KeyChar) = 13 Then
                'If .Visible Then
                '    .Focus()
    ...
    Private Sub cbo_Bale_Bora_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_Bale_Bora.TextChanged
        With dgv_Details
            If .Visible Then
                If Trim(cbo_Bale_Bora.Text) = "BORA" Then
                    .Columns(3).HeaderText = "BORA"
                Else
                    .Columns(3).HeaderText = "BALE"
                End If
            End If
        End With
        NetAmount_Calculation()
    ...
    Private Sub btn_Generate_eInvoice_Click(sender As Object, e As EventArgs) Handles btn_Generate_eInvoice.Click
        ...
        Cmd.Connection = con
        Cmd.CommandText = "Select count(*) from Cotton_Waste_Sales_Head Where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
        ...
        Cmd.CommandText = "Select count(*) from Cotton_Waste_Sales_Head Where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "' and Len(E_Invoice_IRNO) > 0"
        c = Cmd.ExecuteScalar
        ...
        Dim vInvoice_No As String = ""
        Dim VInvPrefixNo As String = ""
        ...
        If Dt1.Rows.Count > 0 Then
            Inv_No = Dt1.Rows(0).Item("Cotton_Waste_Sales_No").ToString
            VInvPrefixNo = Dt1.Rows(0).Item("Invoice_PrefixNo").ToString
        End If
        ...
        Try
            ...
            If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1088" Or Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1087" Then '---- Kalaimagal Textiles (OE) (palladam)
                'Inv_No = "Cotton_Waste_Sales_No"
                InvSubNo = Replace(Trim(Inv_No), Trim(Val(Inv_No)), "")

                'old
                ' vInvoice_No = "(Invoice_PrefixNo + '/' + Cotton_Waste_Sales_No )"

                If Trim(txt_InvoicePrefixNo.Text) <> "" Then
                    vInvoice_No = VInvPrefixNo & "/" & Trim(Format(Val(Inv_No), "######000")) & Trim(InvSubNo)
                Else
                    vInvoice_No = (Trim(Format(Val(Inv_No), "######000")) & Trim(InvSubNo))
                End If

            Else
                vInvoice_No = VInvPrefixNo & Trim(Inv_No)
            ...
            Cmd.CommandText = "Insert into e_Invoice_Head ( e_Invoice_No     ,         e_Invoice_date , Buyer_IdNo , Consignee_IdNo , Assessable_Value  ,    CGST    ,       SGST ,       IGST , Cess , State_Cess ,       Round_Off, Nett_Invoice_Value ,       Ref_Sales_Code   , Other_Charges , Dispatcher_Idno  ) " &
                                " Select        '" & Trim(vInvoice_No) & "'  , Cotton_Waste_Sales_Date, Ledger_IdNo, DeliveryTo_Idno, Taxable_Amount    , CGST_Amount, SGST_Amount, IGST_Amount,  0   ,       0    , RoundOff_Amount,      Net_Amount    , '" & Trim(NewCode) & "',     0         ,     0     from Cotton_Waste_Sales_Head where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
            Cmd.ExecuteNonQuery()

            'Cmd.CommandText = "Insert into e_Invoice_Head Select '" & Trim(vInvoice_No) & "' , Cotton_Waste_Sales_Date, Ledger_IdNo, DeliveryTo_Idno, Taxable_Amount, " &
            '                  " CGST_Amount, SGST_Amount, IGST_Amount , cmc_Amount , 0, RoundOff_Amount, Net_Amount as Nett_Invoice_Amount, '" & Trim(NewCode) & "', 0 as OtherCharges , 0 from Cotton_Waste_Sales_Head where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
            'Cmd.ExecuteNonQuery()

            'old
            'Cmd.CommandText = "Insert into e_Invoice_Head Select (Invoice_PrefixNo + '/' + Cotton_Waste_Sales_No ) , Cotton_Waste_Sales_Date, Ledger_IdNo, DeliveryTo_Idno, Taxable_Amount, " &
            '                  " CGST_Amount, SGST_Amount, " &
            '                  " IGST_Amount , cmc_Amount , 0, RoundOff_Amount, Net_Amount as Nett_Invoice_Amount, '" & Trim(NewCode) & "', 0 as OtherCharges , DeliveryTo_Idno from Cotton_Waste_Sales_Head where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
            'Cmd.ExecuteNonQuery()
            ...
            '                   " Select 1 as Sl_No, 0 as IsServc, a.Description as producDescription , a.HSN_Code, '' as batchdetails, a.Total_Weight, 'KGS' as UOM, a.Rate, (a.Amount + (Freight_Amount+AddLess_Amount) ), a.Discount_Amount, " &
            ...
            '                  " from Cotton_Waste_Sales_Head a INNER JOIN Cotton_Waste_Sales_Details b On a.Cotton_Waste_Sales_Code = b.Cotton_Waste_Sales_Code" &
            '                 " inner join Variety_Head C on b.Variety_idno = c.Variety_idno " &
            '                  " Where b.Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
            'Cmd.ExecuteNonQuery()


            Cmd.CommandText = "Insert into e_Invoice_Details  " &
                               " Select b.Sl_No, 0 as IsServc, a.Description as producDescription , a.HSN_Code, '' as batchdetails, a.Total_Weight, 'KGS' as UOM, a.Rate, (a.Amount + (Freight_Amount+AddLess_Amount) ), a.Discount_Amount, " &
                              " a.Taxable_Amount, a.GST_Percentage, 0 AS SgstAmt, 0 AS CgstAmt, 0 AS igstAmt,  (CASE WHEN b.Sl_no = 1 then a.cmc_Percentage else 0 end )  AS CesRt, (CASE WHEN b.Sl_no = 1 then a.cmc_Amount else 0 end ) AS CesAmt , 0 AS CesNonAdvlAmt, 0 AS StateCesRt, 0 AS StateCesAmt, 0 as StateCesNonAdvlAmt, " &
                              " 0 as OthChrg, 0 as TotItemVal, '' as AttributesDetails, '" & Trim(NewCode) & "' " &
                              " from Cotton_Waste_Sales_Head a INNER JOIN Cotton_Waste_Sales_Details b On a.Cotton_Waste_Sales_Code = b.Cotton_Waste_Sales_Code" &
                             " inner join Variety_Head C on b.Variety_idno = c.Variety_idno " &
                              " Where b.Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
        ...
        Dim einv As New eInvoice(Val(lbl_Company.Tag))
        einv.GenerateIRN(Val(lbl_Company.Tag), NewCode, con, rtbeInvoiceResponse, pic_IRN_QRCode_Image, txt_eInvoiceNo, txt_eInvoiceAckNo, txt_eInvoiceAckDate, txt_eInvoice_CancelStatus, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", Pk_Condition)
    ...
    Private Sub btn_Delete_eInvoice_Click(sender As Object, e As EventArgs) Handles btn_Delete_eInvoice.Click
        ...
        Dim NewCode As String = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        Dim einv As New eInvoice(Val(lbl_Company.Tag))
        einv.CancelIRNByIRN(txt_eInvoiceNo.Text, rtbeInvoiceResponse, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", con, txt_eInvoice_CancelStatus, NewCode, "Data entry mistake")
    ...
    Private Sub btn_Refresh_eInvoice_Info_Click(sender As Object, e As EventArgs) Handles btn_Refresh_eInvoice_Info.Click
        ...
        'Dim NewCode As String = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        'Dim einv As New eInvoice(Val(lbl_Company.Tag))
        'einv.RefresheInvoiceInfoByIRN(txt_eInvoiceNo.Text, Val(lbl_Company.Tag), NewCode, con, rtbeInvoiceResponse, pic_IRN_QRCode_Image, txt_eInvoiceNo, txt_eInvoiceAckNo, txt_eInvoiceAckDate, txt_eInvoice_CancelStatus, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code")



        Dim NewCode As String = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_BillNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)

        Dim da As New SqlClient.SqlDataAdapter("Select E_Invoice_QR_Image,E_Invoice_IRNO,E_Invoice_ACK_No,E_Invoice_ACK_Date,E_Invoice_Cancelled_Status FROM  Cotton_Waste_Sales_Head WHERE Cotton_Waste_Sales_Code = '" & NewCode & "'", con)
    ...
    Private Sub btn_Get_QR_Code_Click(sender As Object, e As EventArgs) Handles btn_Get_QR_Code.Click
        ...
        CMD.CommandText = " INSERT INTO " & Common_Procedures.CompanyDetailsDataBaseName & "..e_Invoice_Refresh ([IRN] ,[ACK_No] , [DOC_No] , [SEARCH_BY]  , [COMPANY_IDNO],[Update_Table] ,[Update_table_Unique_Code],[COMPANYGROUP_IDNO] ) VALUES " &
                          "('" & txt_eInvoiceNo.Text & "' ,'','','I'," & Val(Common_Procedures.CompIdNo).ToString & ",'Cotton_Waste_Sales_Head', 'E_Invoice_IRNO'," & Val(Common_Procedures.CompGroupIdNo).ToString & ")"
        CMD.ExecuteNonQuery()
    ...
    Private Sub btn_Generate_EWB_Click(sender As Object, e As EventArgs) Handles btn_Generate_EWB.Click
        ...
        Dim Cmd As New SqlClient.SqlCommand
        Cmd.Connection = con
        Cmd.CommandText = "Select count(*) from Cotton_Waste_Sales_Details Where Cotton_Waste_Sales_Code = '" & NewCode & "'"
        Dim c As Int16 = Cmd.ExecuteScalar
        ...
        Cmd.CommandText = "Select count(*) from Cotton_Waste_Sales_Head Where Cotton_Waste_Sales_Code = '" & NewCode & "' and (Len(EWay_BIll_No) >0 or Len(E_Invoice_IRNO) = 0 OR E_Invoice_IRNO IS NULL )"
        c = Cmd.ExecuteScalar
        ...
        Try
            ...
            Cmd.CommandText = "Insert into EWB_By_IRN  (	[IRN]                ,	[TransID]        ,	[TransMode] ,	[TransDocNo] ,[TransDocDate]  ,	[VehicleNo]  , [Distance],	[VehType]  ,	[TransName]         , [InvCode]   , Company_Idno   , Company_Pincode  , Shipped_To_Idno ,  Shipped_To_Pincode )   Select A.E_Invoice_IRNO  ,  t.Ledger_GSTINNo,        '1'    ,        ''   ,   ''     ,       a.Vechile_No     , (CASE WHEN a.DeliveryTo_IdNo <> 0 THEN  D.Distance ELSE L.Distance END),      'R'    ,  t.Ledger_Mainname     ,'" & NewCode & "' , a.Company_Idno  , tz.Company_Pincode  , (CASE WHEN a.DeliveryTo_IdNo <> 0 THEN  a.DeliveryTo_IdNo ELSE a.Ledger_idno END) , (CASE WHEN a.DeliveryTo_IdNo <> 0 THEN  d.Pincode ELSE L.Pincode END)   " &
                                                       " from Cotton_Waste_Sales_Head a INNER JOIN Company_Head tz on tz.Company_idno = a.Company_Idno INNER JOIN Ledger_Head L on a.Ledger_IdNo = L.Ledger_IdNo LEFT OUTER JOIN Ledger_Head D on a.DeliveryTo_IdNo = D.Ledger_IdNo LEFT OUTER JOIN Ledger_Head T on a.Transport_IdNo = T.Ledger_IdNo  Where a.Cotton_Waste_Sales_Code = '" & NewCode & "'"
        ...
        Dim einv As New eInvoice(Val(lbl_Company.Tag))
        einv.GenerateEWBByIRN(NewCode, rtbeInvoiceResponse, txt_eWayBill_No, txt_EWB_Date, txt_EWB_ValidUpto, con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", txt_EWB_Canellation_Reason, txt_EWB_Cancel_Status, Pk_Condition)
    ...
    Private Sub btn_Cancel_EWB_Click(sender As Object, e As EventArgs) Handles btn_Cancel_EWB.Click
        ...
        einv.Cancel_EWB_IRN(NewCode, txt_eWayBill_No.Text, rtbeInvoiceResponse, txt_eInvoice_CancelStatus, con, "Cotton_Waste_Sales_Head", "Cotton_Waste_Sales_Code", txt_EWB_Canellation_Reason.Text)
    End Sub
    ...
    Private Sub cbo_Transport_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_Transport.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Transport, cbo_VehicleNo, cbo_DeliveryTo, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'TRANSPORT')", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub msk_Date_TextChanged(sender As Object, e As System.EventArgs) Handles dtp_Date.TextChanged
        NetAmount_Calculation()
    End Sub
    ...
    Private Sub cbo_Vechile_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_VehicleNo.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Cotton_Waste_Sales_Head", "Vechile_No", "", "")
    End Sub
    ...
    Private Sub cbo_VehicleNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_VehicleNo.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, sender, txt_CommKg, cbo_Transport, "Cotton_Waste_Sales_Head", "Vechile_No", "", "")
    End Sub
    ...
    Private Sub cbo_VehicleNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_VehicleNo.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, sender, cbo_Transport, "Cotton_Waste_Sales_Head", "Vechile_No", "", "", False, True)
    End Sub
