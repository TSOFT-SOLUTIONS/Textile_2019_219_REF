Public Class PayRoll_Employee_Timing_Addition
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_TIMING_ADDITION, New_Entry, Me, con, "Payroll_Timing_Addition_Head", "Timing_Addition_Code", NewCode, "Timing_Addition_Date", "(Timing_Addition_Code = '" & Trim(NewCode) & "')") = False Then Exit Sub
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.InsertingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_TIMING_ADDITION, New_Entry, Me) = False Then Exit Sub
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        End If
        NewCode = Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RefNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_TIMING_ADDITION, New_Entry, Me, con, "Payroll_Timing_Addition_Head", "Timing_Addition_Code", NewCode, "Timing_Addition_Date", "(Timing_Addition_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Timing_Addition_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Timing_Addition_no desc", dtp_Date.Value.Date) = False Then Exit Sub
        ...
        Try
            ...
            If New_Entry = True Then
                cmd.CommandText = "Insert into Payroll_Timing_Addition_Head (   Timing_Addition_Code   ,               Company_IdNo       ,           Timing_Addition_No  ,         for_OrderBy    , Timing_Addition_Date ,       Day_Name               ) " & _
                    ...
                    If Val(Empe_ID) <> 0 Then
                        ...
                        If Trim(.Rows(i).Cells(3).Value) <> "" Then
                            ...
                            If IsDate(.Rows(i).Cells(3).Value) = True Then
                                ...
                                cmd.CommandText = "Insert into Payroll_Timing_Addition_Details (         Timing_Addition_Code                ,               Company_IdNo       ,       Timing_Addition_No      ,     for_OrderBy        ,   Timing_Addition_Date,             Sl_No     ,      Employee_IdNo       ,                       InOut_Type        ,                 InOut_Time_Text        ,  InOut_DateTime ) " & _
                                                  "            Values                          ( '" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(OrdBy)) & ",       @EntryDate      ,  " & Str(Val(Sno)) & ", " & Str(Val(Empe_ID)) & ",  '" & Trim(.Rows(i).Cells(2).Value) & "', '" & Trim(.Rows(i).Cells(3).Value) & "', @InOutDateTime  ) "
                                cmd.ExecuteNonQuery()
