Public Class Stores_Item_PurchaseReturn_Entry
    ...
    Private Pk_Condition As String = "ITPRT-"
    Private Pk_Condition1 As String = "ITPRA-"
    ...
    Private Sub Item_PurchaseReturn_Entry_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
    ...
    Private Sub Item_PurchaseReturn_Entry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim da As New SqlClient.SqlDataAdapter
        Dim dt1 As New DataTable
        ...
        cbo_EntType.Items.Add("PURCHASE")

        cbo_POType.Items.Clear()
        cbo_POType.Items.Add("CANCEL")
        cbo_POType.Items.Add("REPLACEMENT")
        ...
        AddHandler cbo_POType.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler cbo_POType.LostFocus, AddressOf ControlLostFocus
    ...
    Private Sub Item_PurchaseReturn_Entry_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        con.Close()
        con.Dispose()
    End Sub

    Private Sub Item_PurchaseReturn_Entry_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        Try
            ...
            If IsNothing(dgv1) = False Then
                ...
                With dgv1
                    ...
                    If keyData = Keys.Enter Or keyData = Keys.Down Then
                        ...
                        If Trim(UCase(cbo_EntType.Text)) = "PURCHASE" Then
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name from Stores_Item_PurchaseReturn_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo  Where a.Item_PurchaseReturn_Code = '" & Trim(NewCode) & "'", con)
            da1.Fill(dt1)
            ...
            If dt1.Rows.Count > 0 Then
                ...
                cbo_POType.Text = dt1.Rows(0).Item("PO_Type").ToString
                ...
                cbo_PurchaseAc.Text = Common_Procedures.Ledger_IdNoToName(con, Val(dt1.Rows(0).Item("PurchaseReturnAc_IdNo").ToString))
                cbo_PaymentMethod.Text = dt1.Rows(0).Item("Payment_Method").ToString

                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_name, b.Drawing_No, c.Department_name, d.Unit_name, e.Brand_Name from Stores_Item_PurchaseReturn_Details a INNER JOIN Stores_Item_Head b ON a.Item_idno = b.Item_idno LEFT OUTER JOIN Department_Head c ON b.Department_idno = c.Department_idno LEFT OUTER JOIN Unit_Head d ON a.Unit_idno = d.Unit_idno LEFT OUTER JOIN Brand_Head e ON a.Brand_idno = e.Brand_idno where a.Item_PurchaseReturn_Code = '" & Trim(NewCode) & "' Order by a.Sl_No", con)
                dt2 = New DataTable
                da2.Fill(dt2)
                ...
                If dt2.Rows.Count > 0 Then
                    ...
                    For i = 0 To dt2.Rows.Count - 1
                        ...
                        dgv_Details.Rows(n).Cells(9).Value = dt2.Rows(i).Item("Purchase_No").ToString
                        dgv_Details.Rows(n).Cells(10).Value = dt2.Rows(i).Item("Bill_No").ToString
                        dgv_Details.Rows(n).Cells(11).Value = dt2.Rows(i).Item("Po_No").ToString
                        dgv_Details.Rows(n).Cells(12).Value = dt2.Rows(i).Item("Purchase_Code").ToString
                        dgv_Details.Rows(n).Cells(13).Value = Val(dt2.Rows(i).Item("Purchase_Details_SlNo").ToString)
                        dgv_Details.Rows(n).Cells(14).Value = dt2.Rows(i).Item("Po_Code").ToString
                        dgv_Details.Rows(n).Cells(15).Value = Val(dt2.Rows(i).Item("Po_Details_SlNo").ToString)
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Entry_Stores_Purchase_Return, "~L~") = 0 And InStr(Common_Procedures.UR.Entry_Stores_Purchase_Return, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.Store_purchase_Return_entry, New_Entry, Me, con, "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", NewCode, "Item_PurchaseReturn_Date", "(Item_PurchaseReturn_Code = '" & Trim(NewCode) & "')") = False Then Exit Sub
        ...
        Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "DELETE", "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, "", "", "Item_PurchaseReturn_Code, Company_IdNo, for_OrderBy", trans)

        Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "DELETE", "Stores_Item_PurchaseReturn_Details", "Item_PurchaseReturn_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, " Item_IdNo, Brand_IdNo, Quantity, Unit_idNo, Rate, Amount, Purchase_No, Bill_No, Po_No, Purchase_Code, Purchase_Details_SlNo, Po_Code, PO_Details_SlNo", "Sl_No", "Item_PurchaseReturn_Code, For_OrderBy, Company_IdNo, Item_PurchaseReturn_No, Item_PurchaseReturn_Date, Ledger_Idno", trans)
        ...
        Try
            ...
            cmd.CommandText = "Update Stores_Item_Purchase_Details Set PurchaseReturn_Quantity = a.PurchaseReturn_Quantity - b.Quantity from Stores_Item_Purchase_Details a, Stores_Item_PurchaseReturn_Details b where b.Item_PurchaseReturn_Code = '" & Trim(NewCode) & "' and b.Entry_Type = 'PURCHASE' and a.Item_Purchase_Code = b.Purchase_Code and a.Purchase_Details_SlNo = b.Purchase_Details_SlNo"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "Update Stores_Item_PO_Details Set PurchaseReturn_Quantity = a.PurchaseReturn_Quantity - b.Quantity from Stores_Item_PO_Details a, Stores_Item_PurchaseReturn_Details b where b.Item_PurchaseReturn_Code = '" & Trim(NewCode) & "' and b.Entry_Type = 'PURCHASE' and b.PO_Type = 'REPLACEMENT' and a.Po_Code = b.Po_Code and a.PO_Details_SlNo = b.PO_Details_SlNo"
            cmd.ExecuteNonQuery()
            ...
            cmd.CommandText = "Delete from Stores_Item_PurchaseReturn_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code = '" & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "delete from Stores_Item_PurchaseReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code = '" & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        Try
            ...
            ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Entry_Stores_Purchase_Return, "~L~") = 0 And InStr(Common_Procedures.UR.Entry_Stores_Purchase_Return, "~I~") = 0 Then MessageBox.Show("You have No Rights to Insert", "DOES NOT INSERT...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
            If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.InsertingEntry, Common_Procedures.UR.Store_purchase_Return_entry, New_Entry, Me) = False Then Exit Sub

            inpno = InputBox("Enter New Ret No.", "FOR NEW NO INSERTION...")

            RefCode = Trim(Val(lbl_Company.Tag)) & "-" & Trim(inpno) & "/" & Trim(Common_Procedures.FnYearCode)

            Da = New SqlClient.SqlDataAdapter("select Item_PurchaseReturn_No from Stores_Item_PurchaseReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code = '" & Trim(RefCode) & "'", con)
            Da.Fill(Dt)
            ...
            Else
                If Val(inpno) = 0 Then
                    MessageBox.Show("Invalid Ret No", "DOES NOT INSERT RETURN DC...", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Item_PurchaseReturn_No from Stores_Item_PurchaseReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby, Item_PurchaseReturn_No", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Item_PurchaseReturn_No from Stores_Item_PurchaseReturn_Head where for_orderby > " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby, Item_PurchaseReturn_No", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Item_PurchaseReturn_No from Stores_Item_PurchaseReturn_Head where for_orderby < " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby desc, Item_PurchaseReturn_No desc", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select top 1 Item_PurchaseReturn_No from Stores_Item_PurchaseReturn_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby desc, Item_PurchaseReturn_No desc", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        Try
            ...
            lbl_RefNo.Text = Common_Procedures.get_MaxCode(con, "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", "For_OrderBy", "", Val(lbl_Company.Tag), Common_Procedures.FnYearCode)

            lbl_RefNo.ForeColor = Color.Red


            da1 = New SqlClient.SqlDataAdapter("select Top 1 a.*, b.ledger_name as SalesAcName, c.ledger_name as TaxAcName from Stores_Item_PurchaseReturn_Head a LEFT OUTER JOIN Ledger_Head b ON a.PurchaseReturnAc_IdNo = b.Ledger_IdNo LEFT OUTER JOIN Ledger_Head c ON a.TaxAc_IdNo = c.Ledger_IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by a.for_Orderby desc, a.Item_PurchaseReturn_No desc", con)
            dt1 = New DataTable
            da1.Fill(dt1)

            If dt1.Rows.Count > 0 Then
                If dt1.Rows(0).Item("Entry_Type").ToString <> "" Then cbo_EntType.Text = dt1.Rows(0).Item("Entry_Type").ToString
                If dt1.Rows(0).Item("PO_Type").ToString <> "" Then cbo_POType.Text = dt1.Rows(0).Item("PO_Type").ToString
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Try
            ...
            inpno = InputBox("Enter Ret. Dc No.", "FOR FINDING...")
            ...
            Else
                MessageBox.Show("Retur Dc.No. does not exists", "DOES NOT FIND...", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Dim PurcCd As String = ""
        Dim PurcNo As String = ""
        Dim PurcDetSlNo As Long = 0
        Dim PoCd As String = ""
        Dim PoNo As String = ""
        Dim PoDetSlNo As Long = 0
        Dim CsParNm As String
        Dim vOrdByNo As String = ""

        vOrdByNo = Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text)

        '  If Common_Procedures.UserRight_Check(Common_Procedures.UR.Entry_Stores_Purchase_Return, New_Entry) = False Then Exit Sub
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.Store_purchase_Return_entry, New_Entry, Me, con, "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", NewCode, "Item_PurchaseReturn_Date", "(Item_PurchaseReturn_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Item_PurchaseReturn_No desc", dtp_date.Value.Date) = False Then Exit Sub
        ...
        If (Trim(UCase(cbo_EntType.Text)) <> "DIRECT" And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE") Then
        ...
        If (Trim(UCase(cbo_POType.Text)) <> "CANCEL" And Trim(UCase(cbo_POType.Text)) <> "REPLACEMENT") Then
            MessageBox.Show("Invalid P.O Type", "DOES NOT SAVE...", MessageBoxButtons.OK, MessageBoxIcon.Error)
            If cbo_POType.Enabled And cbo_POType.Visible Then cbo_EntType.Focus()
            Exit Sub
        End If
        ...
        Try
            ...
            Else
                ...
                lbl_RefNo.Text = Common_Procedures.get_MaxCode(con, "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", "For_OrderBy", "", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
            ...
            cmd.Parameters.AddWithValue("@PurcRetDate", dtp_date.Value.Date)

            If New_Entry = True Then

                cmd.CommandText = "Insert into Stores_Item_PurchaseReturn_Head(Item_PurchaseReturn_Code,             Company_IdNo         ,     Item_PurchaseReturn_No    ,                               for_OrderBy                              , Item_PurchaseReturn_Date,                 Entry_Type      ,           Ledger_IdNo     ,               PO_Type          ,     PurchaseReturnAc_IdNo  ,        Total_Quantity    ,       Total_Amount       ,      CashDiscount_Percentage       ,       CashDiscount_Amount            ,            Assessable_Value               ,            TaxAc_IdNo    ,             Tax_Type            ,             Tax_Percentage        ,             Tax_Amount              ,               Freight_Amount       ,             AddLess_Amount       ,             RoundOff_Amount         ,                   Net_Amount             ,               Remarks      , Payment_Method     ) " & _
                                    "              Values              ('" & Trim(NewCode) & "' , " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ",       @PurcRetDate      , '" & Trim(cbo_EntType.Text) & "', " & Str(Val(Led_IdNo)) & ", '" & Trim(cbo_POType.Text) & "', " & Str(Val(PurcAc_ID)) & ", " & Str(Val(vTotQty)) & ", " & Str(Val(vTotAmt)) & ", " & Str(Val(txt_DiscPerc.Text)) & ", " & Str(Val(lbl_DiscAmount.Text)) & ", " & Str(Val(lbl_AssessableValue.Text)) & ", " & Str(Val(TxAc_ID)) & ", '" & Trim(cbo_TaxType.Text) & "', " & Str(Val(txt_TaxPerc.Text)) & ", " & Str(Val(lbl_TaxAmount.Text)) & ", " & Str(Val(txt_Freight.Text)) & ", " & Str(Val(txt_AddLess.Text)) & ", " & Str(Val(lbl_RoundOff.Text)) & ", " & Str(Val(CSng(lbl_NetAmount.Text))) & ", '" & Trim(txt_Remarks.Text) & "'  ,'" & Trim(cbo_PaymentMethod.Text) & "' )"
                cmd.ExecuteNonQuery()

            Else

                Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "OLD", "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "", "", "Item_PurchaseReturn_Code, Company_IdNo, for_OrderBy", tr)

                Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "OLD", "Stores_Item_PurchaseReturn_Details", "Item_PurchaseReturn_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, " Item_IdNo, Brand_IdNo, Quantity, Unit_idNo, Rate, Amount, Purchase_No, Bill_No, Po_No, Purchase_Code, Purchase_Details_SlNo, Po_Code, PO_Details_SlNo", "Sl_No", "Item_PurchaseReturn_Code, For_OrderBy, Company_IdNo, Item_PurchaseReturn_No, Item_PurchaseReturn_Date, Ledger_Idno", tr)

                cmd.CommandText = "Update Stores_Item_PurchaseReturn_Head set Item_PurchaseReturn_Date= @PurcRetDate, Entry_Type = '" & Trim(cbo_EntType.Text) & "', Ledger_IdNo = " & Str(Val(Led_IdNo)) & ", PO_Type = '" & Trim(cbo_POType.Text) & "', PurchaseReturnAc_IdNo = " & Str(Val(PurcAc_ID)) & ", Total_Quantity = " & Str(Val(vTotQty)) & ", Total_Amount = " & Str(Val(vTotAmt)) & ", CashDiscount_Percentage = " & Str(Val(txt_DiscPerc.Text)) & ", CashDiscount_Amount = " & Str(Val(lbl_DiscAmount.Text)) & ", Assessable_Value = " & Str(Val(lbl_AssessableValue.Text)) & ", TaxAc_IdNo = " & Str(Val(TxAc_ID)) & ", Tax_Type = '" & Trim(cbo_TaxType.Text) & "', Tax_Percentage = " & Str(Val(txt_TaxPerc.Text)) & ", Tax_Amount = " & Str(Val(lbl_TaxAmount.Text)) & ", Freight_Amount = " & Str(Val(txt_Freight.Text)) & ", AddLess_Amount = " & Str(Val(txt_AddLess.Text)) & ", Payment_Method = '" & Trim(cbo_PaymentMethod.Text) & "' ,  RoundOff_Amount = " & Str(Val(lbl_RoundOff.Text)) & ", Net_Amount = " & Str(Val(CSng(lbl_NetAmount.Text))) & ", Remarks = '" & Trim(txt_Remarks.Text) & "' Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code = '" & Trim(NewCode) & "'"
                cmd.ExecuteNonQuery()
            ...
            End If
            Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "NEW", "Stores_Item_PurchaseReturn_Head", "Item_PurchaseReturn_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "", "", "Item_PurchaseReturn_Code, Company_IdNo, for_OrderBy", tr)

           
            EntID = Trim(Pk_Condition) & Trim(lbl_RefNo.Text)
            PBlNo = Trim(lbl_RefNo.Text)
            Partcls = "PurcRet : Ret.DcNo. " & Trim(lbl_RefNo.Text)
            ...
            With dgv_Details
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Val(dgv_Details.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        PurcCd = "" : PurcNo = "" : PurcDetSlNo = 0
                        PoCd = "" : PoNo = "" : PoDetSlNo = 0
                        If Trim(UCase(cbo_EntType.Text)) = "PURCHASE" Then
                            PurcNo = dgv_Details.Rows(i).Cells(9).Value
                            PoNo = dgv_Details.Rows(i).Cells(11).Value
                            PurcCd = dgv_Details.Rows(i).Cells(12).Value
                            PurcDetSlNo = Val(dgv_Details.Rows(i).Cells(13).Value)
                            PoCd = dgv_Details.Rows(i).Cells(14).Value
                            PoDetSlNo = Val(dgv_Details.Rows(i).Cells(15).Value)
                        End If

                        cmd.CommandText = "Insert into Stores_Item_PurchaseReturn_Details ( Item_PurchaseReturn_Code, Company_IdNo, Item_PurchaseReturn_No, for_OrderBy, Item_PurchaseReturn_Date, Entry_Type, PO_Type, Ledger_IdNo, Sl_No, Item_IdNo, Brand_IdNo, Quantity, Unit_idNo, Rate, Amount, Purchase_No, Bill_No, Po_No, Purchase_Code, Purchase_Details_SlNo, Po_Code, PO_Details_SlNo) Values ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ", @PurcRetDate, '" & Trim(cbo_EntType.Text) & "', '" & Trim(cbo_POType.Text) & "', " & Str(Val(Led_IdNo)) & ", " & Str(Val(Sno)) & ", " & Str(Val(Item_ID)) & ", " & Str(Val(Brand_ID)) & ", " & Str(Val(.Rows(i).Cells(5).Value)) & ", " & Val(Unit_ID) & ", " & Str(Val(.Rows(i).Cells(7).Value)) & ", " & Str(Val(.Rows(i).Cells(8).Value)) & ", '" & Trim(PurcNo) & "', '" & Trim(.Rows(i).Cells(10).Value) & "', '" & Trim(PoNo) & "', '" & Trim(PurcCd) & "', " & Str(Val(PurcDetSlNo)) & ", '" & Trim(PoCd) & "', " & Str(Val(PoDetSlNo)) & " )"
                        cmd.ExecuteNonQuery()
                        ...
                        If Trim(UCase(cbo_EntType.Text)) = "PURCHASE" Then
                            ...
                            If Trim(.Rows(i).Cells(12).Value) <> "" Then
                                ...
                                cmd.CommandText = "Update Stores_Item_Purchase_Details Set PurchaseReturn_Quantity = PurchaseReturn_Quantity + " & Str(Val(.Rows(i).Cells(5).Value)) & " where Item_Purchase_Code = '" & Trim(.Rows(i).Cells(12).Value) & "' and Purchase_Details_SlNo = " & Str(Val(.Rows(i).Cells(13).Value)) & " and Ledger_IdNo = " & Str(Val(Led_IdNo))
                                Nr = cmd.ExecuteNonQuery()

                                If Nr = 0 Then
                                    tr.Rollback()
                                    MessageBox.Show("Mismatch of Purchase and Party details", "FOR SAVING...", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                    If dtp_date.Enabled And dtp_date.Visible Then dtp_date.Focus()
                                    Exit Sub
                                End If
                            ...
                            If Trim(UCase(cbo_POType.Text)) = "REPLACEMENT" Then

                                If Trim(.Rows(i).Cells(14).Value) <> "" Then

                                    cmd.CommandText = "Update Stores_Item_PO_Details Set PurchaseReturn_Quantity = PurchaseReturn_Quantity + " & Str(Val(.Rows(i).Cells(5).Value)) & " where Po_Code = '" & Trim(.Rows(i).Cells(14).Value) & "' and PO_Details_SlNo = " & Str(Val(.Rows(i).Cells(15).Value)) & " and Ledger_IdNo = " & Str(Val(Led_IdNo))
                                    Nr = cmd.ExecuteNonQuery()
            ...
            vLed_IdNos = PurcAc_ID & "|" & Led_IdNo
            vVou_Amts = Val(CSng(lbl_NetAmount.Text)) & "|" & -1 * (Val(CSng(lbl_NetAmount.Text)))
            If Common_Procedures.Voucher_Updation(con, "Store.ItemPurcRetn", Val(lbl_Company.Tag), Trim(Pk_Condition) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(dtp_date.Text), "Ref No : " & Trim(lbl_RefNo.Text), vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                Throw New ApplicationException(ErrMsg)
            End If
            'End If

            Dim VouBil As String = ""
            VouBil = Common_Procedures.VoucherBill_Posting(con, Val(lbl_Company.Tag), dtp_date.Text, Led_IdNo, Trim(lbl_RefNo.Text), 0, Val(CSng(lbl_NetAmount.Text)), "DR", Trim(Pk_Condition) & Trim(NewCode), tr)
            ...
            If Trim(UCase(cbo_PaymentMethod.Text)) = "CASH" And Led_IdNo <> 1 Then
                ...
                If Val(CSng(lbl_NetAmount.Text)) <> 0 Then
                    vLed_IdNos = Led_IdNo & "|" & 1
                    vVou_Amts = Val(CSng(lbl_NetAmount.Text)) & "|" & -1 * (Val(CSng(lbl_NetAmount.Text)))
                    If Common_Procedures.Voucher_Updation(con, "Store.ItemPurcRetn", Val(lbl_Company.Tag), Trim(Pk_Condition1) & Trim(NewCode), Trim(lbl_RefNo.Text), Convert.ToDateTime(dtp_date.Text), "Ref No : " & Trim(lbl_RefNo.Text), vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                        Throw New ApplicationException(ErrMsg)
                    End If
                End If
            End If
        ...
        Catch ex As Exception
            ...
            ElseIf InStr(1, LCase(ex.Message), "ck_Stores_Item_Purchase_Details_2") > 0 Then
                MessageBox.Show("Invalid Return quantity, Greater than Purchase Quantity", "DOES NOT SAVE...", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub cbo_Ledger_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Ledger.KeyDown
        cbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Ledger, cbo_PaymentMethod, cbo_PurchaseAc, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = '' and AccountsGroup_IdNo = 14 ) ", "(Ledger_IdNo = 0)")
    End Sub

    Private Sub cbo_Ledger_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Ledger.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Ledger, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = '' and AccountsGroup_IdNo = 14 ) ", "(Ledger_IdNo = 0)")
    ...
    Private Sub cbo_PurchaseAc_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PurchaseAc.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PurchaseAc, cbo_Ledger, cbo_POType, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 27)", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_PurchaseAc_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PurchaseAc.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PurchaseAc, cbo_POType, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 27)", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub dgv_Details_CellEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellEnter
        ...
        With dgv_Details
            ...
            If e.ColumnIndex = 1 And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
            ...
            If e.ColumnIndex = 2 And vCloPic_STS = False And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
                btn_ShowPicture_Click(sender, e)
            Else
                pnl_Picture.Visible = False
            End If

            If e.ColumnIndex = 3 And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
            ...
            If e.ColumnIndex = 4 And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
            ...
            'If e.ColumnIndex = 6  And Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
        ...
        With dgv_Details
            If .Visible Then
                ...
                ElseIf .CurrentCell.ColumnIndex = 7 Then
                    If Common_Procedures.Accept_NumericOnly(Asc(e.KeyChar)) = 0 Or Trim(UCase(cbo_EntType.Text)) = "PURCHASE" Then
                        e.Handled = True
                    End If
    ...
    Private Sub btn_Filter_Show_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Filter_Show.Click
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select a.*, B.Ledger_Name from Stores_Item_PurchaseReturn_Head a INNER JOIN  ledger_head b on a.Ledger_IdNo = b.Ledger_IdNo Where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Item_PurchaseReturn_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' " & IIf(Trim(Condt) <> "", " and ", "") & Condt & " Order by a.Item_PurchaseReturn_Date, a.for_orderby, a.Item_PurchaseReturn_No", con)
            dt2 = New DataTable
            da.Fill(dt2)
            ...
            If dt2.Rows.Count > 0 Then
                ...
                For i = 0 To dt2.Rows.Count - 1
                    ...
                    dgv_Filter_Details.Rows(n).Cells(1).Value = dt2.Rows(i).Item("Item_PurchaseReturn_No").ToString
                    dgv_Filter_Details.Rows(n).Cells(2).Value = Format(Convert.ToDateTime(dt2.Rows(i).Item("Item_PurchaseReturn_Date").ToString), "dd-MM-yyyy")
    ...
    Private Sub cbo_TaxType_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_TaxType.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_TaxType, txt_DiscPerc, txt_TaxPerc, "", "", "", "")
    End Sub
    ...
    Private Sub cbo_TaxType_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbo_TaxType.SelectedIndexChanged
    ...
    Private Sub NetAmount_Calculation()
        ...
        If Trim(cbo_TaxType.Text) <> "" And Trim(UCase(cbo_TaxType.Text)) <> "-NIL-" Then
    ...
    Public Sub print_record() Implements Interface_MDIActions.print_record
        ...
        NewCode = Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RefNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.PrintEntry, Common_Procedures.UR.Store_purchase_Return_entry, New_Entry) = False Then Exit Sub

        Try

            da1 = New SqlClient.SqlDataAdapter("select * from Stores_Item_PurchaseReturn_Head Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Item_PurchaseReturn_Code = '" & Trim(NewCode) & "'", con)
            dt1 = New DataTable
            da1.Fill(dt1)
    ...
    Private Sub cbo_POType_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_POType.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_POType, cbo_Ledger, Nothing, "", "", "", "")
        If e.KeyValue = 40 Then
    ...
    Private Sub cbo_POType_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_POType.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_POType, Nothing, "", "", "", "")
        If Asc(e.KeyChar) = 13 Then
            If dgv_Details.Rows.Count > 0 Then
                dgv_Details.Focus()
                If Trim(UCase(cbo_EntType.Text)) = "PURCHASE" Then
    ...
    Private Sub btn_Selection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Selection.Click
        ...
        Dim NewCode As String
        Dim Ent_Qty As Single, Ent_Rate As Single, Ent_DiscPerc As Single
        Dim Ent_TaxType As String, Ent_TaxPerc As Single

        If Trim(UCase(cbo_EntType.Text)) <> "PURCHASE" Then
            MessageBox.Show("Invalid Entry Type", "DOES NOT SELECT PURCHASE...", MessageBoxButtons.OK, MessageBoxIcon.Error)
            If cbo_EntType.Enabled And cbo_EntType.Visible Then cbo_EntType.Focus()
            Exit Sub
        End If
        ...
        With dgv_Selection
            ...
            Da = New SqlClient.SqlDataAdapter("select a.*, b.Item_name, b.Drawing_No, c.Department_name, d.Unit_name, e.Brand_Name, f.Quantity as Ent_Ret_Quantity, f.Rate as Ent_Rate, g.CashDiscount_Percentage as Ent_DiscPerc, g.Tax_Type as Ent_VatType, g.Tax_Percentage as Ent_VatPerc from Stores_Item_Purchase_Details a INNER JOIN Stores_Item_Head b ON a.Item_idno = b.Item_idno LEFT OUTER JOIN Department_Head c ON b.Department_idno = c.Department_idno LEFT OUTER JOIN Unit_Head d ON a.Unit_idno = d.Unit_idno LEFT OUTER JOIN Brand_Head e ON a.Brand_idno = e.Brand_idno LEFT OUTER JOIN Stores_Item_PurchaseReturn_Details F ON f.Item_PurchaseReturn_Code = '" & Trim(NewCode) & "' and f.Entry_Type = '" & Trim(cbo_EntType.Text) & "' and a.Item_Purchase_Code = f.Purchase_Code and a.Purchase_Details_SlNo = f.Purchase_Details_SlNo LEFT OUTER JOIN Stores_Item_PurchaseReturn_Head g ON f.Item_PurchaseReturn_Code = g.Item_PurchaseReturn_Code Where a.ledger_idno = " & Str(Val(LedIdNo)) & " and ( (a.Quantity - a.PurchaseReturn_Quantity) > 0 or f.Quantity > 0 ) Order by a.Item_Purchase_Date desc, a.For_OrderBy desc, a.PO_No, a.PO_Details_SlNo", con)
            Dt1 = New DataTable
            Da.Fill(Dt1)
            ...
            If Dt1.Rows.Count > 0 Then
                ...
                For i = 0 To Dt1.Rows.Count - 1
                    ...
                    Ent_Qty = 0 : Ent_Rate = 0 : Ent_DiscPerc = 0 : Ent_TaxType = "" : Ent_TaxPerc = 0

                    If IsDBNull(Dt1.Rows(i).Item("Ent_Ret_Quantity").ToString) = False Then Ent_Qty = Val(Dt1.Rows(i).Item("Ent_Ret_Quantity").ToString)
                    If IsDBNull(Dt1.Rows(i).Item("Ent_Rate").ToString) = False Then Ent_Rate = Val(Dt1.Rows(i).Item("Ent_Rate").ToString)
                    If IsDBNull(Dt1.Rows(i).Item("Ent_DiscPerc").ToString) = False Then Ent_DiscPerc = Val(Dt1.Rows(i).Item("Ent_DiscPerc").ToString)
                    If IsDBNull(Dt1.Rows(i).Item("Ent_VatType").ToString) = False Then Ent_TaxType = Dt1.Rows(i).Item("Ent_VatType").ToString
                    If IsDBNull(Dt1.Rows(i).Item("Ent_VatPerc").ToString) = False Then Ent_TaxPerc = Val(Dt1.Rows(i).Item("Ent_VatPerc").ToString)

                    If (Val(Dt1.Rows(i).Item("Quantity").ToString) - Val(Dt1.Rows(i).Item("PurchaseReturn_Quantity").ToString) + Ent_Qty) > 0 Then
                        ...
                        .Rows(n).Cells(3).Value = Dt1.Rows(i).Item("PO_No").ToString

                        If IsDBNull(Dt1.Rows(i).Item("Department_name").ToString) = False Then
                            If Trim(Dt1.Rows(i).Item("Department_name").ToString) <> "" Then
                                .Rows(n).Cells(4).Value = Dt1.Rows(i).Item("Department_name").ToString
                            Else
                                .Rows(n).Cells(4).Value = Common_Procedures.Department_IdNoToName(con, 1)
                            End If
                        End If

                        .Rows(n).Cells(5).Value = Dt1.Rows(i).Item("Drawing_No").ToString
                        .Rows(n).Cells(6).Value = Dt1.Rows(i).Item("Item_name").ToString
                        .Rows(n).Cells(7).Value = Dt1.Rows(i).Item("Brand_name").ToString
                        .Rows(n).Cells(8).Value = (Val(Dt1.Rows(i).Item("Quantity").ToString) - Val(Dt1.Rows(i).Item("PurchaseReturn_Quantity").ToString) + Ent_Qty)
                        .Rows(n).Cells(9).Value = Dt1.Rows(i).Item("Unit_name").ToString
                        ...
                        End If
                        .Rows(n).Cells(12).Value = Dt1.Rows(i).Item("Item_Purchase_Code").ToString
                        .Rows(n).Cells(13).Value = Val(Dt1.Rows(i).Item("Purchase_Details_SlNo").ToString)
                        .Rows(n).Cells(14).Value = Dt1.Rows(i).Item("Po_Code").ToString
                        .Rows(n).Cells(15).Value = Val(Dt1.Rows(i).Item("PO_Details_SlNo").ToString)
                        .Rows(n).Cells(16).Value = Val(Ent_Qty)
                        .Rows(n).Cells(17).Value = Val(Ent_Rate)
                        .Rows(n).Cells(18).Value = Val(Ent_DiscPerc)
                        .Rows(n).Cells(19).Value = Ent_TaxType
                        .Rows(n).Cells(20).Value = Val(Ent_TaxPerc)
    ...
    Private Sub Grid_Selection(ByVal RwIndx As Integer)
        ...
        With dgv_Selection
            ...
            If .RowCount > 0 And RwIndx >= 0 Then
                ...
                If Val(.Rows(RwIndx).Cells(8).Value) = 0 And Trim(.Rows(RwIndx).Cells(12).Value) = "" Then Exit Sub
    ...
    Private Sub btn_Close_Selection_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Close_Selection.Click
        ...
        Dim Ent_DiscPerc As Single, Ent_TaxPerc As Single
        Dim Ent_TaxType As String, Ent_SelPurCode As String
        ...
        Ent_DiscPerc = 0 : Ent_TaxPerc = 0
        Ent_TaxType = "" : Ent_SelPurCode = ""

        For i = 0 To dgv_Selection.RowCount - 1

            If Val(dgv_Selection.Rows(i).Cells(11).Value) = 1 Then

                If Trim(Ent_SelPurCode) = "" Then
                    Ent_SelPurCode = dgv_Selection.Rows(i).Cells(12).Value
                End If

                If Val(dgv_Selection.Rows(i).Cells(16).Value) <> 0 Then
                    Ent_Qty = Val(dgv_Selection.Rows(i).Cells(16).Value)
                ...
                If Val(dgv_Selection.Rows(i).Cells(17).Value) <> 0 Then
                    Ent_Rate = Val(dgv_Selection.Rows(i).Cells(17).Value)
                ...
                If Val(Ent_DiscPerc) = 0 Then

                    If Val(dgv_Selection.Rows(i).Cells(18).Value) <> 0 Then
                        Ent_DiscPerc = Val(dgv_Selection.Rows(i).Cells(18).Value)
                    End If

                End If

                If Val(Ent_TaxPerc) = 0 Then

                    If Val(dgv_Selection.Rows(i).Cells(20).Value) <> 0 Then
                        Ent_TaxType = dgv_Selection.Rows(i).Cells(19).Value
                        Ent_TaxPerc = Val(dgv_Selection.Rows(i).Cells(20).Value)
                    End If
                ...
                dgv_Details.Rows(n).Cells(10).Value = dgv_Selection.Rows(i).Cells(2).Value
                dgv_Details.Rows(n).Cells(11).Value = dgv_Selection.Rows(i).Cells(3).Value
                ...
                dgv_Details.Rows(n).Cells(15).Value = dgv_Selection.Rows(i).Cells(15).Value

            End If

        Next i

        If Val(Ent_DiscPerc) = 0 And Val(Ent_TaxPerc) = 0 Then
            If Trim(Ent_SelPurCode) <> "" Then

                Da1 = New SqlClient.SqlDataAdapter("select a.* from Stores_Item_Purchase_Head a Where a.Item_Purchase_Code = '" & Trim(Ent_SelPurCode) & "'", con)
                Dt1 = New DataTable
                Da1.Fill(Dt1)
