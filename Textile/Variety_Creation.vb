Public Class Variety_Creation
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select a.* from Variety_Head a where a.Variety_IdNo = " & Str(Val(idno)) & " And " & Trim(VotherCond) & "", con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("Variety_IdNo").ToString
            txt_Name.Text = dt.Rows(0).Item("Variety_Name").ToString
    ...
    Private Sub BathSize_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Trim(UCase(vEntryType)) = Trim(UCase("WASTE")) Then
            lbl_Heading.Text = "WASTE CREATION"
        Else
            lbl_Heading.Text = "VARIETY CREATION"
        End If

        VotherCond = " variety_type ='" & Trim(UCase(vEntryType)) & "' "
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from Variety_Head where Variety_IdNo = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("Select Variety_IdNo, Variety_Name from Variety_Head where Variety_IdNo <> 0  and  " & Trim(VotherCond) & " order by Variety_IdNo", con)
        Dim dt As New DataTable
        ...
        With dgv_Filter
            ...
            .Columns(1).HeaderText = "VARIETY NAME"
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select min(Variety_IdNo) From Variety_Head Where Variety_IdNo <> 0  and " & Trim(VotherCond) & " ", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select max(Variety_IdNo) from Variety_Head Where Variety_IdNo <> 0 and " & Trim(VotherCond) & " ", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select min(Variety_IdNo) from Variety_Head Where Variety_IdNo > " & Str(Val(lbl_IdNo.Text)) & " And Variety_IdNo <> 0 and " & Trim(VotherCond) & " ", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select max(Variety_IdNo) from Variety_Head Where Variety_IdNo < " & Str(Val(lbl_IdNo.Text)) & " And Variety_IdNo <> 0 and " & Trim(VotherCond) & "", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Variety_Head", "Variety_IdNo", "")
        'lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Variety_Head", "Variety_IdNo","")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("Select Variety_Name from Variety_Head where " & Trim(VotherCond) & " order by Variety_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Open.DataSource = dt
        cbo_Open.DisplayMember = "Variety_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Variety_Head", "Variety_IdNo", "", trans)

                If Val(lbl_IdNo.Text) < 11 Then lbl_IdNo.Text = 11
                cmd.CommandText = "Insert into Variety_Head(Variety_IdNo, Variety_Name, sur_name,HSN_Code,GST_Percentege,variety_type) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "','" & Trim(txt_HSNCode.Text) & "'," & Val(txt_GSTPerc.Text) & ",'" & Trim(vEntryType) & "') "
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Variety_Head set Variety_Name = '" & Trim(txt_Name.Text) & "', sur_name = '" & Trim(Sur) & "',HSN_Code='" & Trim(txt_HSNCode.Text) & "',GST_Percentege=" & Val(txt_GSTPerc.Text) & ", variety_type='" & Trim(vEntryType) & "' where Variety_IdNo = " & Str(Val(lbl_IdNo.Text)) & ""
                cmd.ExecuteNonQuery()
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_variety_head") > 0 Then
                MessageBox.Show("Duplicate Variety Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btn_Open_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Open.Click
        ...
        da = New SqlClient.SqlDataAdapter("select Variety_IdNo from Variety_Head where Variety_Name = '" & Trim(cbo_Open.Text) & "'", con)
        da.Fill(dt)
    ...
    Private Sub cbo_Open_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Open.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Open, Nothing, Nothing, "Variety_Head", "Variety_Name", "variety_type ='" & Trim(vEntryType) & "'", "(Variety_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_Open_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Open.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Open, Nothing, "Variety_Head", "Variety_Name", "variety_type ='" & Trim(vEntryType) & "'", "(Variety_IdNo = 0)")
