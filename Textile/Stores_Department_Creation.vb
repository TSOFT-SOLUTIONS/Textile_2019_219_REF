...
Public Class Stores_Department_Creation
    ...
    Private Sub Department_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        grp_Find.Left = (Me.Width - grp_Find.Width) - 25
        grp_Find.Top = (Me.Height - grp_Find.Height) - 50
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        '  If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Master_Stores_Department_Creation, "~L~") = 0 And InStr(Common_Procedures.UR.Master_Stores_Department_Creation, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        ...
        If Val(lbl_IdNo.Text) <= 100 Then
            MessageBox.Show("Default departments cannot be deleted", "DOES NOT DELETION....", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("select Department_IdNo ,Department_Name from Department_Head where Department_IdNo <> 0 and  Department_idno <> 1 order by Department_IdNo", con)
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select * from Department_head a where Department_idno <> 1 and Department_idno = " & Str(Val(idno)), con)
        da.Fill(dt)
        ...
        If dt.Rows.Count > 0 Then
            ...
            If IsDBNull(dt.Rows(0).Item("Department_Image")) = False Then
                Dim imageData As Byte() = DirectCast(dt.Rows(0).Item("Department_Image"), Byte())
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Department_idno) from Department_Head Where Department_idno <> 0 and  Department_idno <> 1 ", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Department_idno) from Department_Head Where Department_idno <> 0 and  Department_idno <> 1 ", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Department_idno) from Department_Head Where Department_idno > " & Str(Val(lbl_IdNo.Text)) & " and Department_idno <> 0 and  Department_idno <> 1 ", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Department_idno) from Department_Head Where Department_idno < " & Str(Val(lbl_IdNo.Text)) & " and Department_idno <> 0 and  Department_idno <> 1 ", con)
            da.Fill(dt)
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("select Department_Name from Department_Head where Department_idno <> 1 order by Department_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Find.DataSource = dt
        cbo_Find.DisplayMember = "Department_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        '  If Common_Procedures.UserRight_Check(Common_Procedures.UR.Master_Stores_Department_Creation, New_Entry) = False Then Exit Sub
        ...
        da = New SqlClient.SqlDataAdapter("select * from  Department_Head where Department_Idno <> " & Str(Val(lbl_IdNo.Text)) & " and Sur_Name = '" & Trim(Sur) & "' Order by Department_Idno", con)
        dt = New DataTable
        da.Fill(dt)
        If dt.Rows.Count > 0 Then
            MessageBox.Show("The Department Name is duplicated, and it has already been created with an ID of " & dt.Rows(0).Item("Department_Idno").ToString, "DOES NOT SAVE....", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
            Exit Sub
        End If
        dt.Clear()
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                cmd.CommandText = "Insert into Department_Head(Department_idno, Department_Name, Sur_Name, Department_Image ) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "', @photo)"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Department_Head set Department_Name = '" & Trim(txt_Name.Text) & "', Sur_Name = '" & Trim(Sur) & "', Department_Image = @photo Where Department_idno = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
    ...
    Private Sub btn_FindOpen_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_FindOpen.Click
        ...
        da = New SqlClient.SqlDataAdapter("select Department_IdNo from Department_Head where Department_Name = '" & Trim(cbo_Find.Text) & "'", con)
        da.Fill(dt)
    ...
    Private Sub cbo_Find_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Find.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Find, Nothing, Nothing, "Department_Head", "Department_Name", "(Department_idno <> 1)", "(Department_idno = 0)")
    End Sub
    ...
    Private Sub cbo_Find_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Find.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Find, Nothing, "Department_Head", "Department_Name", "(Department_idno <> 1)", "(Department_idno = 0)")
        If Asc(e.KeyChar) = 13 Then
            btn_FindOpen_Click(sender, e)
        End If
    End Sub
