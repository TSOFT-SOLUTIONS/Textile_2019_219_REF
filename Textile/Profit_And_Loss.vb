...
'    Private prn_NoofBmDets As Integer
    ...
    Private Sub Profit_Loss_Simple_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        pnl_MultiInput.Left = (Me.Width - pnl_MultiInput.Width) \ 2
        pnl_MultiInput.Top = (Me.Height - pnl_MultiInput.Height) \ 2
    ...
    Private Sub Profit_AND_LOSS_Calculation()
        ...
        Else
            ...
            lbl_GrossTotalDb.Text = Common_Procedures.Currency_Format(CDbl(lbl_PurchaseAccValue.Text) + CDbl(lbl_DirectExpensesValue.Text) + CDbl(lbl_OpeningStock_Value.Text))
            lbl_GrossTotalCr.Text = Common_Procedures.Currency_Format(CDbl(lbl_PurchaseAccValue.Text) + CDbl(lbl_DirectExpensesValue.Text) + CDbl(lbl_OpeningStock_Value.Text))
...
8:
    ...
    Private Sub cbo_Inputs1_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Inputs1.KeyUp
        If e.Control = False And e.KeyValue = 17 And vcbo_KeyDwnVal = e.KeyValue Then
            Show_MultiInput_List(1)
        End If
    End Sub


    Private Sub Show_MultiInput_List(ByVal indx As Integer)
        Dim Da1 As New SqlClient.SqlDataAdapter
        Dim Dt1 As New DataTable
        ...
        SelcNms = ""
        If Trim(RptCboDet(Val(indx)).MultiSelectedNames_AsString) <> "" Then
            a = Split(RptCboDet(Val(indx)).MultiSelectedNames_AsString, ",")
            SelcNms = Join(a, "~")
        ...
        SqlCondt = " Where (" & RptCboDet(indx).Selection_FieldName & " <> '') "
        If Trim(RptCboDet(indx).Condition) <> "" Then SqlCondt = SqlCondt & " and " & RptCboDet(indx).Condition

        Da1 = New SqlClient.SqlDataAdapter("Select DISTINCT " & RptCboDet(indx).Selection_FieldName & IIf(Trim(RptCboDet(indx).Return_FieldName) <> "", ", ", "") & RptCboDet(indx).Return_FieldName & " from " & RptCboDet(indx).TableName & " " & SqlCondt & " Order by " & RptCboDet(indx).Selection_FieldName, con)
        Dt1 = New DataTable
        Da1.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
            For I = 0 To Dt1.Rows.Count - 1
                n = chklst_MultiInput.Items.Add(Dt1.Rows(I)(0).ToString)
                If Trim(RptCboDet(indx).Return_FieldName) <> "" Then
                    lst_MultiInput_IdNos.Items.Add(Dt1.Rows(I)(1).ToString)
                Else
        ...
        If Val(indx) = 1 Then
            lbl_MultiInput_Heading.Text = "Select " & lbl_Inputs1.Text
            pnl_MultiInput.Left = cbo_Inputs1.Left
            pnl_MultiInput.Top = cbo_Inputs1.Top + cbo_Inputs1.Height + 2
        End If

        chklst_MultiInput.Tag = indx
    ...
    Private Sub Set_CheckedList_SelectedItem_Text()
        ...
        If Val(chklst_MultiInput.Tag) = 1 Then
            If cbo_Inputs1.Visible Then cbo_Inputs1.Text = s
        Else
            dtp_FromDate.Focus()
        End If
    End Sub
    ...
    Private Sub btn_Close_MultiInput_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Close_MultiInput.Click
                ...
                If Trim(RptCboDet(Val(chklst_MultiInput.Tag)).Return_FieldName) <> "" Then
                    If indexChecked <= lst_MultiInput_IdNos.Items.Count - 1 Then
                        If Val(lst_MultiInput_IdNos.Items(indexChecked).ToString) <> 0 Then
                            RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_AsString = Trim(RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_AsString) & IIf(Trim(RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_AsString) <> "", ",", "") & Trim(lst_MultiInput_IdNos.Items(indexChecked).ToString)
                            RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_ForInQuery = Trim(RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_ForInQuery) & IIf(Trim(RptCboDet(Val(chklst_MultiInput.Tag)).MultiSelectedIdNos_ForInQuery) <> "", ",", "") & Trim(lst_MultiInput_IdNos.Items(indexChecked).ToString)
                        End If
                    End If
                End If
        ...
        pnl_MultiInput.Visible = False
        If Val(chklst_MultiInput.Tag) = 1 Then
            If cbo_Inputs1.Visible And cbo_Inputs1.Enabled Then cbo_Inputs1.Focus()
