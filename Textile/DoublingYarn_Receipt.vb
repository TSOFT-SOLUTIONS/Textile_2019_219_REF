Public Class DoublingYarn_Receipt
    ...
    Private Pk_Condition As String = "DYNRC-"
    ...
    Private Sub clear()
        ...
        Cbo_DelTo.Enabled = True
        Cbo_DelTo.BackColor = Color.White
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name , c.Ledger_Name as Transport_Name from DoublingYarn_Receipt_Head a INNER JOIN Ledger_Head b ON a.Ledger_IdNo = b.Ledger_IdNo LEFT OUTER JOIN Ledger_Head c ON a.Transport_IdNo = c.Ledger_IdNo  Where a.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'", con)
            da1.Fill(dt1)

            If dt1.Rows.Count > 0 Then

                lbl_RecNo.Text = dt1.Rows(0).Item("DoublingYarn_Receipt_No").ToString
                dtp_Date.Text = dt1.Rows(0).Item("DoublingYarn_Receipt_Date").ToString
                ...
                Cbo_DelTo.Text = Common_Procedures.Ledger_IdNoToName(con, Val(dt1.Rows(0).Item("DelvTo_Idno").ToString))
                ...
                da2 = New SqlClient.SqlDataAdapter("select a.*,C.Count_Name,d.Colour_Name,e.Mill_Name from DoublingYarn_Receipt_Details a  INNER JOIN Count_Head C ON c.Count_Idno = a.Count_Idno LEFT OUTER JOIN Colour_Head d ON d.Colour_IdNo = a.Colour_IdNo LEFT OUTER JOIN Mill_Head e ON e.Mill_IdNo = a.Mill_IdNo  where a.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' Order by Sl_No", con)
                dt2 = New DataTable
                da2.Fill(dt2)
                ...
                If dt2.Rows.Count > 0 Then
                    ...
                    For i = 0 To dt2.Rows.Count - 1
                        ...
                        dgv_Details.Rows(n).Cells(8).Value = dt2.Rows(i).Item("DoublingYarn_Delivery_Code").ToString
                        dgv_Details.Rows(n).Cells(9).Value = dt2.Rows(i).Item("DoublingYarn_Delivery_Slno").ToString
                        dgv_Details.Rows(n).Cells(10).Value = dt2.Rows(i).Item("DoublingYarn_Receipt_Slno").ToString
                        dgv_Details.Rows(n).Cells(11).Value = dt2.Rows(i).Item("DoublingYarn_BillMaking_Code").ToString

                        dgv_Details.Rows(n).Cells(12).Value = Format(Val(dt2.Rows(i).Item("Excess_Short_Weight").ToString), "########0.000")
                        dgv_Details.Rows(n).Cells(13).Value = Format(Val(dt2.Rows(i).Item("RATE").ToString), "########0.00")
                        dgv_Details.Rows(n).Cells(14).Value = Format(Val(dt2.Rows(i).Item("AMOUNT").ToString), "########0.00")
                        ...
                        'If IsDBNull(dt2.Rows(0).Item("DoublingYarn_BillMaking_Code").ToString) = False Then
                        '    If Trim(dt2.Rows(0).Item("DoublingYarn_BillMaking_Code").ToString) <> "" Then
                        '        LockSTS = True
                        '    End If
                        'End If
                ...
                Else
                    ...
                    If Trim(dgv_Details.Rows(n).Cells(1).Value) = "" And Val(dgv_Details.Rows(n).Cells(7).Value) = 0 Then
                ...
                With dgv_Details_Total
                    ...
                    .Rows(0).Cells(14).Value = Format(Val(dt1.Rows(0).Item("Net_Amount").ToString), "########0.00")
                ...
                If LockSTS = True Then
                    ...
                    Cbo_DelTo.Enabled = False
                    Cbo_DelTo.BackColor = Color.LightGray
    ...
    Private Sub DoublingYarn_Receipt_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Try
    ...
    Private Sub DoublingYarn_Receipt_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        AddHandler Cbo_DelTo.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler Cbo_DelTo.LostFocus, AddressOf ControlLostFocus
    ...
    Private Sub DoublingYarn_Receipt_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        con.Close()
        con.Dispose()
        Common_Procedures.Hide_CurrentStock_Display()
    End Sub

    Private Sub DoublingYarn_Receipt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
                ...
                With dgv1
                    ...
                    If keyData = Keys.Enter Or keyData = Keys.Down Then
                        If .CurrentCell.ColumnIndex >= .ColumnCount - 1 Then
                            ...
                            'ElseIf .CurrentCell.ColumnIndex >= .ColumnCount - 6 Then
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If Qa = Windows.Forms.DialogResult.No Or Qa = Windows.Forms.DialogResult.Cancel Then
            Exit Sub
        End If
        NewCode = Trim(EntryType) & "-" & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RecNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        If New_Entry = True Then
            MessageBox.Show("This is New Entry", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If

        Da = New SqlClient.SqlDataAdapter("select count(*) from DoublingYarn_Receipt_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and  DoublingYarn_BillMaking_Code <> ''", con)
        Dt1 = New DataTable
        Da.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
        ...
        Try
            ...
            cmd.CommandText = "Update DoublingYarn_Delivery_Details set Receipt_Bag = a.Receipt_Bag - (b.Receipt_Bag) , Receipt_Cone = a.Receipt_Cone - (b.Receipt_Cone) , Receipt_Weight = a.Receipt_Weight - (b.Receipt_Weight) from DoublingYarn_Delivery_Details a, DoublingYarn_Receipt_Details b Where b.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and a.DoublingYarn_Delivery_Code = b.DoublingYarn_Delivery_Code and a.DoublingYarn_Delivery_SlNo = b.DoublingYarn_Delivery_SlNo"
            cmd.ExecuteNonQuery()


            cmd.CommandText = "delete from DoublingYarn_Receipt_Details where company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()

            cmd.CommandText = "delete from DoublingYarn_Receipt_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'"
            cmd.ExecuteNonQuery()
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 DoublingYarn_Receipt_No from DoublingYarn_Receipt_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code like '" & Trim(EntryType) & "%' and DoublingYarn_Receipt_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby, DoublingYarn_Receipt_No", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 DoublingYarn_Receipt_No from DoublingYarn_Receipt_Head where for_orderby > " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code like '" & Trim(EntryType) & "%' and DoublingYarn_Receipt_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby, DoublingYarn_Receipt_No", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 DoublingYarn_Receipt_No from DoublingYarn_Receipt_Head where for_orderby < " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code like '" & Trim(EntryType) & "%' and DoublingYarn_Receipt_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby desc, DoublingYarn_Receipt_No desc", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        Try
            ...
            Condition = "DoublingYarn_Receipt_Code like '" & Trim(EntryType) & "%'"

            lbl_RecNo.Text = Common_Procedures.get_MaxCode(con, "DoublingYarn_Receipt_Head", "DoublingYarn_Receipt_Code", "For_OrderBy", Condition, Val(lbl_Company.Tag), Common_Procedures.FnYearCode)
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 * from DoublingYarn_Receipt_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby desc, DoublingYarn_Receipt_No desc", con)
            dt1 = New DataTable
            da.Fill(dt1)
            If dt1.Rows.Count > 0 Then
                If Val(Common_Procedures.settings.PreviousEntryDate_ByDefault) = 1 Then '---- M.S Textiles (Tirupur)

                    If dt1.Rows(0).Item("DoublingYarn_Receipt_Date").ToString <> "" Then msk_Date.Text = dt1.Rows(0).Item("DoublingYarn_Receipt_Date").ToString
                End If
            End If
            dt1.Clear()
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Try
            ...
            RecCode = Trim(EntryType) & "-" & Trim(Val(lbl_Company.Tag)) & "-" & Trim(inpno) & "/" & Trim(Common_Procedures.FnYearCode)

            Da = New SqlClient.SqlDataAdapter("select DoublingYarn_Receipt_No from DoublingYarn_Receipt_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code like '" & Trim(EntryType) & "%' and DoublingYarn_Receipt_Code = '" & Trim(RecCode) & "'", con)
            Da.Fill(Dt)
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Dim PBlNo As String = ""
        Dim vTotBgs As Single, vTotCns As Single, vTotAmount As Single
        ...
        Delv_To = Common_Procedures.Ledger_AlaisNameToIdNo(con, Cbo_DelTo.Text)
        If Delv_To = 0 Then
            MessageBox.Show("Invalid Delivery To Name", "DOES NOT SAVE...", MessageBoxButtons.OK, MessageBoxIcon.Error)
            If Cbo_DelTo.Enabled And Cbo_DelTo.Visible Then Cbo_DelTo.Focus()
            Exit Sub
        End If
        ...
        Total_Calculation()
        vTotBgs = 0 : vTotCns = 0 : vTotWeight = 0 : vTotExcessWeight = 0 : vTotAmount = 0
        If dgv_Details_Total.RowCount > 0 Then
            ...
            vTotAmount = Val(dgv_Details_Total.Rows(0).Cells(14).Value())
        End If
        ...
        Try
            ...
            Else
                ...
                lbl_RecNo.Text = Common_Procedures.get_MaxCode(con, "DoublingYarn_Receipt_Head", "DoublingYarn_Receipt_Code", "For_OrderBy", Condition, Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
            ...
            If New_Entry = True Then
                ...
                cmd.CommandText = "Insert into DoublingYarn_Receipt_Head (DoublingYarn_Receipt_Code, Company_IdNo, DoublingYarn_Receipt_No, for_OrderBy, DoublingYarn_Receipt_Date, Ledger_IdNo, Transport_IdNo, Freight_Charges, DelvTo_Idno ,Total_Bag,Total_Cone,  Total_Weight ,Lot_Status , Total_Excess_Weight,Entry_Type , Vehicle_No , GST_Tax_Invoice_Status , Net_Amount ) Values ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RecNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RecNo.Text))) & ", @RecDate, " & Str(Val(Led_ID)) & ",  " & Str(Val(Tr_ID)) & ", " & Str(Val(txt_Frieght.Text)) & ",  " & Val(Delv_To) & "," & Str(Val(vTotBgs)) & "," & Val(vTotCns) & " ,  " & Str(Val(vTotWeight)) & " , " & Val(lotSts) & " , " & Val(vTotExcessWeight) & ", '" & Trim(EntryType) & "','" & Trim(cbo_Vechile.Text) & "'," & Str(Val(vGST_Tax_Inv_Sts)) & " , " & Str(Val(vTotAmount)) & ")"
                cmd.ExecuteNonQuery()

            Else

                cmd.CommandText = "Update DoublingYarn_Receipt_Head set DoublingYarn_Receipt_Date = @RecDate, Ledger_IdNo = " & Val(Led_ID) & ", Transport_IdNo = " & Val(Tr_ID) & ", Freight_Charges = " & Val(txt_Frieght.Text) & ", DelvTo_Idno = " & Val(Delv_To) & " ,Entry_Type = '" & Trim(EntryType) & "' , Total_Bag= " & Val(vTotBgs) & " ,Total_Cone = " & Val(vTotCns) & " ,Total_Excess_Weight = " & Val(vTotExcessWeight) & ",  Total_Weight = " & Val(vTotWeight) & " ,Lot_Status = " & Val(lotSts) & " , Vehicle_No = '" & Trim(cbo_Vechile.Text) & "', GST_Tax_Invoice_Status = " & Str(Val(vGST_Tax_Inv_Sts)) & " , Net_Amount = " & Str(Val(vTotAmount)) & "  Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'"
                cmd.ExecuteNonQuery()

                cmd.CommandText = "Update DoublingYarn_Delivery_Details set Receipt_Bag = a.Receipt_Bag - (b.Receipt_Bag) , Receipt_Cone = a.Receipt_Cone - (b.Receipt_Cone) , Receipt_Weight = a.Receipt_Weight - (b.Receipt_Weight - b.Excess_Short_Weight) from DoublingYarn_Delivery_Details a, DoublingYarn_Receipt_Details b Where b.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and a.DoublingYarn_Delivery_Code = b.DoublingYarn_Delivery_Code and a.DoublingYarn_Delivery_SlNo = b.DoublingYarn_Delivery_SlNo"
                cmd.ExecuteNonQuery()

            End If

            cmd.CommandText = "Delete from DoublingYarn_Receipt_Details Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and DoublingYarn_BillMaking_Code = '' "
            cmd.ExecuteNonQuery()
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Trim(.Rows(i).Cells(2).Value) <> "" Or Val(.Rows(i).Cells(7).Value) <> 0 Then
                        ...
                        Cnt_ID = Common_Procedures.Count_NameToIdNo(con, dgv_Details.Rows(i).Cells(2).Value, tr)

                        Mill_ID = Common_Procedures.Mill_NameToIdNo(con, dgv_Details.Rows(i).Cells(3).Value, tr)
                        ...
                        Nr = 0
                        cmd.CommandText = "Update DoublingYarn_Receipt_Details set DoublingYarn_Receipt_Date = @RecDate , Sl_No = " & Str(Val(Sno)) & " , Dc_Rc_No = '" & Trim(.Rows(i).Cells(1).Value) & "' , Ledger_Idno = " & Val(Led_ID) & ", Count_Idno = " & Str(Val(Cnt_ID)) & ", Mill_Idno = " & Val(Mill_ID) & ", Colour_Idno =  " & Val(Col_ID) & ",Receipt_Bag =  " & Val(.Rows(i).Cells(5).Value) & ",Receipt_Cone = " & Val(.Rows(i).Cells(6).Value) & " , Receipt_Weight =" & Str(Val(.Rows(i).Cells(7).Value)) & " , Excess_Short_Weight =" & Str(Val(.Rows(i).Cells(12).Value)) & "   ,DoublingYarn_Delivery_Code = '" & Trim(.Rows(i).Cells(8).Value) & "', DoublingYarn_Delivery_SlNo = " & Str(Val(.Rows(i).Cells(9).Value)) & " ,  Rate  = " & Str(Val(.Rows(i).Cells(13).Value)) & " , Amount = " & Str(Val(.Rows(i).Cells(14).Value)) & "  where Company_IdNo =  " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and DoublingYarn_Receipt_SlNo = " & Str(Val(dgv_Details.Rows(i).Cells(10).Value)) & ""
                        Nr = cmd.ExecuteNonQuery()

                        If Nr = 0 Then
                            cmd.CommandText = "Insert into DoublingYarn_Receipt_Details(   DoublingYarn_Receipt_Code ,            Company_IdNo           ,    DoublingYarn_Receipt_No  ,                                for_OrderBy,                         DoublingYarn_Receipt_Date,                 Sl_No                       ,                   Dc_Rc_No,                   Ledger_Idno,            Count_Idno,                 Mill_IdNo ,      Colour_Idno        ,       Receipt_Bag,                            Receipt_Cone,                           Receipt_Weight,                         DoublingYarn_Delivery_Code ,                 DoublingYarn_Delivery_SlNo ,           DoublingYarn_Receipt_SlNo ,                     Excess_Short_Weight ,                                   Rate    ,                                    Amount )  " &
                        ...
                        cmd.CommandText = "Insert into Stock_Yarn_Processing_Details ( Reference_Code,             Company_IdNo         ,           Reference_No       ,                               for_OrderBy                             , Reference_Date,     DeliveryTo_Idno     ,                            ReceivedFrom_Idno           , DeliveryToIdno_ForParticulars ,          ReceivedFromIdno_ForParticulars,        Entry_ID     ,       Party_Bill_No  ,       Particulars      ,           Sl_No      ,        Count_Idno       ,  Yarn_Type,    Mill_IdNo          ,                      Bags                  ,                          Cones             , Weight                             ,              Colour_IdNo   ) " &
                                               " Values ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RecNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RecNo.Text))) & ",  @RecDate          ,  " & Val(Delv_To) & "     ," & Str(Val(Led_ID)) & ",  " & Val(Delv_To) & " ," & Str(Val(Led_ID)) & ", '" & Trim(EntID) & "', '" & Trim(PBlNo) & "', '" & Trim(Partcls) & "', " & Str(Val(Sno)) & ", " & Str(Val(Cnt_ID)) & " ,    'MILL'      , " & Str(Val(Mill_ID)) & ",   " & Str(Val(.Rows(i).Cells(5).Value)) & "," & Str(Val(.Rows(i).Cells(6).Value)) & "," & Str(Val(.Rows(i).Cells(7).Value)) & "," & Str(Col_ID) & ") "
                        Nr = cmd.ExecuteNonQuery()
    ...
    Private Sub Total_Calculation()
        Dim vTotBgs As Single, vTotCns As Single, vtotweight As Single, vtotExcessweight As Single, vTotAmount As Single

        Dim i As Integer
        Dim sno As Integer

        vTotBgs = 0 : vTotCns = 0 : vtotweight = 0 : vtotExcessweight = 0 : sno = 0 : vTotAmount = 0
        With dgv_Details
            For i = 0 To dgv_Details.Rows.Count - 1
                ...
                If Val(dgv_Details.Rows(i).Cells(7).Value) <> 0 Then
                    ...
                    vTotAmount = vTotAmount + Val(dgv_Details.Rows(i).Cells(14).Value)
        ...
        dgv_Details_Total.Rows(0).Cells(14).Value = Format(Val(vTotAmount), "#########0.000")
    ...
    Private Sub cbo_Ledger_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Ledger.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Ledger, msk_Date, Cbo_DelTo, "Ledger_AlaisHead", "Ledger_DisplayName", " (Ledger_Type = '' and ( AccountsGroup_IdNo = 10 or AccountsGroup_IdNo = 14) )", "(Ledger_idno = 0)")
    End Sub

    Private Sub cbo_Ledger_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Ledger.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Ledger, Cbo_DelTo, "Ledger_AlaisHead", "Ledger_DisplayName", " (Ledger_Type = '' and ( AccountsGroup_IdNo = 10 or AccountsGroup_IdNo = 14) ) ", "(Ledger_idno = 0)")
    ...
    Private Sub Cbo_DelTo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Cbo_DelTo.GotFocus
    ...
    Private Sub Cbo_DelTo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Cbo_DelTo.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, Cbo_DelTo, cbo_Ledger, cbo_TransportName, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'GODOWN')", "(Ledger_IdNo = 0)")
    End Sub

    Private Sub Cbo_DelTo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Cbo_DelTo.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, Cbo_DelTo, cbo_TransportName, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'GODOWN')", "(Ledger_IdNo = 0)")
    End Sub

    Private Sub Cbo_DelTo_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Cbo_DelTo.KeyUp
        If e.Control = False And e.KeyValue = 17 And vcbo_KeyDwnVal = e.KeyValue Then
            Common_Procedures.MDI_LedType = "GODOWN"
            Dim f As New Ledger_Creation

            Common_Procedures.Master_Return.Form_Name = Me.Name
            Common_Procedures.Master_Return.Control_Name = Cbo_DelTo.Name
            Common_Procedures.Master_Return.Return_Value = ""
            Common_Procedures.Master_Return.Master_Type = ""
    ...
    Private Sub dgv_Details_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgv_Details.CellValueChanged
        ...
        With dgv_Details
            If .Visible Then
                If .CurrentCell.ColumnIndex = 5 Or .CurrentCell.ColumnIndex = 6 Or .CurrentCell.ColumnIndex = 7 Or .CurrentCell.ColumnIndex = 12 Or .CurrentCell.ColumnIndex = 13 Or .CurrentCell.ColumnIndex = 14 Then


                    If Val(.CurrentCell.ColumnIndex) = 7 Or Val(.CurrentCell.ColumnIndex) = 13 Then
                        .Rows(.CurrentCell.RowIndex).Cells(14).Value = Val(dgv_Details.Rows(.CurrentCell.RowIndex).Cells(7).Value) * Val(dgv_Details.Rows(.CurrentCell.RowIndex).Cells(13).Value)
                    End If
    ...
    Private Sub btn_Filter_Show_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Filter_Show.Click
        ...
        Try
            ...
            If IsDate(dtp_Filter_Fromdate.Value) = True And IsDate(dtp_Filter_ToDate.Value) = True Then
                Condt = "a.DoublingYarn_Receipt_Date between '" & Trim(Format(dtp_Filter_Fromdate.Value, "MM/dd/yyyy")) & "' and '" & Trim(Format(dtp_Filter_ToDate.Value, "MM/dd/yyyy")) & "' "
            ElseIf IsDate(dtp_Filter_Fromdate.Value) = True Then
                Condt = "a.DoublingYarn_Receipt_Date = '" & Trim(Format(dtp_Filter_Fromdate.Value, "MM/dd/yyyy")) & "' "
            ElseIf IsDate(dtp_Filter_ToDate.Value) = True Then
                Condt = "a.DoublingYarn_Receipt_Date = '" & Trim(Format(dtp_Filter_ToDate.Value, "MM/dd/yyyy")) & "' "
            End If
            ...
            da = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name,c.*,d.cOUNT_Name,e.Mill_Name from DoublingYarn_Receipt_Head a INNER JOIN Ledger_Head b on a.Ledger_IdNo = b.Ledger_IdNo INNER JOIN DoublingYarn_Receipt_Details c ON c.DoublingYarn_Receipt_Code = a.DoublingYarn_Receipt_Code INNER JOIN Count_Head d ON d.Count_IdNo = c.Count_IdNo LEFT OUTER JOIN Mill_Head e ON c.Mill_Idno = e.Mill_IdNo where a.company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.DoublingYarn_Receipt_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' " & IIf(Trim(Condt) <> "", " and ", "") & Condt & " Order by a.for_orderby, a.DoublingYarn_Receipt_No", con)
            da.Fill(dt2)
            ...
            If dt2.Rows.Count > 0 Then
                ...
                For i = 0 To dt2.Rows.Count - 1
                    ...
                    dgv_Filter_Details.Rows(n).Cells(0).Value = dt2.Rows(i).Item("DoublingYarn_Receipt_No").ToString
                    dgv_Filter_Details.Rows(n).Cells(1).Value = Format(Convert.ToDateTime(dt2.Rows(i).Item("DoublingYarn_Receipt_Date").ToString), "dd-MM-yyyy")
    ...
    Public Sub print_record() Implements Interface_MDIActions.print_record
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select * from DoublingYarn_Receipt_Head Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'", con)
            dt1 = New DataTable
            da1.Fill(dt1)
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.*, c.*,d.Ledger_Name as TransportName from DoublingYarn_Receipt_Head a INNER JOIN Company_Head b ON a.Company_IdNo = b.Company_IdNo INNER JOIN Ledger_Head c ON a.Ledger_IdNo = c.Ledger_IdNo LEFT OUTER JOIN Ledger_Head d ON d.Ledger_IdNo = a.Transport_IdNo where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'", con)
            prn_HdDt = New DataTable
            da1.Fill(prn_HdDt)

            If prn_HdDt.Rows.Count > 0 Then
                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Count_Name, c.Colour_Name ,d.Mill_Name,e.item_gst_percentage, e.Item_HSN_Code  from DoublingYarn_Receipt_Details a LEFT OUTER JOIN Count_Head b on a.Count_Idno = b.Count_Idno LEFT OUTER JOIN Colour_Head c on a.Colour_IdNo = c.Colour_IdNo LEFT OUTER JOIN Mill_Head d ON d.Mill_IdNo = a.Mill_IdNo LEFT OUTER JOIN itemgroup_head e ON b.itemgroup_idno = e.itemgroup_idno  where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' Order by a.sl_no", con)
    ...
    Private Sub Printing_Format1(ByRef e As System.Drawing.Printing.PrintPageEventArgs)
        ...
        ClArr(1) = Val(35) : ClArr(2) = 160 : ClArr(3) = 170 : ClArr(4) = 75 : ClArr(5) = 50 : ClArr(6) = 50 : ClArr(7) = 75 : ClArr(8) = 50 '75
            ...
            If prn_HdDt.Rows.Count > 0 Then
                ...
                If prn_DetDt.Rows.Count > 0 Then
                    ...
                    Do While prn_DetIndx <= prn_DetDt.Rows.Count - 1
                        ...
                        End If
                        Common_Procedures.Print_To_PrintDocument(e, Format(Val(prn_DetDt.Rows(prn_DetIndx).Item("Receipt_Weight").ToString), "########0.000"), LMargin + ClArr(1) + ClArr(2) + ClArr(3) + ClArr(4) + ClArr(5) + ClArr(6) + ClArr(7) - 5, CurY, 1, 0, pFont)
    ...
    Private Sub Printing_Format1_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByVal NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single)
        ...
        If Trim(prn_HdDt.Rows(0).Item("Entry_Type").ToString) = "SPINNING" Then
            vPRINT_HEADING = "SPINNING YARN RECEIPT"
        ElseIf Trim(prn_HdDt.Rows(0).Item("Entry_Type").ToString) = "DOUBLING" Then
            vPRINT_HEADING = "DOUBLING YARN RECEIPT"
        ElseIf Trim(prn_HdDt.Rows(0).Item("Entry_Type").ToString) = "REELING" Then
            vPRINT_HEADING = "REELING YARN RECEIPT"

        End If
        '
        CurY = CurY + TxtHgt - 5
        p1Font = New Font("Calibri", 16, FontStyle.Bold)
        Common_Procedures.Print_To_PrintDocument(e, Trim(vPRINT_HEADING), LMargin, CurY, 2, PrintWidth, p1Font)
        strHeight = e.Graphics.MeasureString(Cmp_Name, p1Font).Height
        ...
        Try
            ...
            Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("DoublingYarn_Receipt_No").ToString, LMargin + C1 + W1 + 30, CurY, 0, 0, p1Font)
            ...
            Common_Procedures.Print_To_PrintDocument(e, ":", LMargin + C1 + W1 + 10, CurY, 0, 0, pFont)
            Common_Procedures.Print_To_PrintDocument(e, Format(Convert.ToDateTime(prn_HdDt.Rows(0).Item("DoublingYarn_Receipt_Date").ToString), "dd-MM-yyyy").ToString, LMargin + C1 + W1 + 30, CurY, 0, 0, pFont)
    ...
    Private Sub cbo_TransportName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_TransportName.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_TransportName, Cbo_DelTo, txt_Frieght, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'TRANSPORT')", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub btn_Selection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Selection.Click
        ...
        With dgv_Selection
            ...
            Da = New SqlClient.SqlDataAdapter("select a.* , b.* ,  e.Ledger_Name as Transportname,h.Receipt_Bag As Ent_Bag,  h.Receipt_Weight As Ent_Wgt,  h.Excess_Short_Weight As Ent_exc, h.Receipt_Cone As Ent_COne, g.Count_Name , I.Mill_Name , k.Colour_Name  from DoublingYarn_Delivery_Head a INNER JOIN DoublingYarn_Delivery_Details b ON a.DoublingYarn_Delivery_Code = b.DoublingYarn_Delivery_Code INNER JOIN Count_Head g ON g.Count_Idno = b.Count_IdNo  LEFT OUTER JOIN Mill_Head i ON b.Mill_IdNo = i.Mill_IdNo  LEFT OUTER JOIN Colour_Head k ON b.Colour_IdNo = k.Colour_IdNo LEFT OUTER JOIN Ledger_Head e ON a.Transport_IdNo = e.Ledger_IdNo LEFT OUTER JOIN DoublingYarn_Receipt_Details h ON h.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' and b.DoublingYarn_Delivery_Code = h.DoublingYarn_Delivery_Code and b.DoublingYarn_Delivery_SlNo = h.DoublingYarn_Delivery_SlNo Where " & CompIDCondt & IIf(Trim(CompIDCondt) <> "", " and ", "") & " A.DoublingYarn_Delivery_Code LIKE  '" & Trim(EntryType) & "%' AND a.Entry_Type =  '" & Trim(EntryType) & "' and a.ledger_Idno = " & Str(Val(LedIdNo)) & " and ((b.Delivery_Weight - b.Receipt_Weight ) > 0 or h.Receipt_Weight > 0 ) order by a.DoublingYarn_Delivery_Date, a.for_orderby, a.DoublingYarn_Delivery_No", con)
            Dt1 = New DataTable
            nr = Da.Fill(Dt1)
            ...
            If Dt1.Rows.Count > 0 Then
                ...
                For i = 0 To Dt1.Rows.Count - 1
                    ...
                    .Rows(n).Cells(1).Value = Dt1.Rows(i).Item("DoublingYarn_Delivery_No").ToString
                    .Rows(n).Cells(2).Value = Format(Convert.ToDateTime(Dt1.Rows(i).Item("DoublingYarn_Delivery_Date").ToString), "dd-MM-yyyy")
                    ...
                    .Rows(n).Cells(11).Value = Dt1.Rows(i).Item("DoublingYarn_Delivery_Code").ToString
                    .Rows(n).Cells(12).Value = Dt1.Rows(i).Item("DoublingYarn_Delivery_SlNo").ToString
    ...
    Private Sub cbo_Vechile_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_Vechile.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Vechile, txt_Frieght, Nothing, "DoublingYarn_Receipt_Head", "Vehicle_No", "", "")
    ...
    Private Sub cbo_Vechile_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cbo_Vechile.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Vechile, Nothing, "DoublingYarn_Receipt_Head", "Vehicle_No", "", "", False)
    ...
    Private Sub cbo_Vechile_GotFocus(sender As Object, e As EventArgs) Handles cbo_Vechile.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "DoublingYarn_Receipt_Head", "Vehicle_No", "", "")
    End Sub
    ...
    Private Sub btn_GENERATEEWB_Click(sender As Object, e As EventArgs) Handles btn_GENERATEEWB.Click
        ...
        Dim NewCode As String = Trim(EntryType) & "-" & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RecNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)
        'Dim NewCode As String = Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RecNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)

        Dim da As New SqlClient.SqlDataAdapter("Select EwayBill_No from DoublingYarn_Receipt_Head where DoublingYarn_Receipt_Code = '" & NewCode & "'", con)
        Dim dt As New DataTable
        ...
        CMD.CommandText = "Insert into EWB_Head ([SupplyType]  ,[SubSupplyType]  , [SubSupplyDesc]  ,[DocType]  ,	[EWBGenDocNo]                           ,[EWBDocDate]        ,[FromGSTIN]       ,[FromTradeName]  ,[FromAddress1]   ,[FromAddress2]     ,[FromPlace] ," &
                         ...
                         " " &
                         " " &
                         "  SELECT               'I'              , '6'             ,   'JOB WORK RETURNS'        ,    'CHL'    , a.DoublingYarn_Receipt_No , a.DoublingYarn_Receipt_Date     , L.Ledger_GSTINNo, L.Ledger_MainName   , L.Ledger_Address1 +  L.Ledger_Address2 , L.Ledger_Address3 + L.Ledger_Address4 , L.City_Town ," &
                         ...
                         " ''        ,''            ,  0        ,     '1'  AS TrMode ," &
                         " a.Vehicle_No, 'R', '" & NewCode & "', (case when a.DeliveryTo_IdNo = 4 or a.DeliveryTo_IdNo = 0 then  c.Company_GSTINNo else tDELV.Ledger_GSTINNo end ) as ShippedTo_GSTIN, tDELV.Ledger_MainName as ShippedTo_LedgerName  from DoublingYarn_Receipt_Head a inner Join Company_Head C on a.Company_IdNo = C.Company_IdNo " &
                         ...
                         " where a.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "'"
        CMD.ExecuteNonQuery()
        ...
        Dim dt1 As New DataTable
        da = New SqlClient.SqlDataAdapter(" Select  I.Count_Name,  IG.ItemGroup_Name  , IG.Item_HSN_Code, IG.Item_GST_Percentage   , sum(SD.Receipt_Weight * SD.Rate) As TaxableAmt,sum(SD.Receipt_Weight) as Qty, 1 , 'WGT' AS Units , tz.Company_State_IdNo , Lh.Ledger_State_Idno  ,a.GST_Tax_Invoice_Status  " &
                                          " from DoublingYarn_Receipt_Details SD Inner Join DoublingYarn_Receipt_Head a On a.DoublingYarn_Receipt_Code = sd.DoublingYarn_Receipt_Code Inner Join Count_Head I On SD.Count_IdNo = I.Count_IdNo Inner Join ItemGroup_Head IG on I.ItemGroup_IdNo = IG.ItemGroup_IdNo " &
                                          " INNER Join Ledger_Head Lh ON Lh.Ledger_Idno = a.Ledger_Idno   INNER JOIN Company_Head tz On tz.Company_Idno = a.Company_Idno  Where SD.DoublingYarn_Receipt_Code = '" & Trim(NewCode) & "' Group By " &
                                          " I.Count_Name,IG.ItemGroup_Name,IG.Item_HSN_Code,IG.ItemGroup_Name ,IG.Item_HSN_Code,IG.Item_GST_Percentage,Lh.Ledger_GSTINNo , tz.Company_State_IdNo , Lh.Ledger_State_Idno ,a.GST_Tax_Invoice_Status  ", con)
        ...
        EWB.GenerateEWB(NewCode, con, rtbEWBResponse, txt_EWBNo, "DoublingYarn_Receipt_Head", "EwayBill_No", "DoublingYarn_Receipt_Code", Pk_Condition)
    End Sub
    ...
    Private Sub btn_CancelEWB_1_Click(sender As Object, e As EventArgs) Handles btn_CancelEWB_1.Click
        ...
        EWB.CancelEWB(txt_EWBNo.Text, NewCode, con, rtbEWBResponse, txt_EWBNo, "DoublingYarn_Receipt_Head", "EwayBill_No", "DoublingYarn_Receipt_Code")
    End Sub
