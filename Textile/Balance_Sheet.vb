Public Class Balance_Sheet
    ...
    Private Sub Balance_Sheet_Calculation()
        ...
        Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date <= @todate and a.ledger_idno = tz.ledger_idno and  a.parent_code NOT LIKE '%~9~4~' and a.parent_code NOT LIKE '%~30~' group by a.parent_code Having sum(tz.voucher_amount) <> 0"
        Cmd.ExecuteNonQuery()
        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date <= @todate and a.ledger_idno = tz.ledger_idno and a.parent_code LIKE '%~30~' group by a.parent_code Having sum(tz.voucher_amount) <> 0 "
        'Nr = Cmd.ExecuteNonQuery()


        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date <= @todate and a.ledger_idno = tz.ledger_idno and a.parent_code NOT LIKE '%~30~' group by a.parent_code Having sum(tz.voucher_amount) <> 0"
        'Cmd.ExecuteNonQuery()
        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date < @fromdate and a.ledger_idno = tz.ledger_idno and a.parent_code LIKE '%~30~' group by a.parent_code Having sum(tz.voucher_amount) <> 0 "
        'Nr = Cmd.ExecuteNonQuery()

        '----CAPITAL ACCOUNTS
        VouAmt = get_VoucherSummary("~2~")
        VouAmt2 = get_VoucherSummary("~30~")
        lbl_CapitalAcc.Text = Common_Procedures.Currency_Format(Val(VouAmt) + Val(VouAmt2))
    ...
    Private Function NetProfitOrLoss_Calculation_111() As Double
        NetProfitOrLoss_Calculation_111 = 0
        'Dim Cmd As New SqlClient.SqlCommand
        'Dim Da1 As New SqlClient.SqlDataAdapter
        'Dim Dt1 As New DataTable
        'Dim condt As String = ""
        ...
        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date <= @todate and a.ledger_idno = tz.ledger_idno group by a.parent_code Having sum(tz.voucher_amount) <> 0"
        'Cmd.ExecuteNonQuery()
        ...
        'NetProfitOrLoss_Calculation_111 = VouAmt
    ...
    Private Sub Profit_AND_LOSS_Calculation()
        ...
        Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " ( name1, currency1 ) Select a.parent_code, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " tz.voucher_date between @companyfromdate and @todate and a.ledger_idno = tz.ledger_idno group by a.parent_code Having sum(tz.voucher_amount) <> 0"
        Cmd.ExecuteNonQuery()

        Cmd.CommandText = "truncate table " & Trim(Common_Procedures.ReportTempSubTable) & ""
        Cmd.ExecuteNonQuery()

        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempSubTable) & " ( date1, currency1 ) Select tZ.voucher_date, sum(tz.voucher_amount) from ledger_head a, voucher_details tz where " & condt & IIf(condt <> "", " and ", "") & " a.parent_code like '%~9~4~%' and tz.voucher_date <= @todate and a.ledger_idno = tz.ledger_idno group by tZ.voucher_date Having sum(tz.voucher_amount) <> 0"
        'Cmd.ExecuteNonQuery()

        Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempSubTable) & " ( date1, currency1 ) Select tZ.Closing_Stock_Value_Date, sum(tz.Closing_Stock_Value) from Closing_Stock_Value_Head tz where " & condt & IIf(condt <> "", " and ", "") & " tz.Closing_Stock_Value_Date <= @todate group by tZ.Closing_Stock_Value_Date "
        'Cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempSubTable) & " ( date1, currency1 ) Select tZ.Closing_Stock_Value_Date, sum(tz.Closing_Stock_Value) from Closing_Stock_Value_Head tz where " & condt & IIf(condt <> "", " and ", "") & " tz.Closing_Stock_Value_Date <= @todate group by tZ.Closing_Stock_Value_Date Having sum(tz.Closing_Stock_Value) <> 0"
        Cmd.ExecuteNonQuery()
    ...
    Private Function Voucher_Summary_ForCurrentAsset() As Double
        ...
        VouAmt = 0
        Da1 = New SqlClient.SqlDataAdapter("Select sum(currency1) from " & Trim(Common_Procedures.ReportTempTable) & " where name1 LIKE '%~4~' and name1 NOT LIKE '%~9~4~'", con)
        Dt1 = New DataTable
        Da1.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
            If IsDBNull(Dt1.Rows(0)(0).ToString) = False Then
        ...
        'Da1 = New SqlClient.SqlDataAdapter("Select sum(currency1) from " & Trim(Common_Procedures.ReportTempTable) & " where name1 LIKE '%~9~4~'", con)
        'Dt1 = New DataTable
        'Da1.Fill(Dt1)
        'If Dt1.Rows.Count > 0 Then
        '    If IsDBNull(Dt1.Rows(0)(0).ToString) = False Then
    ...
    Private Function get_VoucherSummary(ByVal grp_cd As String) As Double
        ...
        '----OPENING STOCK
        VouAmt = 0
        Da1 = New SqlClient.SqlDataAdapter("Select sum(currency1) from " & Trim(Common_Procedures.ReportTempTable) & " where name1 LIKE '%" & Trim(grp_cd) & "'", con)
        Dt1 = New DataTable
        Da1.Fill(Dt1)
        If Dt1.Rows.Count > 0 Then
            If IsDBNull(Dt1.Rows(0)(0).ToString) = False Then
    ...
    Private Sub get_DetailedGridReport()
        ...
        cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " (Name1, Name2 , meters1 , int1 ,Name3 , currency1, currency2) Select a.parent_code ,tG.AccountsGroup_Name, tG.Order_Position, a.ledger_idno, a.ledger_name, (case when sum(tV.voucher_amount) < 0 then sum(tV.voucher_amount) else 0 end), (case when sum(tV.voucher_amount) > 0 then sum(tV.voucher_amount) else 0 end) from ledger_head a, voucher_details tV ,AccountsGroup_Head tG , company_head tz where " & condt & IIf(condt <> "", " and ", "") & " tV.voucher_date <= @todate and a.parent_code NOT LIKE '%~9~4~' and a.parent_code NOT LIKE '%~30~' and a.ledger_idno = tV.ledger_idno and a.AccountsGroup_IdNo = tG.AccountsGroup_IdNo AND tV.Company_IdNo = tz.Company_IdNo group by a.parent_code ,tG.AccountsGroup_Name, tG.Order_Position, a.ledger_idno, a.ledger_name Having sum(tV.voucher_amount) <> 0"
        cmd.ExecuteNonQuery()
        'cmd.CommandText = "insert into " & Trim(Common_Procedures.ReportTempTable) & " (Name1, Name2 , meters1 , int1 ,Name3 , currency1, currency2) Select a.parent_code ,tG.AccountsGroup_Name, tG.Order_Position, a.ledger_idno, a.ledger_name, (case when sum(tV.voucher_amount) < 0 then sum(tV.voucher_amount) else 0 end), (case when sum(tV.voucher_amount) > 0 then sum(tV.voucher_amount) else 0 end) from ledger_head a, voucher_details tV ,AccountsGroup_Head tG , company_head tz where " & condt & IIf(condt <> "", " and ", "") & " tV.voucher_date < @fromdate and a.parent_code LIKE '%~30~' and a.ledger_idno = tV.ledger_idno and a.AccountsGroup_IdNo = tG.AccountsGroup_IdNo AND tV.Company_IdNo = tz.Company_IdNo group by a.parent_code ,tG.AccountsGroup_Name, tG.Order_Position, a.ledger_idno, a.ledger_name Having sum(tV.voucher_amount) <> 0"
        'Nr = cmd.ExecuteNonQuery()
        ...
        With dgv_PrfitAndLossDetails
            ...
            For Cyc = 1 To 11 Step 1
                ...
                '------------Group 
                Da = New SqlClient.SqlDataAdapter("select Name1, name2, sum(currency1) as DR_AMOUNT,sum(currency2) AS CR_AMOUNT from " & Trim(Common_Procedures.ReportTempTable) & " where name1 LIKE '%" & Trim(grp_cd) & "' group by name2, Name1 order by name2, Name1", con)
                Dtbl1 = New DataTable
                Da.Fill(Dtbl1)
                Bal = 0
                If Dtbl1.Rows.Count > 0 Then
                    For i = 0 To Dtbl1.Rows.Count - 1
                        ...
                        Da = New SqlClient.SqlDataAdapter("select  name3, sum(currency1) as DR_AMOUNT,sum(currency2) AS CR_AMOUNT from " & Trim(Common_Procedures.ReportTempTable) & " where name1  = '" & Trim(Dtbl1.Rows(i).Item("name1").ToString) & "' GROUP BY name3", con)
                        Dtbl2 = New DataTable
                        Da.Fill(Dtbl2)
                        Bal = 0
    ...
    Private Sub Print_Details(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal gp_id As String, ByVal cur_x As Decimal, ByRef cur_y As Decimal, ByVal CrDr As Integer)
        ...
        If dt2.Rows.Count <> 0 Then
            ...
            For d = 0 To dt2.Rows.Count - 1
                ...
                da2 = New SqlClient.SqlDataAdapter("Select sum(currency1) as SumOfAmount from " & Trim(Common_Procedures.ReportTempTable) & " where name1 like '%" & Trim(dt2.Rows(d).Item("Parent_Idno").ToString) & "%' having sum(currency1) <> 0", con)
                da2.Fill(dt1)
                If dt1.Rows.Count <> 0 Then
            ...
            For j = 0 To dgv_PrfitAndLossDetails.Columns.Count - 2
                ...
                If Not (dgv_PrfitAndLossDetails.Columns(j).Width < 20 Or dgv_PrfitAndLossDetails.Columns(j).Visible = False) Then
                    If dgv_PrfitAndLossDetails.Rows(i).Cells(0).Style.ForeColor = Color.Blue Or dgv_PrfitAndLossDetails.Rows(i).Cells(0).Style.ForeColor = Color.DarkGreen Or dgv_PrfitAndLossDetails.Rows(i).Cells(0).Style.ForeColor = Color.Red Or dgv_PrfitAndLossDetails.Rows(i).Cells(0).Style.ForeColor = Color.White Then
                        p1Font = New Font("Calibri", 10, FontStyle.Bold)
                        If j = 2 Then
                            Common_Procedures.Print_To_PrintDocument(e, Trim(dgv_PrfitAndLossDetails.Rows(i).Cells(j).Value), LMargin + ClArr(j) + 180 - 10, CurY, 1, 0, p1Font)
                        Else
                            Common_Procedures.Print_To_PrintDocument(e, Trim(dgv_PrfitAndLossDetails.Rows(i).Cells(j).Value), LMargin + ClArr(j) + 150 - 10, CurY, 1, 0, p1Font)
                        End If
