Public Class Basket_Creation
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select a.* from Basket_Head a where a.Basket_IdNo = " & Str(Val(idno)), con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("Basket_IdNo").ToString
            txt_Name.Text = dt.Rows(0).Item("Basket_Name").ToString
            txt_Weight.Text = Format(Val(dt.Rows(0).Item("Weight").ToString), "########0.000")
        End If
    ...
    Private Sub Basket_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Basket_Creation_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        con.Close()
        con.Dispose()
    End Sub

    Private Sub Basket_Creation_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from Basket_Head where Basket_IdNo = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("select Basket_IdNo, Basket_Name,Weight from Basket_Head where Basket_IdNo <> 0 order by Basket_IdNo", con)
        Dim dt As New DataTable
        ...
        With dgv_Filter
            ...
            .Columns(1).HeaderText = "BASKET NAME"
            .Columns(2).HeaderText = "WEIGHT"
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Basket_IdNo) from Basket_Head Where Basket_IdNo <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Basket_IdNo) from Basket_Head Where Basket_IdNo <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Basket_IdNo) from Basket_Head Where Basket_IdNo > " & Str(Val(lbl_IdNo.Text)) & " and Basket_IdNo <> 0", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Basket_IdNo) from Basket_Head Where Basket_IdNo < " & Str(Val(lbl_IdNo.Text)) & " and Basket_IdNo <> 0", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Basket_Head", "Basket_IdNo", "")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("select Basket_Name from Basket_Head order by Basket_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Open.DataSource = dt
        cbo_Open.DisplayMember = "Basket_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Basket_Head", "Basket_IdNo", "", trans)

                cmd.CommandText = "Insert into Basket_Head(Basket_IdNo, Basket_Name, sur_name,Weight) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "'," & Val(txt_Weight.Text) & ")"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Basket_Head set Basket_Name = '" & Trim(txt_Name.Text) & "', sur_name = '" & Trim(Sur) & "',Weight = " & Val(txt_Weight.Text) & " where Basket_IdNo = " & Str(Val(lbl_IdNo.Text)) & ""
                cmd.ExecuteNonQuery()
            ...
            Common_Procedures.Master_Return.Master_Type = "BASKET"
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_basket_head") > 0 Then
                MessageBox.Show("Duplicate Basket Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btn_Open_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Open.Click
        ...
        da = New SqlClient.SqlDataAdapter("select Basket_IdNo from Basket_Head where Basket_Name = '" & Trim(cbo_Open.Text) & "'", con)
        da.Fill(dt)
    ...
    Private Sub cbo_Open_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Open.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Open, Nothing, Nothing, "Basket_Head", "Basket_Name", "", "(Basket_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_Open_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Open.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Open, Nothing, "Basket_Head", "Basket_Name", "", "(Basket_IdNo = 0)")
