Public Class ItemGroup_Creation
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select a.* from itemgroup_head a  where a.itemgroup_idno = " & Str(Val(idno)), con)
        dt = New DataTable
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0)("itemgroup_idno").ToString
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select count(*) from Cloth_Head where ItemGroup_IdNo = " & Str(Val(lbl_IdNo.Text)), con)
            dt = New DataTable
            da.Fill(dt)
            If dt.Rows.Count > 0 Then
            ...
            da = New SqlClient.SqlDataAdapter("select count(*) from Sales_Details where ItemGroup_IdNo = " & Str(Val(lbl_IdNo.Text)), con)
            dt = New DataTable
            da.Fill(dt)
            If dt.Rows.Count > 0 Then
                If IsDBNull(dt.Rows(0)(0).ToString) = False Then
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from itemgroup_head where itemgroup_idno = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            cmd.Connection = con
            cmd.CommandText = "select min(itemgroup_idno) from itemgroup_head where itemgroup_idno > " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(itemgroup_idno) from itemgroup_head where itemgroup_idno < " & Str(Val(lbl_IdNo.Text)) & " and itemgroup_idno <> 0 ", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "ItemGroup_Head", "itemgroup_idno", "")
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        If Trim(txt_HSN_Code.Text) = "" Then
            MessageBox.Show("Invalid HSN code", "DOES NOT SAVE", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
            txt_HSN_Code.Focus()
            Exit Sub
        End If
        ...
        If Common_Procedures.Check_Duplicate_Item_Group_Name(con, Val(lbl_IdNo.Text), Sur) = True Then
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "ItemGroup_Head", "itemgroup_idno", "", trans)

                cmd.CommandText = "Insert into ItemGroup_Head(  itemgroup_idno            ,         itemgroup_name      ,       Item_HSN_Code               ,       sur_name             , Item_GST_Percentage ) " & _
                                                    "values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "','" & Trim(txt_HSN_Code.Text) & "', '" & Trim(Sur) & "' ," & Str(Val(txt_GST_Percentage.Text)) & ")"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update itemgroup_head set itemgroup_name = '" & Trim(txt_Name.Text) & "', Item_HSN_Code = '" & Trim(txt_HSN_Code.Text) & "', sur_name = '" & Trim(Sur) & "' ,Item_GST_Percentage=" & Str(Val(txt_GST_Percentage.Text)) & " where itemgroup_idno = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
