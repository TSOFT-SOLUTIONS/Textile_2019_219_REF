...
Public Class Transfer_Master_Ledgers_From_Tsoft_Billing
    ...
    Private Sub Transfer_Master_Ledgers_From_Smart_TextileNT9_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(cbo_DBFrom, CnTo, "master..sysdatabases", "name", "(name LIKE 'tsoft%billing%')", "")
    ...
    Private Sub btn_Transfer_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Transfer.Click
        ...
        Try
            ...
            If Common_Procedures.settings.CustomerCode = "1186" Then
                ...
                DbFrmName = Trim(cbo_DBFrom.Text)

                If chk_MasterTransfer.Checked = True Then
                ...
                If chk_EntriesTransfer.Checked = True Then


                    Transfer_Table(tr, DbFrmName, "Item_ExcessShort_Head")

                    Transfer_Table(tr, DbFrmName, "Purchase_Head")
                    Transfer_Table(tr, DbFrmName, "Purchase_Details")

                    Transfer_Table(tr, DbFrmName, "Purchase_return_Head")
                    Transfer_Table(tr, DbFrmName, "Purchase_return_Details")

                    Transfer_Table(tr, DbFrmName, "Sales_Head")
                    Transfer_Table(tr, DbFrmName, "Sales_Details")

                    Transfer_Table(tr, DbFrmName, "SalesReturn_Head")
                    Transfer_Table(tr, DbFrmName, "SalesReturn_Details")

                    Transfer_Table(tr, DbFrmName, "Voucher_Head")
                    Transfer_Table(tr, DbFrmName, "Voucher_Details")

                    Transfer_Table(tr, DbFrmName, "Voucher_Bill_Head")
                    Transfer_Table(tr, DbFrmName, "Voucher_Bill_details")
    ...
    Private Sub AccountsGroupHead_Transfer(ByVal sqltr As SqlClient.SqlTransaction)
        ...
        Da1 = New SqlClient.SqlDataAdapter("select * from AccountsGroup_Head where AccountsGroup_IdNo > 30 Order by AccountsGroup_IdNo", CnFrm)
        Dt1 = New DataTable
        Da1.Fill(Dt1)
    ...
    Private Sub LedgerHead_Transfer(ByVal sqltr As SqlClient.SqlTransaction)
        ...
        Da1 = New SqlClient.SqlDataAdapter("select a.*, b.state_name from Ledger_Head a left outer join state_head b ON b.state_idno = a.State_IdNo where Ledger_IdNo > 20 Order by Ledger_IdNo", CnFrm)
        Dt1 = New DataTable
        Da1.Fill(Dt1)
        ...
        If Dt1.Rows.Count > 0 Then
            ...
            For I = 0 To Dt1.Rows.Count - 1
                ...
                If vOldLID = 0 Then
                    ...
                    If Trim(UCase(Dt1.Rows(I).Item("Bill_Type").ToString)) = "BILL TO BILL CLEAR" Then
                    ...
                    vLedger_Emailid = Replace(Dt1.Rows(I).Item("Ledger_EmailID").ToString, "'", "")
                    ...
                    vLedger_AgentIdNo = Val(Dt1.Rows(I).Item("Agent_IdNo").ToString)

                    '  vNote = Replace(Dt1.Rows(I).Item("Note1").ToString, "'", "")

                    vMobileNo_Sms = Dt1.Rows(I).Item("Ledger_PhoneNo").ToString
                    vOwner_Name = Replace(Dt1.Rows(I).Item("Owner_Name").ToString, "'", "")

                    vLedger_GsTinNo = Replace(Dt1.Rows(I).Item("Ledger_GSTinNo").ToString, "'", "")
                    vLedgerGroup_Idno = Val(Dt1.Rows(I).Item("LedgerGroup_Idno").ToString)
                    vLedgerState_Idno = Common_Procedures.State_NameToIdNo(CnTo, Dt1.Rows(I).Item("state_name").ToString, sqltr)


                    'Val(Dt1.Rows(I).Item("State_Idno").ToString)
            ...
            Da1 = New SqlClient.SqlDataAdapter("select * from Ledger_Head where Ledger_IdNo <> 936 ", CnTo)
