Public Class PayRoll_Employee_Attendance_Simple
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.* from PayRoll_Employee_Attendance_Head a Where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Employee_Attendance_Code = '" & Trim(NewCode) & "'", con)
            dt1 = New DataTable
            da1.Fill(dt1)
                    ...
                    If dt2.Rows.Count > 0 Then
                        ...
                        For i = 0 To dt2.Rows.Count - 1
                            ...
                            If Val(dt2.Rows(i).Item("Incentive_Amount").ToString) <> 0 Then
                                .Rows(n).Cells(5).Value = Format(Val(dt2.Rows(i).Item("Incentive_Amount").ToString), "########0.00")
                            End If
    ...
    Private Sub get_EmployeeList()
        ...
        Cmd.CommandText = "select a.*, b.Category_Name from PayRoll_Employee_Head a LEFT OUTER JOIN PayRoll_Category_Head b ON a.Category_IdNo = b.Category_IdNo where a.Join_DateTime <= @AttDate and (a.Date_Status = 0 or (a.Date_Status = 1 and a.Releave_DateTime >= @AttDate ) ) Order by Employee_Name"
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_ATTENDANCE, New_Entry, Me, con, "PayRoll_Employee_Attendance_Head", "Employee_Attendance_Code", NewCode, "Employee_Attendance_Date", "(Employee_Attendance_Code = '" & Trim(NewCode) & "')") = False Then Exit Sub
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.InsertingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_ATTENDANCE, New_Entry, Me) = False Then Exit Sub
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_ADVANCE, New_Entry, Me, con, "PayRoll_Employee_Attendance_Head", "Employee_Attendance_Code", NewCode, "Employee_Attendance_Date", "(Employee_Attendance_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Employee_Attendance_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Employee_Attendance_No desc", dtp_Date.Value.Date) = False Then Exit Sub
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(Emp_ID) <> 0 Then
                        ...
                        cmd.CommandText = "Insert into PayRoll_Employee_Attendance_Details ( Employee_Attendance_Code ,               Company_IdNo       ,     Employee_Attendance_No    ,                               for_OrderBy                              , Employee_Attendance_Date,             Sl_No     ,        Employee_IdNo    ,                      No_Of_Shift         ,                      OT_Hours            ,           OT_Minutes     ,                      Incentive_Amount  , Mess_Attendance     ) " & _
                                          "            Values                      (   '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ",      @EntryDate         ,  " & Str(Val(Sno)) & ", " & Str(Val(Emp_ID)) & ", " & Str(Val(.Rows(i).Cells(3).Value)) & ", " & Str(Val(.Rows(i).Cells(4).Value)) & ", " & Str(Val(OT_Mins)) & ", " & Str(Val(.Rows(i).Cells(5).Value)) & " ,  " & Str(Val(.Rows(i).Cells(6).Value)) & " ) "
                        cmd.ExecuteNonQuery()
