Public Class LotNo_creation
    ...
    Private Sub clear()
        ...
        cbo_FnYrCode.Text = Common_Procedures.FnYearCode
        New_Entry = False
    End Sub
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.LotNo_Creation, "~L~") = 0 And InStr(Common_Procedures.UR.LotNo_Creation, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.FP_LotNo_Creation, New_Entry, Me) = False Then Exit Sub
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        ...
        Dim da As New SqlClient.SqlDataAdapter("select Lot_IdNo, Lot_No,Lot_Description from Lot_Head where Lot_IdNo <> 0 and (Auto_Created is Null or Auto_Created = 0) order by Lot_IdNo", con)
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        If dt.Rows.Count > 0 Then
            ...
            txt_LotNo.Text = dt.Rows(0).Item("Lot_Main_Name").ToString
            txt_Description.Text = dt.Rows(0).Item("Lot_Description").ToString
            cbo_FnYrCode.Text = dt.Rows(0).Item("Lot_Fn_Yr_Code").ToString
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Lot_idno) from Lot_head Where Lot_idno <> 0 and (Auto_Created is null or Auto_Created =0)", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Lot_idno) from Lot_head Where Lot_idno <> 0  and (Auto_Created is null or Auto_Created =0)", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Lot_idno) from lot_head Where Lot_idno > " & Str(Val(lbl_IdNo.Text)) & " and Lot_idno <> 0  and (Auto_Created is null or Auto_Created =0)", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Lot_idno) from Lot_head Where Lot_idno < " & Str(Val(lbl_IdNo.Text)) & " and Lot_idno <> 0  and (Auto_Created is null or Auto_Created =0)", con)
            da.Fill(dt)
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Dim da As New SqlClient.SqlDataAdapter("select Lot_No from Lot_Head Where (Auto Created Is Null or Auto_Created = 0) order by Lot_No", con)
        Dim dt As New DataTable
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Dim cmd As New SqlClient.SqlCommand
        Dim Sur As String
        Dim Full_Lot_No As String = ""

        ' If Common_Procedures.UserRight_Check(Common_Procedures.UR.LotNo_Creation, New_Entry) = False Then Exit Sub

        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.FP_LotNo_Creation, New_Entry, Me) = False Then Exit Sub
        ...
        Full_Lot_No = Trim(txt_LotNo.Text) + IIf(Len(Trim(cbo_FnYrCode.Text)) > 0, "/" & cbo_FnYrCode.Text, "")

        Sur = Common_Procedures.Remove_NonCharacters(Trim(Full_Lot_No))
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                cmd.CommandText = "Insert into Lot_Head(Lot_IdNo, Lot_No, Sur_Name,Lot_Description,Lot_Main_Name,Lot_Fn_Yr_Code,Auto_Created) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(Full_Lot_No) & "', '" & Trim(Sur) & "','" & Trim(txt_Description.Text) & "','" & txt_LotNo.Text & "','" & cbo_FnYrCode.Text & "',0)"
                cmd.ExecuteNonQuery()

            Else

                cmd.CommandText = "update Lot_Head Set Lot_No = '" & Trim(Full_Lot_No) & "', Sur_Name = '" & Trim(Sur) & "',Lot_Description='" & Trim(Full_Lot_No) & "',Lot_Main_Name = '" & txt_LotNo.Text & "', Lot_Fn_Yr_Code = '" & cbo_FnYrCode.Text & "' where Lot_IdNo = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
