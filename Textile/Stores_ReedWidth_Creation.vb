Public Class Stores_ReedWidth_Creation
    ...
    Private Sub ReedWidth_Creation_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        con.Close()
        con.Dispose()
    End Sub

    Private Sub ReedWidth_Creation_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub ReedWidth_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Master_Stores_ReedWidth_Creation, "~L~") = 0 And InStr(Common_Procedures.UR.Master_Stores_ReedWidth_Creation, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from ReedWidth_Head where ReedWidth_Idno = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("select ReedWidth_Idno, ReedWidth_Name from ReedWidth_Head where ReedWidth_Idno <> 0 order by ReedWidth_Idno", con)
        ...
        With dgv_filter
            ...
            .Columns(1).HeaderText = "REED WIDTH NAME"
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select * from ReedWidth_Head a where ReedWidth_Idno = " & Str(Val(idno)), con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("ReedWidth_Idno").ToString
            txt_Name.Text = dt.Rows(0).Item("ReedWidth_Name").ToString
        End If
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(ReedWidth_Idno) from ReedWidth_Head Where ReedWidth_Idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(ReedWidth_Idno) from ReedWidth_Head Where ReedWidth_Idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(ReedWidth_Idno) from ReedWidth_Head Where ReedWidth_Idno > " & Str(Val(lbl_IdNo.Text)) & " and ReedWidth_Idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(ReedWidth_Idno) from ReedWidth_Head Where ReedWidth_Idno < " & Str(Val(lbl_IdNo.Text)) & " and ReedWidth_Idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "ReedWidth_Head", "ReedWidth_Idno", "")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("select ReedWidth_Name from ReedWidth_Head order by ReedWidth_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Find.DataSource = dt
        cbo_Find.DisplayMember = "ReedWidth_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        'If Common_Procedures.UserRight_Check(Common_Procedures.UR.Master_Stores_ReedWidth_Creation, New_Entry) = False Then Exit Sub
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "ReedWidth_Head", "ReedWidth_Idno", "", trans)

                cmd.CommandText = "Insert into ReedWidth_Head(ReedWidth_Idno, ReedWidth_Name, Sur_Name) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "')"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update ReedWidth_Head set ReedWidth_Name = '" & Trim(txt_Name.Text) & "', Sur_Name = '" & Trim(Sur) & "' where ReedWidth_Idno = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
            ...
            Common_Procedures.Master_Return.Master_Type = "REEDWIDTH"
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_ReedWidth_Head") > 0 Then
                MessageBox.Show("Duplicate Reed Width Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btn_Open_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Open.Click
        ...
        If Trim(cbo_Find.Text) = "" Then
            MessageBox.Show("Invalid Reed Width", "DOES NOT FIND...", MessageBoxButtons.OK, MessageBoxIcon.Error)
            If cbo_Find.Enabled Then cbo_Find.Focus()
            Exit Sub
        End If

        da = New SqlClient.SqlDataAdapter("select ReedWidth_Idno from ReedWidth_Head where ReedWidth_Name = '" & Trim(cbo_Find.Text) & "'", con)
        da.Fill(dt)
                ...
                If Asc(e.KeyChar) <> 27 Then
                    ...
                    Else
                        ...
                        If Trim(FindStr) <> "" Then
                            Condt = " Where ReedWidth_Name like '" & Trim(FindStr) & "%' or ReedWidth_Name like '% " & Trim(FindStr) & "%' "
                        End If

                        da = New SqlClient.SqlDataAdapter("select ReedWidth_Name from ReedWidth_Head " & Condt & " order by ReedWidth_Name", con)
                        da.Fill(dt)

                        .DataSource = dt
                        .DisplayMember = "ReedWidth_Name"
