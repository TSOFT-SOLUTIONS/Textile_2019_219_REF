Public Class Billing_SalesReturn_Entry_GST
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                cbo_EntType.Text = dt1.Rows(0).Item("Return_type").ToString
    ...
    Private Sub SalesReturn_Entry_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        AddHandler cbo_PhoneNo.GotFocus, AddressOf ControlGotFocus
        AddHandler cbo_PhoneNo.LostFocus, AddressOf ControlLostFocus

        'AddHandler cbo_EntType.KeyDown, AddressOf TextBoxControlKeyDown
        'AddHandler cbo_EntType.KeyPress, AddressOf TextBoxControlKeyPress
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Sales_Return_Entry, "~L~") = 0 And InStr(Common_Procedures.UR.Sales_Return_Entry, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        ' If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Sales_Return_Entry, "~L~") = 0 And InStr(Common_Procedures.UR.Sales_Return_Entry, "~I~") = 0 Then MessageBox.Show("You have No Rights to Insert", "DOES NOT INSERT...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        ...
        With dgv_Details
            ...
            For i = 0 To .RowCount - 1
                ...
                If Val(.Rows(i).Cells(4).Value) <> 0 Then
                    ...
                    'If cbo_EntType.Text <> "DIRECT" Then
                    '    Da = New SqlClient.SqlDataAdapter("select isnull(noof_items,0)-isnull(return_qty,0) from Sales_Head a inner join Sales_Details b on a.Sales_Code = b.Sales_Code where b.Sales_Code='" & Trim(.Rows(i).Cells(16).Value) & "' and b.Sales_Detail_SlNo=" & Str(.Rows(i).Cells(15).Value) & "", con)

                    '    Da.Fill(Dt)
                    '    If (Dt.Rows.Count > 0) Then
                    '        checkQty = Val(Dt.Rows(0)(0))
                    '    End If

                    '    If Val(.Rows(i).Cells(4).Value) > checkQty Then
                ...
                For i = 0 To .Rows.Count - 1
                    ...
                    If Val(.Rows(i).Cells(4).Value) <> 0 Then
                        ...
                        If itm_id <> 0 Then
                            ...
                            cmd.CommandText = "Insert into Item_Processing_Details (    Reference_Code    ,             Company_IdNo         ,            Reference_No           ,          for_OrderBy       , Reference_Date,          Ledger_IdNo    ,           Party_Bill_No        ,            SL_No     ,          Item_IdNo      ,           Unit_IdNo     ,                    Quantity         ,sales_rate      ) " & _
                                                    " Values ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_ReturnNo.Text) & "', " & Str(Val(vforOrdby)) & ",    @SalesDate , " & Str(Val(led_id)) & ", '" & Trim(txt_BillNo.Text) & "', " & Str(Val(Sno)) & ", " & Str(Val(itm_id)) & ", " & Str(Val(unt_id)) & ", " & Str(Val(.Rows(i).Cells(4).Value)) & "," & Str(Val(.Rows(i).Cells(5).Value)) & " )"
                            cmd.ExecuteNonQuery()

                            If cbo_EntType.Text <> "DIRECT" Then
                                Nr = 0

                                If cbo_PaymentMethod.Text = "CASH" Then
                                    cmd.CommandText = "Update Sales_Details set Return_Qty = Return_Qty + (" & Val(.Rows(i).Cells(4).Value) & ") Where Sales_Code = '" & Trim(.Rows(i).Cells(16).Value) & "' and Sales_Detail_SlNo = " & Str(Val(.Rows(i).Cells(15).Value)) & ""
                                Else
    ...
    Private Sub cbo_PaymentMethod_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PaymentMethod.KeyDown
        Try
            ...
            Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PaymentMethod, dtp_Date, cbo_PhoneNo, "", "", "", "")
    ...
    Private Sub cbo_PaymentMethod_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PaymentMethod.KeyPress
        Try
            Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PaymentMethod, cbo_PhoneNo, "", "", "", "", True)
    ...
    Public Sub PrtyName_TO_PhnNo()
        ...
        da1 = New SqlClient.SqlDataAdapter("select lh.Ledger_PhoneNo from Ledger_Head lh  Where LH.Ledger_IdNo  = " & Trim(led_id) & " ", con)
        dt1 = New DataTable
        da1.Fill(dt1)

        If dt1.Rows.Count > 0 Then
            cbo_PhoneNo.Text = dt1.Rows(0).Item("Ledger_PhoneNo").ToString
        Else
            cbo_Ledger.Focus()
        End If
    End Sub
...
LOOP1:
    ...
    Private Sub btn_Selection_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Selection.Click
        ...
        If cbo_PaymentMethod.Text <> "CASH" Then
            ...
            If LedIdNo = 0 Then
                ...
                Exit Sub
            End If
            SQL = "select a.*,b.*,c.*,IG.* ,e.*,d.noof_items entqty from Sales_Details a " &
                " INNER JOIN Sales_Head b ON a.Sales_Code = b.Sales_Code " & _
                 " LEFT OUTER JOIN salesreturn_Details d ON D.salesreturn_Code = '" & Trim(NewCode) & "'" & _
                        " and  a.sales_Code = d.sales_Code and  a.sales_detail_Slno = d.sales_detail_Slno  " & _
                " INNER JOIN Item_Head c ON a.Item_idno = c.Item_idno  " & _
                " LEFT OUTER JOIN ItemGroup_Head IG ON c.ItemGroup_IdNo = ig.ItemGroup_IdNo   " & _
                " LEFT OUTER JOIN Unit_Head e ON a.Unit_IdNo = e.Unit_IdNo Where a.ledger_idno = " & Str(Val(LedIdNo)) & _
                "  and (a.noof_items - a.return_qty) > 0 or d.Noof_Items > 0 Order by a.For_OrderBy, a.Sales_No, a.Sales_Detail_SlNo"
        Else

            SQL = " select a.*,b.*,c.*,IG.* ,e.*,d.noof_items entqty  from Sales_Details a  " & _
                " INNER JOIN Sales_Head b ON a.Sales_Code = b.Sales_Code  " & _
                 " LEFT OUTER JOIN salesreturn_Details d ON D.salesreturn_Code = '" & Trim(NewCode) & "'" & _
                        " and  a.sales_Code = d.sales_Code and  a.sales_detail_Slno = d.sales_detail_Slno  " & _
                " INNER JOIN Item_Head c ON a.Item_idno = c.Item_idno  " & _
                " LEFT OUTER JOIN ItemGroup_Head IG ON c.ItemGroup_IdNo = ig.ItemGroup_IdNo   " & _
                " LEFT OUTER JOIN Unit_Head e ON a.Unit_IdNo = e.Unit_IdNo Where b.Cash_PartyName = '" & cbo_Ledger.Text & _
                "'  and (a.noof_items - a.return_qty) > 0 or d.Noof_Items > 0  Order by a.For_OrderBy, a.Sales_No, a.Sales_Detail_SlNo"
        End If
        ...
        With dgv_Selection
            ...
            If Dt1.Rows.Count > 0 Then
                ...
                For i = 0 To Dt1.Rows.Count - 1
                    ...
                    If IsDBNull(Dt1.Rows(i).Item("entqty").ToString) = False Then
                        Ent_Qty = Val(Dt1.Rows(i).Item("entqty").ToString)
                    End If
                    ...
                    .Rows(n).Cells(5).Value = (Val(Dt1.Rows(i).Item("Noof_Items").ToString) - Val(Dt1.Rows(i).Item("Return_Qty").ToString)) + Val(Ent_Qty)
                    ...
                    .Rows(n).Cells("return_qty").Value = Val(Dt1.Rows(i).Item("entqty").ToString)
    ...
    Private Sub btn_Close_Selection_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Close_Selection.Click
        ...
        For i = 0 To dgv_Selection.RowCount - 1
            ...
            If Val(dgv_Selection.Rows(i).Cells(8).Value) = 1 Then
                ...
                dgv_Details.Rows(n).Cells(4).Value = Val(dgv_Selection.Rows(i).Cells(5).Value)
    ...
    '***** GST START *****
    Private Sub get_Item_Details(ByVal vItmName As String, ByVal vItmCode As String)
        Dim da As SqlClient.SqlDataAdapter
        Dim dt As New DataTable
        Dim vItemID As Integer = 0
        Dim vDetFrom_Name_STS As Boolean = False
        Dim vDetFrom_Code_STS As Boolean = False

        If Trim(vItmCode) = "" And Trim(vItmName) = "" Then Exit Sub

        vItemID = 0
        vDetFrom_Name_STS = False
        vDetFrom_Code_STS = False
        vItemID = Val(Common_Procedures.Item_NameToIdNo(con, Trim(vItmName)))

        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1167" Then
            da = New SqlClient.SqlDataAdapter("select a.*,IH.Item_Name, b.unit_name from Item_Processing_Details a Inner Join Item_Head IH ON a.Item_IdNo = IH.Item_IdNo LEFT OUTER JOIN unit_head b ON ih.unit_idno = b.unit_idno where Reference_Code LIKE 'DCBGE-%' and a.Barcode =  '" & Trim(cbo_ItemCode.Text) & "'", con)
            dt = New DataTable
            da.Fill(dt)

            If dt.Rows.Count > 0 Then
                'If IsDBNull(dt.Rows(0)("barcode").ToString) = False Then
                '    cbo_ItemName.Text = dt.Rows(0)("barcode").ToString
                'End If
                ...
                If IsDBNull(dt.Rows(0)("Sales_Rate").ToString) = False Then
                    txt_Rate.Text = dt.Rows(0)("Sales_Rate").ToString
                End If
                If (Trim(Common_Procedures.settings.CustomerCode)) = "1167" Then
    ...
    Private Sub cbo_PaymentMethod_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbo_PaymentMethod.TextChanged
    ...
    Private Sub cbo_PhoneNo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_PhoneNo.GotFocus
        Try
            Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Sales_Head", "Cash_PartyPhoneNo", "(Cash_PartyPhoneNo<>'')", "(Sales_Code LIKE '%/00-00')")
        Catch ex As Exception
            'MessageBox.Show(ex.Message, "DOES NOT SELECT...", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub cbo_PhoneNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PhoneNo.KeyDown
        Try
            Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PhoneNo, cbo_PaymentMethod, Nothing, "Sales_Head", "Cash_PartyPhoneNo", "(Cash_PartyPhoneNo<>'')", "(Sales_Code LIKE '%/00-00')")
            If e.KeyCode = 40 And cbo_PhoneNo.DroppedDown = False Or (e.Control = True And e.KeyCode = 40) Then

                If cbo_PhoneNo.Text = "" Then
                    cbo_Ledger.Focus()
                Else
                    partyPhne_To_PartyName()
                    If cbo_EntType.Text = "RETURN" Then
                        If MessageBox.Show("Do you want to select Order :", "FOR ORDER SELECTION...", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) = DialogResult.Yes Then
                            btn_Selection_Click(sender, e)
                        End If
                    Else
    ...
    Private Sub cbo_PhoneNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PhoneNo.KeyPress
        Try
            Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PhoneNo, Nothing, "Sales_Head", "Cash_PartyPhoneNo", "(Cash_PartyPhoneNo<>'')", "(Sales_Code LIKE '%/00-00')", False)
            If Asc(e.KeyChar) = 13 Then
                If cbo_PhoneNo.Text = "" Then
                    cbo_Ledger.Focus()
                Else
    ...
    Public Sub partyPhne_To_PartyName()
        Dim da1 As SqlClient.SqlDataAdapter
        Dim dt1 As New DataTable
        'Dim Phn_No As Integer
        'If Trim(cbo_PaymentMethod.Text) <> "CASH" Then Exit Sub

        da1 = New SqlClient.SqlDataAdapter("select sh.Cash_PartyName from Sales_Head sh  Where sh.Cash_PartyPhoneNo  = '" & Trim(cbo_PhoneNo.Text) & "' ", con)
        dt1 = New DataTable
        da1.Fill(dt1)
