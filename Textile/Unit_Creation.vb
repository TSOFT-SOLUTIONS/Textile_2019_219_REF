Public Class Unit_Creation
    ...
    Private Sub Unit_Creation_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        ...
        End If
    End Sub
    Private Sub Unit_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Width = 475
        grp_Find.Left = 6
        grp_Find.Top = 250
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        'If Val(Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Master_Unit_Creation, "~L~") = 0 And InStr(Common_Procedures.UR.Master_Unit_Creation, "~D~") = 0 Then MessageBox.Show("You have No Rights to Delete", "DOES NOT DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.Unit_Creation, New_Entry, Me) = False Then Exit Sub
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from Unit_Head where Unit_IdNo = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("select Unit_IdNo ,Unit_Name from Unit_Head where Unit_IdNo <> 0 order by Unit_IdNo", con)
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("select * from Unit_head a where Unit_idno = " & Str(Val(idno)), con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("Unit_IdNo").ToString
            txt_Name.Text = dt.Rows(0).Item("Unit_Name").ToString
        End If
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Unit_idno) from Unit_Head Where Unit_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select min(Unit_idno) from Unit_Head Where Unit_idno > " & Str(Val(lbl_IdNo.Text)) & " and Unit_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select max(Unit_idno) from Unit_Head Where Unit_idno < " & Str(Val(lbl_IdNo.Text)) & " and Unit_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Unit_Head", "Unit_idno", "")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("select Unit_Name from Unit_Head order by Unit_Name", con)
        Dim dt As New DataTable
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        'If Common_Procedures.UserRight_Check(Common_Procedures.UR.Master_Unit_Creation, New_Entry) = False Then Exit Sub
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.Unit_Creation, New_Entry, Me) = False Then Exit Sub
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Unit_Head", "Unit_idno", "", trans)

                cmd.CommandText = "Insert into Unit_Head(Unit_idno, Unit_Name, Sur_Name) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "')"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Unit_Head set Unit_Name = '" & Trim(txt_Name.Text) & "', Sur_Name = '" & Trim(Sur) & "' where Unit_idno = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_Unit_Head") > 0 Then
                MessageBox.Show("Duplicate Unit Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub cbo_Find_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Find.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Find, Nothing, Nothing, "Unit_Head", "Unit_Name", "", "")
    End Sub
    ...
    Private Sub cbo_Find_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Find.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Find, Nothing, "Unit_Head", "Unit_Name", "", "")
        If Asc(e.KeyChar) = 13 Then
            btn_FindOpen_Click(sender, e)
        End If
    End Sub
