Public Class Beam_Width_Creation
    ...
    Private Sub Color_Creation_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1464" Then
            ...
            lbl_beam.Text = "BEAM WIDTH CREATION"
            Me.Text = "BEAM WIDTH CREATION"
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If (Common_Procedures.User.IdNo) <> 1 And InStr(Common_Procedures.UR.Masters_Beam_Width_Creation, "~L~") = 0 And InStr(Common_Procedures.UR.Masters_Beam_Width_Creation, "~D~") = 0 Then MessageBox.Show("You have No Rights To Delete", "DOES Not DELETE...", MessageBoxButtons.OK, MessageBoxIcon.Error) : Exit Sub
        ...
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "delete from Beam_Width_Head where Beam_Width_IdNo = " & Str(Val(lbl_IdNo.Text))
    ...
    Public Sub filter_record() Implements Interface_MDIActions.filter_record
        Dim da As New SqlClient.SqlDataAdapter("Select Beam_Width_IdNo, Beam_Width_Name from Beam_Width_Head where Beam_Width_IdNo <> 0 order by Beam_Width_IdNo", con)
        ...
        With dgv_filter
            ...
            .Columns(1).HeaderText = "BEAM WIDTH NAME"
    ...
    Public Sub move_record(ByVal idno As Integer)
        ...
        da = New SqlClient.SqlDataAdapter("Select * from Beam_Width_head a where Beam_Width_idno = " & Str(Val(idno)), con)
        da.Fill(dt)

        If dt.Rows.Count > 0 Then
            lbl_IdNo.Text = dt.Rows(0).Item("Beam_Width_IdNo").ToString
            txt_Name.Text = dt.Rows(0).Item("Beam_Width_Name").ToString
            txt_Rate.Text = dt.Rows(0).Item("Beam_Width_Rate").ToString
        End If
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select min(Beam_Width_idno) from Beam_Width_head Where Beam_width_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select max(Beam_Width_idno) from Beam_Width_head Where Beam_Width_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select min(Beam_width_idno) from Beam_Width_head Where Beam_Width_idno > " & Str(Val(lbl_IdNo.Text)) & " And Beam_Width_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("Select max(Beam_Width_idno) from Beam_Width_head Where Beam_width_idno < " & Str(Val(lbl_IdNo.Text)) & " And Beam_Width_idno <> 0", con)
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Beam_Width_Head", "Beam_Width_IdNo", "")
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        Dim da As New SqlClient.SqlDataAdapter("Select Beam_Width_Name from Beam_Width_Head order by Beam_Width_Name", con)
        Dim dt As New DataTable

        da.Fill(dt)

        cbo_Find.DataSource = dt
        cbo_Find.DisplayMember = "Beam_Width_Name"
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.Masters_Beam_Width_Creation, New_Entry, Me) = False Then Exit Sub
        ...
        Try
            ...
            If New_Entry = True Then
                ...
                lbl_IdNo.Text = Common_Procedures.get_MaxIdNo(con, "Beam_Width_Head", "Beam_Width_IdNo", "", trans)

                cmd.CommandText = "Insert into Beam_Width_Head(Beam_Width_IdNo, Beam_Width_Name, Sur_Name, Beam_Width_Rate) values (" & Str(Val(lbl_IdNo.Text)) & ", '" & Trim(txt_Name.Text) & "', '" & Trim(Sur) & "', " & Val(txt_Rate.Text) & ")"
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "update Beam_Width_Head set Beam_Width_Name = '" & Trim(txt_Name.Text) & "', Sur_Name = '" & Trim(Sur) & "', Beam_Width_Rate = " & Val(txt_Rate.Text) & "  where Beam_Width_IdNo = " & Str(Val(lbl_IdNo.Text))
                cmd.ExecuteNonQuery()
            ...
            Common_Procedures.Master_Return.Master_Type = "BEAMWIDTH"
        ...
        Catch ex As Exception
            trans.Rollback()
            If InStr(1, Trim(LCase(ex.Message)), "ix_beam_width_head") > 0 Then
                MessageBox.Show("Duplicate Beam Width Name", "DOES NOT SAVE", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub btn_Open_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_Open.Click
        ...
        da = New SqlClient.SqlDataAdapter("select Beam_Width_IdNo from Beam_Width_Head where Beam_Width_Name = '" & Trim(cbo_Find.Text) & "'", con)
        da.Fill(dt)
                ...
                If Asc(e.KeyChar) <> 27 Then
                    ...
                    Else
                        ...
                        If Trim(FindStr) <> "" Then
                            Condt = " Where Beam_Width_Name like '" & Trim(FindStr) & "%' or Beam_Width_Name like '% " & Trim(FindStr) & "%' "
                        End If

                        da = New SqlClient.SqlDataAdapter("select Beam_Width_Name from Beam_Width_Head " & Condt & " order by Beam_Width_Name", con)
                        da.Fill(dt)

                        .DataSource = dt
                        .DisplayMember = "Beam_Width_Name"
