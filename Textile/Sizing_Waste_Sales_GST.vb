Imports System.IO
Public Class Sizing_Waste_Sales_GST
    ...
    Private Pk_Condition As String = "SIZWS-"
    '  Private Pk_Condition As String = "GSCWS-"
    Private Pk_Condition2 As String = "SIWSA-"
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        clr_ID = 0
                        'Vrty_ID = Common_Procedures.Waste_NameToIdNo(con, .Rows(i).Cells(1).Value, tr)
                        ...
                        'cmd.CommandText = "Insert into Stock_WasteMaterial_Processing_Details ( Reference_Code      ,             Company_IdNo         ,           Reference_No         ,            For_OrderBy    , Reference_Date,        Ledger_IdNo      ,      Party_Bill_No   ,           Sl_No      ,          Waste_IdNo      ,                           Quantity             ) " &
                        '                     "          Values                              ('" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_BillNo.Text) & "', " & Str(Val(vORDBYNO)) & ",    @SalesDate , " & Str(Val(Led_ID)) & ", '" & Trim(PBlNo) & "', " & Str(Val(Sno)) & ", " & Str(Val(Vrty_ID)) & ", " & Str(-1 * Val(.Rows(i).Cells(5).Value)) & ")"
                        'cmd.ExecuteNonQuery()
                        ...
                        '                        "           Values                         (" & Str(Val(Common_Procedures.SoftwareTypes.Sizing_Software)) & " , '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_BillNo.Text) & "', " & Str(Val(vORDBYNO)) & ",    @SalesDate   , '" & Trim(EntID) & "' ,'" & Trim(PBlNo) & "'," & Val(Sno) & ", " & Str(Val(Led_ID)) & ", " & Str(Val(Vrty_ID)) & ", " & Str(-1 * Val(.Rows(i).Cells(3).Value)) & " , " & Str(-1 * Val(.Rows(i).Cells(5).Value)) & " ) "
                        '    cmd.ExecuteNonQuery()
    ...
    Private Sub Printing_Format2_PageHeader(ByRef e As System.Drawing.Printing.PrintPageEventArgs, ByVal EntryCode As String, ByVal TxtHgt As Single, ByVal pFont As Font, ByVal LMargin As Single, ByVal RMargin As Single, ByVal TMargin As Single, ByVal BMargin As Single, ByVal PageWidth As Single, ByVal PrintWidth As Single, ByRef PageNo As Integer, ByRef NoofItems_PerPage As Integer, ByRef CurY As Single, ByRef LnAr() As Single, ByRef ClAr() As Single, ByVal vLine_Pen As Pen)
        ...
        Try
            ...
            p1Font = New Font("Calibri", 11, FontStyle.Bold)
            If prn_HdDt.Rows(0).Item("Party_Name").ToString <> "" Then
                Common_Procedures.Print_To_PrintDocument(e, "M/s. " & prn_HdDt.Rows(0).Item("Party_Name").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, p1Font)
            Else
            ...
            If prn_HdDt.Rows(0).Item("Address1").ToString <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Address1").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("Address2").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Address2").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("Address3").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Address3").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
            ...
            If Trim(prn_HdDt.Rows(0).Item("Address4").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, prn_HdDt.Rows(0).Item("Address4").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            ...
            'CurY2 = CurY2 + TxtHgt
            If Trim(prn_HdDt.Rows(0).Item("Gstin_No").ToString) <> "" Then
                CurY2 = CurY2 + TxtHgt
                Common_Procedures.Print_To_PrintDocument(e, "GSTIN : " & prn_HdDt.Rows(0).Item("Gstin_No").ToString, LMargin + C1 + S1 + 10, CurY2, 0, 0, pFont)
            Else
                CurY2 = CurY2 + TxtHgt
    ...
    Private Sub Cbo_TaxType_KeyDown(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_TaxType.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_TaxType, txt_VehicleNo, cbo_DeliveryTo, "", "", "", "")
    End Sub
    ...
    Private Sub btn_Generate_eInvoice_Click(sender As Object, e As EventArgs) Handles btn_Generate_eInvoice.Click
        ...
        Try
            ...
            '                  " IGST_Amount , 0, 0, RoundOff_Amount, Net_Amount as Nett_Invoice_Amount, '" & Trim(NewCode) & "', 0 as OtherCharges from Cotton_Waste_Sales_Head where Cotton_Waste_Sales_Code = '" & Trim(NewCode) & "'"
            'Cmd.ExecuteNonQuery()
    ...
    Private Sub cbo_Transport_KeyDown(sender As Object, e As KeyEventArgs) Handles cbo_Transport.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Transport, txt_VehicleNo, cbo_DeliveryTo, "Ledger_AlaisHead", "Ledger_DisplayName", "(Ledger_Type = 'TRANSPORT')", "(Ledger_IdNo = 0)")
    End Sub
