Public Class Employee_payment_Entry
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, b.Employee_Name,m.Name as Payment  from Employee_Payment_Head a LEFT OUTER JOIN PayRoll_Employee_Head b ON a.ledger_IdNo = b.EMployee_IdNo LEFT OUTER JOIN Mode_Of_payment_Head M ON a.Payment_IdNo = M.IdNo  where a.Employee_Payment_Code = '" & Trim(NewCode) & "' ", con)
            dt1 = New DataTable
            da1.Fill(dt1)

            If dt1.Rows.Count > 0 Then
                lbl_RecNo.Text = dt1.Rows(0).Item("Employee_Payment_No").ToString
                ' dtp_Date.Text = dt1.Rows(0).Item("Employee_Payment_Date").ToString

                dtp_voucher_date.Text = dt1.Rows(0).Item("Employee_Payment_Date").ToString
                Msk_Date_Voucher.Text = dtp_voucher_date.Text

                cbo_PartyName.Text = dt1.Rows(0).Item("Employee_Name").ToString
                txt_Amount.Text = Format(Val(dt1.Rows(0).Item("Amount").ToString), "#########0.00")
                cbo_PaymentMode.Text = dt1.Rows(0).Item("Payment").ToString
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        Try
            ...
            cmd.CommandText = "delete from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code = '" & Trim(NewCode) & "' "
            cmd.ExecuteNonQuery()
    ...
    Public Sub insert_record() Implements Interface_MDIActions.insert_record
        ...
        Try
            ...
            Da = New SqlClient.SqlDataAdapter("select Employee_Payment_No from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code = '" & Trim(Pk_Condition) & Trim(InvCode) & "' AND Employee_Payment_Code LIKE '" & Trim(Pk_Condition) & "%'", con)
            Da.Fill(Dt)
    ...
    Public Sub movefirst_record() Implements Interface_MDIActions.movefirst_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Employee_Payment_No from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' AND Employee_Payment_Code LIKE '" & Trim(Pk_Condition) & "%'  Order by for_Orderby, Employee_Payment_No", con)
            dt = New DataTable
            da.Fill(dt)
    ...
    Public Sub movelast_record() Implements Interface_MDIActions.movelast_record
        ...
        Try
            da = New SqlClient.SqlDataAdapter("select top 1 Employee_Payment_No from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' AND Employee_Payment_Code LIKE '" & Trim(Pk_Condition) & "%' Order by for_Orderby desc, Employee_Payment_No desc", con)
            dt = New DataTable
            da.Fill(dt)
    ...
    Public Sub movenext_record() Implements Interface_MDIActions.movenext_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Employee_Payment_No from Employee_Payment_Head where for_orderby > " & Str(Val(OrdByNo)) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' AND Employee_Payment_Code LIKE '" & Trim(Pk_Condition) & "%' Order by for_Orderby, Employee_Payment_No", con)
            dt = New DataTable
            da.Fill(dt)
    ...
    Public Sub new_record() Implements Interface_MDIActions.new_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select max(for_orderby) from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' ", con)
            dt = New DataTable
            da.Fill(dt)
            ...
            Da1 = New SqlClient.SqlDataAdapter("select top 1 a.* ,m.name  as payment from Employee_Payment_Head a LEFT OUTER JOIN Mode_Of_payment_Head M ON a.Payment_IdNo = M.IdNo where a.company_idno = " & Str(Val(lbl_Company.Tag)) & "and  a.Employee_Payment_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by  a.Employee_Payment_No desc, a.For_OrderBy desc", con)
            Dt1 = New DataTable
            Da1.Fill(Dt1)
            If Dt1.Rows.Count > 0 Then
                '  dtp_Date.Text = Dt1.Rows(0).Item("Employee_Payment_Date").ToString
    ...
    Public Sub open_record() Implements Interface_MDIActions.open_record
        ...
        Try
            ...
            cmd.CommandText = "select Employee_Payment_No from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & "and Employee_Payment_No = '" & Trim(inpno) & "' and Employee_Payment_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "'"
            'cmd.CommandText = "select Employee_Payment_No from Employee_Payment_Head where company_idno = " & Str(Val(lbl_Company.Tag)) & "and Employee_Payment_Code = '" & Trim(NewCode) & "'"
    ...
    Public Sub print_record() Implements Interface_MDIActions.print_record
        ...
        Try
            ...
            da1 = New SqlClient.SqlDataAdapter("select a.*, c.ledger_name , d.ledger_name as debitor_name, c.ledger_address1, c.ledger_address2, c.ledger_address3, c.ledger_address4 from Employee_Payment_head a, ledger_head c, ledger_head d where a.company_idno = " & Str(Val(lbl_Company.Tag)) & "and a.Employee_Payment_code = '" & Trim(entcode) & "' and a.ledger_idno = c.ledger_idno and a.DebitAc_IdNo = d.ledger_idno", con)
            da1.Fill(dt1)
            ' else
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Da1 = New SqlClient.SqlDataAdapter("select a.*, cm.*,c.ledger_name , d.ledger_name as debitor_name, c.ledger_address1, c.ledger_address2, c.ledger_address3, c.ledger_address4 from Employee_Payment_head a, ledger_head c, ledger_head d,Company_Head cm where a.company_idno = " & Str(Val(lbl_Company.Tag)) & "and a.Employee_Payment_code = '" & Trim(entcode) & "' and a.ledger_idno = c.ledger_idno and a.DebitAc_IdNo = d.ledger_idno and  a.Company_idno = cm.Company_idno", con)
        Da1.Fill(prn_HdDt)
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Dim Mode_Pymnt As Integer = 0
        Dim Net_Amount As Integer = 0
        Dim vNarr As String = ""

        NewCode = Trim(Pk_Condition) & Trim(Val(lbl_Company.Tag)) & "-" & Trim(lbl_RecNo.Text) & "/" & Trim(Common_Procedures.FnYearCode)

        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.PAYROLL_ENTRY_EMPLOYEE_SALARY_ADVANCE_PAYMENT, New_Entry, Me, con, "Employee_Payment_Head", "Employee_Payment_Code", NewCode, "Employee_Payment_Date", "(Employee_Payment_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Employee_Payment_No desc", dtp_voucher_date.Value.Date) = False Then Exit Sub

        da = New SqlClient.SqlDataAdapter("select a.Idno From Mode_Of_payment_Head a where a.Name = '" & Trim(cbo_PaymentMode.Text) & "'", con)
        da.Fill(dt1)

        If dt1.Rows.Count > 0 Then
            Mode_Pymnt = dt1.Rows(0).Item("Idno").ToString()
        End If
        dt1.Clear()
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.Weaver_Advance_payment, New_Entry, Me, con, "Employee_Payment_Head", "Employee_Payment_Code", NewCode, "Employee_Payment_Date", "(Employee_Payment_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Employee_Payment_No desc", dtp_voucher_date.Value.Date) = False Then Exit Sub
        ...
        If Not (Convert.ToDateTime(Msk_Date_Voucher.Text) >= Common_Procedures.Company_FromDate And Convert.ToDateTime(Msk_Date_Voucher.Text) <= Common_Procedures.Company_ToDate) Then
            MessageBox.Show("Date is out of financial range", "DOES NOT SAVE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
            If Msk_Date_Voucher.Enabled And Msk_Date_Voucher.Visible Then Msk_Date_Voucher.Focus()
            Exit Sub
        End If

        If Not (Convert.ToDateTime(dtp_voucher_date.Text) >= Common_Procedures.Company_FromDate And Convert.ToDateTime(dtp_voucher_date.Text) <= Common_Procedures.Company_ToDate) Then
            MessageBox.Show("Date is out of financial range", "DOES NOT SAVE...", MessageBoxButtons.OKCancel, MessageBoxIcon.Error)
            If Msk_Date_Voucher.Enabled And Msk_Date_Voucher.Visible Then Msk_Date_Voucher.Focus()
            Exit Sub
        End If
        ...
        Mem_id = Common_Procedures.Employee_NameToIdNo(con, cbo_PartyName.Text)
        If Mem_id = 0 Then
        ...
        Try
            ...
            Else
                ...
                lbl_RecNo.Text = Common_Procedures.get_MaxCode(con, "Employee_Payment_Head", "Employee_Payment_Code", "For_OrderBy", "", Val(lbl_Company.Tag), Common_Procedures.FnYearCode, tr)
            ...
            If New_Entry = True Then
                ...
                cmd.CommandText = "Insert into Employee_Payment_Head (        Employee_Payment_Code   ,                 Company_IdNo     ,          Employee_Payment_No  ,                               for_OrderBy                               , Employee_Payment_Date,           Ledger_IdNo      ,                 Amount   ,               Payment_idno          ,                   DebitAc_IdNo        ,               Reference_No ,        ChequeBank_Name        ,    Cheque_Date    ,              Remarks    ) " &
                                  "Values                                    ( '" & Trim(NewCode) & "' , " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RecNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RecNo.Text))) & " ,     @EntryDate       , " & Str(Val(Mem_id)) & ",  " & Str(Val(txt_Amount.Text)) & "  , " & Val(Mode_Pymnt) & " ,  " & Val(CrdtAc_id) & "  ,  '" & Trim(txt_ReferenceNo.Text) & "','" & Trim(cbo_chequeBank.Text) & "' ,  @ChequeDate  ,  '" & Trim(txt_Remarks.Text) & "' ) "
                cmd.ExecuteNonQuery()

            Else

                cmd.CommandText = "Update Employee_Payment_Head set Employee_Payment_Date = @EntryDate, Ledger_IdNo = " & Str(Val(Mem_id)) & ", Amount = " & Str(Val(txt_Amount.Text)) & ",Payment_idno = " & Val(Mode_Pymnt) & ", DebitAc_IdNo = " & Str(Val(CrdtAc_id)) & ", Reference_No ='" & Trim(txt_ReferenceNo.Text) & "' ,ChequeBank_Name ='" & Trim(cbo_chequeBank.Text) & "', Cheque_Date = @ChequeDate , Remarks = '" & Trim(txt_Remarks.Text) & "'  Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Employee_Payment_Code = '" & Trim(NewCode) & "'  "
                cmd.ExecuteNonQuery()

            End If

            vNarr = "Vou.No : " & Trim(lbl_RecNo.Text) & "  Description  : " & Trim(txt_Remarks.Text)


            If Trim(cbo_PaymentMode.Text) = "Cash" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Cash.pymnt", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If

            ElseIf Trim(cbo_PaymentMode.Text) = "Canteen" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = -1 * Val(txt_Amount.Text) & "|" & (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Mess.deductn", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If


            ElseIf Trim(cbo_PaymentMode.Text) = "RTGS" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Rtgs.pymnt", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Cheque/DD" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Cheque.pymnt", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Debit Entry - Others" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Dr.Othrs.pymnt", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Credit Entry - Others" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = -1 * Val(txt_Amount.Text) & "|" & (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Cr.Othrs.pymnt", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Advance Paid - Bank" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Adv.bank", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If


            ElseIf Trim(cbo_PaymentMode.Text) = "Advance Paid - Cash" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Adv.cash", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Advance Deduction" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = -1 * Val(txt_Amount.Text) & "|" & (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Adv.Dedctn", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Deposit Paid - Bank" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Depst.bank", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If


            ElseIf Trim(cbo_PaymentMode.Text) = "Deposit Paid - Cash" Then

                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                vVou_Amts = Val(txt_Amount.Text) & "|" & -1 * (Val(txt_Amount.Text))
                If Common_Procedures.Voucher_Updation(con, "Depst.Cash", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If

            ElseIf Trim(cbo_PaymentMode.Text) = "Deposit A/C" Then

                '   Net_Amount = Val(txt_Amount.Text) + (Val(txt_Amount.Text) / 2)
                vLed_IdNos = CrdtAc_id & "|" & Mem_id
                '   vVou_Amts = -1 * Val(Net_Amount) & "|" & (Val(Net_Amount))
                vVou_Amts = -1 * Val(txt_Amount.Text) & "|" & (Val(txt_Amount.Text))

                If Common_Procedures.Voucher_Updation(con, "Depst.Ac", Val(lbl_Company.Tag), Trim(NewCode), Trim(lbl_RecNo.Text), Convert.ToDateTime(Msk_Date_Voucher.Text), vNarr, vLed_IdNos, vVou_Amts, ErrMsg, tr) = False Then
                    Throw New ApplicationException(ErrMsg)
                    Exit Sub
                End If
    ...
    Private Sub cbo_EmployeeName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PartyName.KeyDown
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PartyName, Msk_Date_Voucher, txt_Amount, "PayRoll_Employee_Head", "Employee_Name", "", "(Employee_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_EmployeeName_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PartyName.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PartyName, Nothing, "PayRoll_Employee_Head", "Employee_Name", "", "(Employee_IdNo = 0)")
    ...
    Private Sub btn_filtershow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_filtershow.Click
        ...
        Try
            ...
            ElseIf IsDate(dtp_FilterTo_date.Value) = True Then
                Condt = "a.Employee_Payment_Date= '" & Trim(Format(dtp_FilterTo_date.Value, "MM/dd/yyyy")) & "' "
            End If
            ...
            da = New SqlClient.SqlDataAdapter("select a.*, b.Ledger_Name as Member_Name from Employee_Payment_Head a INNER JOIN Ledger_Head b ON a.Ledger_Idno = b.Ledger_IdNo  where a.company_IdNo = " & Str(Val(lbl_Company.Tag)) & "and a.Employee_Payment_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "' " & IIf(Trim(Condt) <> "", " and ", "") & Condt & " Order by a.for_orderby, a.Employee_Payment_No", con)
            dt2 = New DataTable
            da.Fill(dt2)
    ...
    Private Sub cbo_CreditAccount_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_DebitorAccount.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Ledger_AlaisHead", "Ledger_DisplayName", "( AccountsGroup_idNo <>14 AND AccountsGroup_idNo <>10)", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_CreditAccount_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_DebitorAccount.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_DebitorAccount, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "( AccountsGroup_idNo <>14 AND AccountsGroup_idNo <>10)", "(Ledger_IdNo = 0)")
    ...
    Private Sub cbo_CreditAccount_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_DebitorAccount.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_DebitorAccount, cbo_PaymentMode, Nothing, "Ledger_AlaisHead", "Ledger_DisplayName", "( AccountsGroup_idNo <>14 AND AccountsGroup_idNo <>10)", "(Ledger_IdNo = 0)")
    ...
    Private Sub cbo_CashCheque_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PaymentMode.KeyPress
        ...
        cbo_PaymentMode.Text = StrConv(cbo_PaymentMode.Text, VbStrConv.Uppercase)
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PaymentMode, Nothing, "Mode_of_Payment_Head", "Name", "", "(id=0)")
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            If Trim(cbo_PaymentMode.Text) = "Cash" Then
                cbo_DebitorAccount.Text = Common_Procedures.Ledger_IdNoToName(con, Common_Procedures.CommonLedger.Cash_Ac)
                cbo_DebitorAccount.Enabled = False
                txt_Remarks.Focus()
            ElseIf Trim(cbo_PaymentMode.Text) = "Advance Paid - Cash" Then
                cbo_DebitorAccount.Text = Common_Procedures.Ledger_IdNoToName(con, Common_Procedures.CommonLedger.Cash_Ac)
                cbo_DebitorAccount.Enabled = False
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Advance Deduction" Then
                cbo_DebitorAccount.Text = Common_Procedures.Ledger_IdNoToName(con, Common_Procedures.CommonLedger.Advance_Deduction)
                cbo_DebitorAccount.Enabled = False
                txt_Remarks.Focus()
            ...
            ElseIf Trim(cbo_PaymentMode.Text) = "Deposit Paid - Cash" Then
                ...
                txt_Remarks.Focus()
            ElseIf Trim(cbo_PaymentMode.Text) = "Canteen" Then
                cbo_DebitorAccount.Text = Common_Procedures.Ledger_IdNoToName(con, Common_Procedures.CommonLedger.Canteen)
                cbo_DebitorAccount.Enabled = False
                txt_Remarks.Focus()
            ElseIf Trim(cbo_PaymentMode.Text) = "Deposit A/C" Then
                cbo_DebitorAccount.Text = Common_Procedures.Ledger_IdNoToName(con, Common_Procedures.CommonLedger.Deposit_ac)
                cbo_DebitorAccount.Enabled = False
                txt_Remarks.Focus()

            Else
                cbo_DebitorAccount.Enabled = True
    ...
    Private Sub cbo_CashCheque_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PaymentMode.KeyDown
        ...
        cbo_PaymentMode.Text = StrConv(cbo_PaymentMode.Text, VbStrConv.Uppercase)
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PaymentMode, txt_Amount, Nothing, "Mode_of_Payment_Head", "Name", "", "(id=0)")
        If (e.KeyValue = 40 And cbo_PaymentMode.DroppedDown = False) Or (e.Control = True And e.KeyValue = 40) Then
            e.Handled = True : e.SuppressKeyPress = True
            If cbo_DebitorAccount.Enabled = True Then
                cbo_DebitorAccount.Focus()
            Else
                txt_Remarks.Focus()
            ...
            If FrmLdSTS = True Then
                ...
                If Val(RptSubReport_Index) > 0 And Trim(RptSubReport_VouCode) <> "" Then
                    ...
                    If Common_Procedures.CompIdNo <> 0 Then
                        ...
                        OrdByNo_Code = ""
                        Da1 = New SqlClient.SqlDataAdapter("Select a.For_OrderBy from Employee_Payment_Head a Where a.company_idno = " & Str(Val(lbl_Company.Tag)) & " and a.Employee_Payment_Code = '" & Trim(RptSubReport_VouCode) & "' ", con)
                        Dt1 = New DataTable
                        Da1.Fill(Dt1)
                        If Dt1.Rows.Count > 0 Then
    ...
    Private Sub cbo_chequeBank_GotFocus(sender As Object, e As System.EventArgs) Handles cbo_chequeBank.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Employee_Payment_Head", "ChequeBank_Name", "", "")
    End Sub
    ...
    Private Sub cbo_chequeBank_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles cbo_chequeBank.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_chequeBank, txt_ReferenceNo, msk_Date, "Employee_Payment_Head", "ChequeBank_Name", "", "")
    End Sub
    ...
    Private Sub cbo_chequeBank_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_chequeBank.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_chequeBank, msk_Date, "Employee_Payment_Head", "ChequeBank_Name", "", "", False)
    End Sub
    ...
    Private Sub get_Ledger_CurrentBalance()
        ...
        Try
            ...
            Led_ID = Common_Procedures.Employee_NameToIdNo(con, cbo_PartyName.Text)
    ...
    Private Sub txt_ReferenceNo_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_ReferenceNo.KeyDown
    ...
    Private Sub txt_ReferenceNo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_ReferenceNo.KeyPress
        If Asc(e.KeyChar) = 13 Then
            If Asc(e.KeyChar) = 13 Then
                If MessageBox.Show("Do you want to save?", "FOR SAVING...", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    save_record()
    ...
    Private Sub cbo_PaymentMode_GotFocus(sender As Object, e As EventArgs) Handles cbo_PaymentMode.GotFocus
        Common_Procedures.ComboBox_ItemSelection_SetDataSource(sender, con, "Mode_of_payment_Head", "Name", "", "(IdNo = 0)")
    End Sub
End Class