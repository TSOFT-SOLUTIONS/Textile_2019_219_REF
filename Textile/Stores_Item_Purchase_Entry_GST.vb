...
Public Class Stores_Item_Purchase_Entry_GST
    ...
    Public vmskBillText As String = ""
    Public vmskBillStrt As Integer = -1
    ...
    Private Sub clear()
        ...
        cbo_PurchaseAc.Enabled = True
        cbo_PurchaseAc.BackColor = Color.White

        cbo_PaymentMethod.Enabled = True
        cbo_PaymentMethod.BackColor = Color.White

        cbo_TaxType.Enabled = True
        cbo_TaxType.BackColor = Color.White
    ...
    Private Sub Item_Purchase_Entry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1391" Then
            lbl_BillDate.Visible = True
            msk_Bill_Date.Visible = True
            dtp_Billdate.Visible = True
        Else
            lbl_BillDate.Visible = False
            msk_Bill_Date.Visible = False
        ...
        AddHandler msk_Bill_Date.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler msk_Bill_Date.LostFocus, AddressOf ControlGotFocus
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                msk_Bill_Date.Text = dt1.Rows(0).Item("Bill_Date").ToString
                ...
                If IsDBNull(dt1.Rows(0).Item("Item_Purchase_BillMaking_Code").ToString) = False Then
                    If Trim(dt1.Rows(0).Item("Item_Purchase_BillMaking_Code").ToString) <> "" Then

                        If Trim(dt1.Rows(0).Item("Item_Purchase_BillMaking_Code").ToString) <> "1-9999/CLOSE" Then
                ...
                If LockSTS = True Then
                    ...
                    cbo_PurchaseAc.Enabled = False
                    cbo_PurchaseAc.BackColor = Color.LightGray

                    cbo_PaymentMethod.Enabled = False
                    cbo_PaymentMethod.BackColor = Color.LightGray

                    cbo_TaxType.Enabled = False
                    cbo_TaxType.BackColor = Color.LightGray

                    txt_BillNo.Enabled = False
                    txt_BillNo.BackColor = Color.LightGray
    ...
    Public Sub delete_record() Implements Interface_MDIActions.delete_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.DeletingEntry, Common_Procedures.UR.Store_purchase_Inward_entry, New_Entry, Me, con, "Stores_Item_Purchase_Head", "Item_Purchase_Code", NewCode, "Item_Purchase_Date", "(Item_Purchase_Code = '" & Trim(NewCode) & "')") = False Then Exit Sub
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1061" OR Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1558" Then
            ...
            Da = New SqlClient.SqlDataAdapter("select A. *, B.ledger_name  FROM Stores_Item_Purchase_HEAD A  LEFT OUTER JOIN LEDGER_HEAD B  ON A.LEDGER_IDNO=B.LEDGER_IDNO WHERE  A.ITEM_PURCHASE_BILLMAKING_Code <>'' AND  A.ITEM_PURCHASE_CODE= '" & Trim(Pk_Condition) & Trim(NewCode) & "'   AND b.LEDGER_name ='" & Trim(cbo_Ledger.Text) & "'  ", con)
            Dt2 = New DataTable
            Da.Fill(Dt2)

            If Dt2.Rows.Count > 0 Then

                If IsDBNull(Dt2.Rows(0).Item("ITEM_PURCHASE_BILLMAKING_Code").ToString) = False Then
                    If Trim(Dt2.Rows(0).Item("ITEM_PURCHASE_BILLMAKING_Code").ToString) <> "" Then
                        If Trim(Dt2.Rows(0).Item("ITEM_PURCHASE_BILLMAKING_Code").ToString) <> "1-9999/CLOSE" Then
        ...
        Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "DELETE", "Stores_Item_Purchase_Head", "Item_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, "", "", "Item_Purchase_Code, Company_IdNo, for_OrderBy", trans)

        Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "DELETE", "Stores_Item_Purchase_Details", "Item_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, True, "Item_IdNo,Brand_IdNo,Quantity,Unit_idNo,Rate,Amount,Po_No,Po_Code,PO_Details_SlNo, PurchaseReturn_Quantity,Taxable_Value,GST_Percentage,HSN_Code", "Sl_No", "Item_Purchase_Code, For_OrderBy, Company_IdNo, Item_Purchase_No, Item_Purchase_Date, Ledger_Idno", trans)
    ...
    Public Sub moveprevious_record() Implements Interface_MDIActions.moveprevious_record
        ...
        Try
            ...
            da = New SqlClient.SqlDataAdapter("select top 1 Item_Purchase_No from Stores_Item_Purchase_Head where for_orderby < " & Str(OrdByNo) & " and company_idno = " & Str(Val(lbl_Company.Tag)) & " and  Item_Purchase_Code LIKE '" & Trim(Pk_Condition) & "%' AND  Item_Purchase_Code like '%/" & Trim(Common_Procedures.FnYearCode) & "' Order by for_Orderby desc, Item_Purchase_No desc", con)
            da.Fill(dt)
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        If Common_Procedures.UserRight_NEWCheck(Common_Procedures.UserRightsCheckFor.SavingEntry, Common_Procedures.UR.Store_purchase_Inward_entry, New_Entry, Me, con, "Stores_Item_Purchase_Head", "Item_Purchase_Code", NewCode, "Item_Purchase_Date", "(Item_Purchase_Code = '" & Trim(NewCode) & "')", "(Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Item_Purchase_Code LIKE '%/" & Trim(Common_Procedures.FnYearCode) & "')", "for_Orderby desc, Item_Purchase_No desc", dtp_date.Value.Date) = False Then Exit Sub
        ...
        vBillDt = ""
        If Trim(msk_Bill_Date.Text) <> "" Then
            If IsDate(msk_Bill_Date.Text) = True Then
                vBillDt = Trim(msk_Bill_Date.Text)
            End If
        End If
        ...
        If Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1061" OR Trim(UCase(Common_Procedures.settings.CustomerCode)) = "1558" Then
            ...
            If Val(Led_IdNo) <> 0 Then
                Da = New SqlClient.SqlDataAdapter("select A. * FROM Stores_Item_Purchase_Head A   WHERE  A.ITEM_PURCHASE_BILLMAKING_Code <>'' AND  A.ITEM_PURCHASE_CODE= '" & Trim(Pk_Condition) & Trim(NewCode) & "'   AND  A.LEDGER_IDNO =" & Str(Val(Led_IdNo)) & "  ", con)
                Dt2 = New DataTable
                Da.Fill(Dt2)
        ...
        Try
            ...
            End If
            Call Common_Procedures.User_Modification_Updation(con, "HEAD", Me.Name, "NEW", "Stores_Item_Purchase_Head", "Item_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "", "", "Item_Purchase_Code, Company_IdNo, for_OrderBy", tr)
            ...
            With dgv_Details
                ...
                For i = 0 To dgv_Details.RowCount - 1
                    ...
                    If Val(dgv_Details.Rows(i).Cells(5).Value) <> 0 Then
                        ...
                        If Nr = 0 Then
                            cmd.CommandText = "Insert into Stores_Item_Purchase_Details (   Item_Purchase_Code      ,           Company_IdNo            ,       Item_Purchase_No        ,                       for_OrderBy                                     ,Item_Purchase_Date,        Entry_Type          ,           Ledger_IdNo         ,           Bill_No                 ,   Sl_No           ,           Item_IdNo       ,           Brand_IdNo          ,           Quantity                ,       Unit_idNo       ,                   Rate                    ,               Amount                  ,                   Po_No                   ,               Po_Code                     ,               PO_Details_SlNo         , PurchaseReturn_Quantity   ,               Taxable_Value               ,           GST_Percentage                  ,           HSN_Code    , Catelog_pageno  , Motion_Type_IdNo   ) " &
                ...
                Next
                Call Common_Procedures.User_Modification_Updation(con, "DETAILS", Me.Name, "NEW", "Stores_Item_Purchase_Details", "Item_Purchase_Code", Val(lbl_Company.Tag), NewCode, lbl_RefNo.Text, Val(vOrdByNo), Pk_Condition, "", "", New_Entry, False, "Item_IdNo,Brand_IdNo,Quantity,Unit_idNo,Rate,Amount,Po_No,Po_Code,PO_Details_SlNo, PurchaseReturn_Quantity,Taxable_Value,GST_Percentage,HSN_Code", "Sl_No", "Item_Purchase_Code, For_OrderBy, Company_IdNo, Item_Purchase_No, Item_Purchase_Date, Ledger_Idno", tr)
            ...
            cmd.CommandText = "insert into Entry_GST_Tax_Details_Temp(GST_Percentage, CGST_Amount, SGST_Amount, IGST_Amount) select (CGST_Percentage + SGST_Percentage + IGST_Percentage), sum(CGST_Amount), sum(SGST_Amount), sum(IGST_Amount) from Store_Item_Purchase_GST_Tax_Details where Store_Item_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and (CGST_Percentage <> 0 or SGST_Percentage <> 0 or IGST_Percentage <> 0) and (CGST_Amount <> 0 or SGST_Amount <> 0 or IGST_Amount <> 0) Group by CGST_Percentage,  SGST_Percentage,  IGST_Percentage Having sum(CGST_Amount) <> 0 or sum(SGST_Amount) <> 0 or sum(IGST_Amount) <> 0"
            cmd.ExecuteNonQuery()
    ...
    Private Sub cbo_Grid_Item_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Grid_Item.KeyPress
        ...
        If Asc(e.KeyChar) = 13 Then
            ...
            If Trim(dgv_Details.Rows(dgv_Details.CurrentRow.Index).Cells(1).Value) = "" Or Trim(dgv_Details.Rows(dgv_Details.CurrentRow.Index).Cells(2).Value) = "" Or Trim(dgv_Details.Rows(dgv_Details.CurrentRow.Index).Cells(6).Value) = "" Or Trim(dgv_Details.Rows(dgv_Details.CurrentRow.Index).Cells(11).Value) = "" Or Trim(UCase(vCbo_ItmNm)) <> Trim(UCase(cbo_Grid_Item.Text)) Then
                ...
                dgv_Details.Rows(dgv_Details.CurrentRow.Index).Cells(4).Value = Trim(Brnd_nm)
    ...
    Private Sub PrintDocument1_BeginPrint(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintEventArgs) Handles PrintDocument1.BeginPrint
        ...
        Try
            ...
            If prn_HdDt.Rows.Count > 0 Then
                ...
                da2 = New SqlClient.SqlDataAdapter("select a.*, b.Item_name, d.Unit_name,  f.Brand_Name,g.PO_Date  from Stores_Item_Purchase_Details a INNER JOIN Stores_Item_Head b ON a.Item_idno = b.Item_idno LEFT OUTER JOIN Unit_Head d ON a.Unit_idno = d.Unit_idno  LEFT OUTER JOIN Brand_Head f ON a.Brand_idno = f.Brand_idno LEFT OUTER JOIN Stores_Item_PO_Head g ON g.PO_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "'  where a.Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and a.Item_Purchase_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' Order by a.Sl_No", con)
    ...
    Private Sub txt_BillNo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txt_BillNo.KeyDown
        ...
        If e.KeyValue = 40 Then
            ...
            If msk_Bill_Date.Visible = True Then
    ...
    Private Sub cbo_PurchaseAc_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_PurchaseAc.KeyDown
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_PurchaseAc, cbo_Ledger, txt_BillNo, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 27 or AccountsGroup_IdNo = 15 or AccountsGroup_IdNo = 16 )", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub cbo_PurchaseAc_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_PurchaseAc.KeyPress
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_PurchaseAc, txt_BillNo, "Ledger_AlaisHead", "Ledger_DisplayName", "(AccountsGroup_IdNo = 27 or AccountsGroup_IdNo = 15 or AccountsGroup_IdNo = 16 )", "(Ledger_IdNo = 0)")
    End Sub
    ...
    Private Sub msk_Bill_Date_KeyDown(sender As Object, e As KeyEventArgs) Handles msk_Bill_Date.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        vmskBillText = ""
        vmskBillStrt = -1
        If e.KeyCode = 46 Or e.KeyCode = 8 Then
            vmskBillText = msk_Bill_Date.Text
            vmskBillStrt = msk_Bill_Date.SelectionStart
        End If
    ...
    Private Sub msk_Bill_Date_KeyPress(sender As Object, e As KeyPressEventArgs) Handles msk_Bill_Date.KeyPress
        If Trim(UCase(e.KeyChar)) = "D" Then
            msk_Bill_Date.Text = Date.Today
            msk_Bill_Date.SelectionStart = 0
        End If
    ...
    Private Sub msk_Bill_Date_KeyUp(sender As Object, e As KeyEventArgs) Handles msk_Bill_Date.KeyUp
        If IsDate(msk_Bill_Date.Text) = True Then
            If e.KeyCode = 107 Then
                msk_Bill_Date.Text = DateAdd("D", 1, Convert.ToDateTime(msk_Bill_Date.Text))
            ElseIf e.KeyCode = 109 Then
                msk_Bill_Date.Text = DateAdd("D", -1, Convert.ToDateTime(msk_Bill_Date.Text))
            End If
        End If
        If e.KeyCode = 46 Or e.KeyCode = 8 Then
            Common_Procedures.maskEdit_Date_ON_DelBackSpace(sender, e, vmskBillText, vmskBillStrt)
        End If
    End Sub

    Private Sub msk_Bill_Date_TextChanged(sender As Object, e As EventArgs) Handles msk_Bill_Date.TextChanged
        Try


            If FrmLdSTS = True Then Exit Sub

            If Me.ActiveControl.Name <> dtp_Billdate.Name Then
                If IsDate(msk_Bill_Date.Text) = True Then
                    dtp_Billdate.Value = Convert.ToDateTime(msk_Bill_Date.Text)
                End If
            End If
    ...
    Private Sub msk_Bill_Date_LostFocus(sender As Object, e As EventArgs) Handles msk_Bill_Date.LostFocus
        If IsDate(msk_Bill_Date.Text) = True Then
            If Microsoft.VisualBasic.DateAndTime.Day(Convert.ToDateTime(msk_Bill_Date.Text)) <= 31 Or Microsoft.VisualBasic.DateAndTime.Month(Convert.ToDateTime(msk_Bill_Date.Text)) <= 31 Then
                If Microsoft.VisualBasic.DateAndTime.Year(Convert.ToDateTime(msk_Bill_Date.Text)) <= 2050 And Microsoft.VisualBasic.DateAndTime.Year(Convert.ToDateTime(msk_Bill_Date.Text)) >= 2000 Then
                    dtp_Billdate.Value = Convert.ToDateTime(msk_Bill_Date.Text)
                End If
            End If
        End If
    ...
    Private Sub dtp_Billdate_TextChanged(sender As Object, e As EventArgs) Handles dtp_Billdate.TextChanged
        Try
            If FrmLdSTS = True Then Exit Sub
            If Me.ActiveControl.Name <> msk_Bill_Date.Name Then
                If IsDate(dtp_Billdate.Text) = True Then
                    msk_Bill_Date.Text = dtp_Billdate.Text
                    msk_Bill_Date.SelectionStart = 0
                End If
            End If
    ...
    Private Sub dtp_Billdate_KeyUp(sender As Object, e As KeyEventArgs) Handles dtp_Billdate.KeyUp
