Public Class Yarn_Bag_Mixing_Opening_OE
    ...
    Private Sub move_record(ByVal no As String)
        ...
        Try
            ...
            If dt1.Rows.Count > 0 Then
                ...
                txt_Carding_Wgt.Text = Format(Val(dt1.Rows(0).Item("Carding_Weight").ToString), "########0.000")
    ...
    Private Sub Cotton_Opening_YarnBagMixing_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        AddHandler txt_Carding_Wgt.GotFocus, AddressOf ControlGotFocus
        ...
        AddHandler txt_Carding_Wgt.LostFocus, AddressOf ControlLostFocus
        ...
        AddHandler txt_Carding_Wgt.KeyDown, AddressOf TextBoxControlKeyDown
        AddHandler txt_Carding_Wgt.KeyPress, AddressOf TextBoxControlKeyPress
    ...
    Public Sub save_record() Implements Interface_MDIActions.save_record
        ...
        Try
            ...
            If New_Entry = True Then
                cmd.CommandText = "Insert into Cotton_Opening_YarnBagMixing_Head (       Cotton_Opening_YarnBagMixing_Code ,               Company_IdNo       ,           Cotton_Opening_YarnBagMixing_No    ,      for_OrderBy          ,       Count_IdNo         ,       Total_BagNo    ,         Total_Weight       ,     Loose_Yarn_Weight               ,         Mixing_Weight          , ConeType_IdNo    ,  Carding_Weight) " &
                                    "     Values                  (   '" & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ", " & Str(Val(Cnt_ID)) & " , " & Val(vTotBale) & ", " & Str(Val(vTotWght)) & " ," & Val(txt_loose_Yarn_Wgt.Text) & " ," & Val(txt_Mixing_Wgt.Text) & ", " & Val(Clr_ID) & "," & Val(txt_Carding_Wgt.Text) & " ) "
                cmd.ExecuteNonQuery()

            Else
                cmd.CommandText = "Update Cotton_Opening_YarnBagMixing_Head set Count_IdNo = " & Str(Val(Cnt_ID)) & " , Total_BagNo = " & Val(vTotBale) & ", Total_Weight = " & Str(Val(vTotWght)) & ",  Loose_Yarn_Weight = " & Val(txt_loose_Yarn_Wgt.Text) & "   ,    Mixing_Weight = " & Val(txt_Mixing_Wgt.Text) & " , ConeType_IdNo = " & Val(Clr_ID) & ",Carding_Weight = " & Val(txt_Carding_Wgt.Text) & " Where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & " and Cotton_Opening_YarnBagMixing_Code = '" & Trim(NewCode) & "'"
                cmd.ExecuteNonQuery()
            ...
            With dgv_Details
                ...
                For i = 0 To .RowCount - 1
                    ...
                    If Val(.Rows(i).Cells(2).Value) <> 0 Then
                        ...
                        Nr = 0
                        cmd.CommandText = "update Cotton_Packing_Details set Cotton_Packing_Date = @OpDate, Sl_No = " & Str(Val(Sno)) & ",  Count_IdNo =" & Str(Val(Cnt_ID)) & "  ,  ConeType_IdNo   =" & Str(Val(Clr_ID)) & "     , Bag_No = '" & Trim(Val(.Rows(i).Cells(1).Value)) & "',  Gross_Weight = " & Val(.Rows(i).Cells(2).Value) & ",Tare_Weight = 0 ,Net_Weight = " & Val(.Rows(i).Cells(2).Value) & " where Company_IdNo = " & Str(Val(lbl_Company.Tag)) & "   and Cotton_Opening_YarnBagMixing_Code = '" & Trim(NewCode) & "' and Cotton_Packing_Code = '" & Trim(Pk_Condition) & Trim(NewCode) & "' and Bag_Code = '" & Trim(PackingCode) & "'"
                        Nr = cmd.ExecuteNonQuery()

                        If Nr = 0 Then

                            cmd.CommandText = "Insert into Cotton_Packing_Details( Cotton_Opening_YarnBagMixing_Code , Cotton_Packing_Code       , Company_IdNo   ,               Cotton_Packing_No      , for_OrderBy       ,           Cotton_Packing_Date      ,           Sl_No ,               Count_IdNo   ,        ConeType_IdNo                       ,       Bag_No                   ,          Bag_Code  ,  Gross_Weight              ,  Tare_Weight                    , Net_Weight      ) " &
                        ...
                        cmd.CommandText = "Insert into Stock_Yarn_Processing_Details (       SoftwareType_IdNo  ,                                            Reference_Code                        ,             Company_IdNo                 ,           Reference_No        ,                               For_OrderBy                         ,        Reference_Date,     Party_Bill_No   ,                Sl_No      ,            Count_idNo      ,         Bags ,     Weight   ,ConeType_IdNo ) " &
                                                                  "   Values  ( " & Str(Val(Common_Procedures.SoftwareTypes.OE_Software)) & " ,  '" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(.Rows(i).Cells(1).Value) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(.Rows(i).Cells(1).Value))) & ",    @OpDate   , '" & Trim(PBlNo) & "', " & Str(Val(Sno)) & ", " & Str(Val(Cnt_ID)) & ",  1 , " & Str(Val(.Rows(i).Cells(2).Value)) & " , " & Val(Clr_ID) & " )"
                        cmd.ExecuteNonQuery()
            ...
            cmd.CommandText = "Insert into Stock_LooseYarn_Processing_Details ( Reference_Code                        ,             Company_IdNo                 ,           Reference_No        ,                               For_OrderBy                         ,        Reference_Date,     Party_Bill_No   ,        Sl_No      , Count_idNo      ,         Weight           ) " &
            ...
            cmd.CommandText = "Insert into Stock_Mixing_Processing_Details ( Reference_Code                        ,             Company_IdNo         ,           Reference_No        ,                               For_OrderBy                         ,        Reference_Date,     Party_Bill_No   ,   Sl_No                     ,   Count_IdNo   ,      Weight             ) " &
            ...
            cmd.CommandText = "Insert into Stock_Carding_Processing_Details ( Reference_Code                        ,             Company_IdNo         ,           Reference_No        ,             For_OrderBy         ,        Reference_Date, Entry_ID        ,   Party_Bill_No   ,   Sl_No    ,   Count_IdNo   ,      Weight             ) " &
                                "   Values  ('" & Trim(Pk_Condition) & Trim(NewCode) & "', " & Str(Val(lbl_Company.Tag)) & ", '" & Trim(lbl_RefNo.Text) & "', " & Str(Val(Common_Procedures.OrderBy_CodeToValue(lbl_RefNo.Text))) & ",    @OpDate   ,'" & Trim(EntID) & "' , '" & Trim(PBlNo) & "', " & Str(Val(Sno)) & ", " & Str(Val(Cnt_ID)) & ",  " & Str(Val(txt_Carding_Wgt.Text)) & " )"
            cmd.ExecuteNonQuery()
    ...
    Private Sub cbo_Conetype_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbo_Conetype.KeyDown
        vcbo_KeyDwnVal = e.KeyValue
        Common_Procedures.ComboBox_ItemSelection_KeyDown(sender, e, con, cbo_Conetype, cbo_Count, txt_Carding_Wgt, "ConeType_head", "ConeType_nAME", "", "(ConeType_IdNo = 0)")
    ...
    Private Sub cbo_Conetype_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cbo_Conetype.KeyPress
        ...
        Common_Procedures.ComboBox_ItemSelection_KeyPress(sender, e, con, cbo_Conetype, txt_Carding_Wgt, "ConeType_head", "ConeType_nAME", "", "(ConeType_IdNo = 0)")
    End Sub
